#!/usr/bin/env python3
"""Run a Gate7 anisotropic-projector closure consumer from Gate6 native objects."""
...
DEFAULT_TOKEN_CSV = "gate7c_token_telemetry.csv"
DEFAULT_SAMPLE_CSV = "gate7c_sample_summary.csv"
DEFAULT_AGGREGATE = "gate7c_aggregate_summary.md"

SCHEMA_VERSION = "gate7_progression_anisotropic_consumer_artifacts_v3"
METHOD_ID = "gate7_progression_anisotropic_consumer_v3"
PRIMARY_METRIC_ID = "progression_anisotropic_closure_v3"
PRIMARY_AUX_METRIC_ID = "progression_anisotropic_closure_energy_ratio_v3"
PRIMARY_LEAKAGE_METRIC_ID = "progression_anisotropic_closure_projected_norm_v3"
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description=(
            "Run a Gate7 anisotropic projector closure consumer that replaces P_t with "
...
def build_anisotropic_operator(
    basis: np.ndarray,
    ...
    if rank_local <= 0:
        return None
    basis_slice = np.asarray(basis[:, :rank_local], dtype=np.float64)
    singular_slice = np.asarray(singular_values[:rank_local], dtype=np.float64)
    sigma_1 = float(singular_slice[0]) if singular_slice.size else 0.0
    if not np.isfinite(sigma_1) or sigma_1 <= 1e-12:
        return None
    weights = np.clip(singular_slice / sigma_1, 0.0, 1.0)
    return basis_slice @ np.diag(weights) @ basis_slice.T
...
def compute_progression_metrics(
    ...
    current_operator = build_anisotropic_operator(current_basis, current_singular_values, current_rank)
    if current_operator is None:
        return {
            "loop_outcome": "invalid_current_operator",
    ...
    next_operator = build_anisotropic_operator(next_basis, next_singular_values, next_rank)
    if next_operator is None:
        return {
            "loop_outcome": "invalid_next_operator",
            BASELINE_METRIC_ID: None,
            PRIMARY_METRIC_ID: None,
            PRIMARY_AUX_METRIC_ID: None,
            PRIMARY_LEAKAGE_METRIC_ID: None,
    ...
    if not np.isfinite(next_norm_sq) or next_norm_sq <= 1e-12 or not np.isfinite(closure_norm_sq):
        return {
    ...
    closure_projected_norm = float(np.sqrt(max(0.0, closure_norm_sq)))
    return {
        "loop_outcome": "none",
        ...
        for idx, step_row in enumerate(rows):
            ...
            if is_final:
                ...
                metrics = compute_progression_metrics(
                    ...
                    current_singular_values=arrays["singular_values"][array_row_index],
                    ...
                    next_singular_values=arrays["singular_values"][next_row_index],
...
def build_aggregate_summary(
    ...
    lines = [
        "# Gate7 Progression Anisotropic Closure Consumer Summary",
        "",
        f"run_id: {run_id}",
