...
$dataDir = Join-Path $repoRoot "data\gate3"
$runA = Join-Path $repoRoot "runs\gate3_smoke_A"
$runB = Join-Path $repoRoot "runs\gate3_smoke_B"
$runKink = Join-Path $repoRoot "runs\gate3_smoke_kink"
...
python $genScript --mode smooth --n-samples 4 --n-steps 24 --seed 7 --noise 0.02 --run-id gate3_smooth_4x24 --out $smoothInput
python $genScript --mode kink --n-samples 4 --n-steps 24 --seed 7 --noise 0.02 --kink-at 12 --run-id gate3_kink_4x24 --out $kinkInput
...
Write-Host "[3/5] Running Gate3 smoke A/B/C..."
Invoke-Gate3Run -InputPath $smoothInput -OutDir $runA -EncoderId synthetic_smooth_v1
Invoke-Gate3Run -InputPath $smoothInput -OutDir $runB -EncoderId synthetic_smooth_v1
Invoke-Gate3Run -InputPath $kinkInput -OutDir $runKink -EncoderId synthetic_kink_v1
...
Write-Host "Gate3 smoke completed successfully."
