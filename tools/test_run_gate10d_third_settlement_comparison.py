#!/usr/bin/env python3
"""Regression tests for Gate10D third settlement comparison helpers."""
...
class RunGate10DThirdSettlementComparisonTest(unittest.TestCase):
    def test_registry_extracts_only_baseline_and_direct_contradiction(self) -> None:
        source_rows = [
            make_gate10b_row(
                edge_id="baseline-1",
                broader_candidate_class=gate10d.BASELINE_LANE,
            ...
            make_gate10b_row(
                edge_id="candidate-1",
                broader_candidate_class=gate10d.SOURCE_CANDIDATE_LANE,
                cell_id=gate10d.THIRD_CANDIDATE_CELL_ID,
                cell_class="conflict",
        ...
        registry = gate10d.build_comparison_registry(source_rows)

        self.assertEqual(len(registry), 2)
        self.assertEqual(registry[0]["comparison_lane"], gate10d.BASELINE_LANE)
        self.assertEqual(registry[1]["comparison_lane"], gate10d.THIRD_CANDIDATE_LANE)
        self.assertEqual(registry[1]["third_candidate_declaration"], "declaratively_extracted")

    def test_settled_when_third_candidate_is_declared_and_checks_clear(self) -> None:
        rows = [
            {
                **make_gate10b_row(
                    edge_id="baseline-1",
                ...
                "comparison_lane": gate10d.BASELINE_LANE,
            ...
            },
            {
                **make_gate10b_row(
                ...
                "comparison_lane": gate10d.THIRD_CANDIDATE_LANE,
                "third_candidate_declaration": "declaratively_extracted",
            },
        ]

        status = gate10d.build_status_payload(
            rows, GATE10B_STATUS_SETTLED, GATE10C_STATUS_SETTLED
        )

        self.assertEqual(status["third_candidate_declaration_status"], "clear")
        self.assertEqual(status["gate10b_slice_non_retroactive_preservation_status"], "clear")
        self.assertEqual(status["gate10c_slice_non_retroactive_preservation_status"], "clear")
    ...
    def test_denied_when_third_candidate_declaration_fails(self) -> None:
        rows = [
            {
                **make_gate10b_row(
                    edge_id="baseline-1",
        ...
        self.assertEqual(status["third_candidate_declaration_status"], "denied")
        self.assertEqual(status["comparison_outcome_status"], "denied")
        self.assertEqual(status["next_named_blocker"], "third_candidate_declaration_integrity_fails")
    ...
    def test_deferred_when_gate10c_source_slice_is_not_preserved(self) -> None:
        rows = [
            {
                **make_gate10b_row(
                    edge_id="baseline-1",
        ...
        gate10c_status = dict(GATE10C_STATUS_SETTLED)
        gate10c_status["comparison_outcome_status"] = "denied"
        gate10c_status["next_named_blocker"] = "conflict_side_bridge_degrades"

        status = gate10d.build_status_payload(rows, GATE10B_STATUS_SETTLED, gate10c_status)
