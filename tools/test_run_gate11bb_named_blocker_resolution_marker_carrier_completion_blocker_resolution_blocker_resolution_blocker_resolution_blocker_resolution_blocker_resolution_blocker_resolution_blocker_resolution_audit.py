#!/usr/bin/env python3
"""Regression tests for Gate11BB named blocker-resolution marker carrier-completion blocker-resolution blocker-resolution blocker-resolution blocker-resolution blocker-resolution blocker-resolution blocker-resolution helpers."""
...
def make_gate11ba_manifest(run_id: str = "gate11ba_run") -> dict:
    return {"run_id": run_id, "code_git_commit": "f50bb71928e2e88a234a773080f9a35421f38f3e"}


def make_gate11ba_status(**overrides: str) -> dict:
    status = {
...
class RunGate11BBNamedBlockerResolutionMarkerCarrierCompletionBlockerResolutionBlockerResolutionBlockerResolutionBlockerResolutionBlockerResolutionBlockerResolutionBlockerResolutionAuditTest(
    unittest.TestCase
):
    def test_not_yet_resolved_for_current_frozen_gate11ba_source(self) -> None:
        status = gate11bb.build_status_payload(
            make_gate11ba_manifest(),
            make_gate11ba_status(),
            "Gate11BA preserves the fixed blocker-resolution path but does not explicitly resolve the named blocker.",
        )

        self.assertEqual(status["gate11ba_path_defined_state_preservation_status"], "preserved")
        self.assertEqual(
            status[
    ...
    later_source_id: runs/gate11bb_future
    ...
    candidate_id: gate11bb_candidate_001
    class: host_failure
...
"""
        status = gate11bb.build_status_payload(make_gate11ba_manifest(), make_gate11ba_status(), report_text)
    ...
    def test_deferred_when_multiple_candidate_later_source_identifiers_compete(self) -> None:
        report_text = """
    blocker_resolution_marker_status: present
    later_source_id: runs/gate11bb_future_a
    later_frozen_run_id: runs/gate11bb_future_b
"""
        status = gate11bb.build_status_payload(make_gate11ba_manifest(), make_gate11ba_status(), report_text)
    ...
    def test_denied_when_boundary_breaks(self) -> None:
        status = gate11bb.build_status_payload(
            make_gate11ba_manifest(),
            make_gate11ba_status(blocker_resolution_boundary_status="not_confirmed"),
            "Gate11BA breaks the blocker-resolution boundary.",
        )
    ...
    def test_deferred_when_source_is_incomplete(self) -> None:
        status = gate11bb.build_status_payload(make_gate11ba_manifest(run_id=""), make_gate11ba_status(), "")
    ...
    def test_policy_compare_echoes_single_source_row(self) -> None:
        ...
        registry = gate11bb.build_registry(make_gate11ba_manifest(), status)
        compare = gate11bb.build_policy_compare(registry)

        self.assertEqual(len(compare), 1)
        self.assertEqual(compare[0]["source_gate11ba_run_id"], "gate11ba_run")
        self.assertEqual(
            compare[0][
                "named_blocker_resolution_marker_carrier_completion_blocker_resolution_blocker_resolution_blocker_resolution_blocker_resolution_blocker_resolution_blocker_resolution_blocker_resolution_status"
            ],
