#!/usr/bin/env python3
"""Run a Gate11Y residual marker-carrier completion blocker-resolution path audit on Gate11X outputs."""
...
SCHEMA_VERSION = "gate11y_residual_marker_carrier_completion_blocker_resolution_path_audit_v1"
METHOD_ID = "gate11y_residual_marker_carrier_completion_blocker_resolution_path_audit_v1"

DEFAULT_REGISTRY = "residual_marker_carrier_completion_blocker_resolution_path_registry.jsonl"
DEFAULT_POLICY_COMPARE = "residual_marker_carrier_completion_blocker_resolution_path_policy_compare.csv"
DEFAULT_STATUS = "residual_marker_carrier_completion_blocker_resolution_path_status.json"
DEFAULT_REPORT = "gate11y_residual_marker_carrier_completion_blocker_resolution_path_read.md"
DEFAULT_MANIFEST = "manifest.json"
DEFAULT_CHECKSUMS = "checksums.json"

REQUIRED_GATE11X_STATUS_KEYS = (
...
KNOWN_RESIDUAL_BLOCKERS = gate11x.KNOWN_RESIDUAL_BLOCKERS
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description=(
            "Build a Gate11Y residual marker-carrier completion blocker-resolution path audit "
            "from the frozen Gate11X blocker audit without resolving the blocker itself."
        )
    )
    parser.add_argument("--gate11x-dir", required=True)
...
def source_is_incomplete(
    source_gate11x_manifest: Dict[str, Any], source_gate11x_status: Dict[str, Any], report_text: str
) -> bool:
    if not str(source_gate11x_manifest.get("run_id") or ""):
        return True
    if not report_text.strip():
        return True
    return any(key not in source_gate11x_status for key in REQUIRED_GATE11X_STATUS_KEYS)


def blocker_sentence(blocker: str) -> str:
    mapping = {
        "no_explicit_residual_completion_marker": "the minimum honest path is fixed narrowly: one same later source must carry one explicit residual completion marker, one explicit later_source_id or later_frozen_run_id, one marker and only one marker, one explicit same-source carrier-completion status marked completed, one bounded read-prefix declaration for the marker, repeated bounded residual_completion_surface rows for the required same-source elements, one explicit admissible later-source presence marker, one declaration marker, one candidate id, one class, one explicit host-failure sentence, and matched status, registry, and read surfaces",
        "minimum_blocker_resolution_rule_not_fixed": "the named residual marker-carrier completion blocker remains preserved, but the minimum blocker-resolution rule is not yet fixed narrowly enough",
        "bounded_read_prefix_resolution_requirement_not_fixed": "the named residual marker-carrier completion blocker remains preserved, but the bounded read-prefix resolution requirement is not yet fixed narrowly enough",
        "named_blocker_not_preserved": "the controlling source no longer preserves the named residual marker-carrier completion blocker narrowly enough for path definition",
    }
    return mapping.get(blocker, "the residual marker-carrier completion blocker-resolution path is not yet fixed narrowly enough")


def build_registry(
    source_gate11x_manifest: Dict[str, Any],
    status_payload: Dict[str, Any],
) -> List[Dict[str, Any]]:
    return [
        {
            "source_gate11x_run_id": str(source_gate11x_manifest.get("run_id") or ""),
            "source_gate11x_code_git_commit": str(source_gate11x_manifest.get("code_git_commit") or ""),
            ...
            "minimum_blocker_resolution_rule_status": str(status_payload["minimum_blocker_resolution_rule_status"]),
            "bounded_read_prefix_resolution_requirement_status": str(status_payload["bounded_read_prefix_resolution_requirement_status"]),
            "blocker_resolution_boundary_status": str(status_payload["blocker_resolution_boundary_status"]),
            "residual_marker_carrier_completion_blocker_resolution_path_status": str(status_payload["residual_marker_carrier_completion_blocker_resolution_path_status"]),
...
def build_policy_compare(registry_rows: Sequence[Dict[str, Any]]) -> List[Dict[str, Any]]:
    return [
        {
            "source_gate11x_run_id": str(row["source_gate11x_run_id"]),
            ...
            "minimum_blocker_resolution_rule_status": str(row["minimum_blocker_resolution_rule_status"]),
            "bounded_read_prefix_resolution_requirement_status": str(row["bounded_read_prefix_resolution_requirement_status"]),
            "blocker_resolution_boundary_status": str(row["blocker_resolution_boundary_status"]),
            "residual_marker_carrier_completion_blocker_resolution_path_status": str(row["residual_marker_carrier_completion_blocker_resolution_path_status"]),
...
def build_status_payload(
    source_gate11x_manifest: Dict[str, Any],
    source_gate11x_status: Dict[str, Any],
    report_text: str,
) -> Dict[str, Any]:
    incomplete = source_is_incomplete(source_gate11x_manifest, source_gate11x_status, report_text)

    gate10_closeout_preservation_status = "preserved" if source_status_value(source_gate11x_status, "gate10_closeout_preservation_status") == "preserved" else "not_preserved"
    gate11a_absence_result_preservation_status = "preserved" if source_status_value(source_gate11x_status, "gate11a_absence_result_preservation_status") == "preserved" else "not_preserved"
    gate11c_declaration_surface_preservation_status = "preserved" if source_status_value(source_gate11x_status, "gate11c_declaration_surface_preservation_status") == "preserved" else "not_preserved"
    gate11d_not_yet_declared_state_preservation_status = "preserved" if source_status_value(source_gate11x_status, "gate11d_not_yet_declared_state_preservation_status") == "preserved" else "not_preserved"
    gate11e_path_defined_state_preservation_status = "preserved" if source_status_value(source_gate11x_status, "gate11e_path_defined_state_preservation_status") == "preserved" else "not_preserved"
    gate11f_not_yet_admissible_state_preservation_status = "preserved" if source_status_value(source_gate11x_status, "gate11f_not_yet_admissible_state_preservation_status") == "preserved" else "not_preserved"
    gate11g_naming_surface_preservation_status = "preserved" if source_status_value(source_gate11x_status, "gate11g_naming_surface_preservation_status") == "preserved" else "not_preserved"
    gate11h_not_yet_named_state_preservation_status = "preserved" if source_status_value(source_gate11x_status, "gate11h_not_yet_named_state_preservation_status") == "preserved" else "not_preserved"
    gate11i_path_defined_state_preservation_status = "preserved" if source_status_value(source_gate11x_status, "gate11i_path_defined_state_preservation_status") == "preserved" else "not_preserved"
    gate11j_not_yet_admissible_state_preservation_status = "preserved" if source_status_value(source_gate11x_status, "gate11j_not_yet_admissible_state_preservation_status") == "preserved" else "not_preserved"
    gate11k_not_yet_present_state_preservation_status = "preserved" if source_status_value(source_gate11x_status, "gate11k_not_yet_present_state_preservation_status") == "preserved" else "not_preserved"
    gate11l_path_defined_state_preservation_status = "preserved" if source_status_value(source_gate11x_status, "gate11l_path_defined_state_preservation_status") == "preserved" else "not_preserved"
    gate11m_not_yet_present_state_preservation_status = "preserved" if source_status_value(source_gate11x_status, "gate11m_not_yet_present_state_preservation_status") == "preserved" else "not_preserved"
    gate11n_residual_named_state_preservation_status = "preserved" if source_status_value(source_gate11x_status, "gate11n_residual_named_state_preservation_status") == "preserved" else "not_preserved"
    gate11o_path_defined_state_preservation_status = "preserved" if source_status_value(source_gate11x_status, "gate11o_path_defined_state_preservation_status") == "preserved" else "not_preserved"
    gate11p_not_yet_completed_state_preservation_status = "preserved" if source_status_value(source_gate11x_status, "gate11p_not_yet_completed_state_preservation_status") == "preserved" else "not_preserved"
    gate11q_surface_defined_state_preservation_status = "preserved" if source_status_value(source_gate11x_status, "gate11q_surface_defined_state_preservation_status") == "preserved" else "not_preserved"
    gate11r_not_yet_present_state_preservation_status = "preserved" if source_status_value(source_gate11x_status, "gate11r_not_yet_present_state_preservation_status") == "preserved" else "not_preserved"
    gate11s_path_defined_state_preservation_status = "preserved" if source_status_value(source_gate11x_status, "gate11s_path_defined_state_preservation_status") == "preserved" else "not_preserved"
    gate11t_not_yet_present_state_preservation_status = "preserved" if source_status_value(source_gate11x_status, "gate11t_not_yet_present_state_preservation_status") == "preserved" else "not_preserved"
    gate11u_residual_named_state_preservation_status = "preserved" if source_status_value(source_gate11x_status, "gate11u_residual_named_state_preservation_status") == "preserved" else "not_preserved"
    gate11v_path_defined_state_preservation_status = "preserved" if source_status_value(source_gate11x_status, "gate11v_path_defined_state_preservation_status") == "preserved" else "not_preserved"
    gate11w_not_yet_completed_state_preservation_status = "preserved" if source_status_value(source_gate11x_status, "gate11w_not_yet_completed_state_preservation_status") == "preserved" else "not_preserved"
    source_gate11x_status_value = source_status_value(source_gate11x_status, "residual_marker_carrier_completion_blocker_status")
    if source_gate11x_status_value == "blocker_named":
        gate11x_blocker_named_state_preservation_status = "preserved"
    elif source_gate11x_status_value == "deferred":
        gate11x_blocker_named_state_preservation_status = "deferred"
    else:
        gate11x_blocker_named_state_preservation_status = "not_preserved"
    broader_trusted_tree_settlement_still_unearned_status = "confirmed" if source_status_value(source_gate11x_status, "broader_trusted_tree_settlement_still_unearned_status") == "confirmed" else "not_confirmed"
    operator_admission_still_denied_status = "confirmed" if source_status_value(source_gate11x_status, "operator_admission_still_denied_status") == "confirmed" else "not_confirmed"
    retroactive_reinterpretation_forbidden_status = "confirmed" if source_status_value(source_gate11x_status, "retroactive_reinterpretation_forbidden_status") == "confirmed" else "not_confirmed"

    source_blocker = source_status_value(source_gate11x_status, "next_named_blocker")

    if incomplete:
        named_blocker_preservation_status = "deferred"
    elif (
        source_status_value(source_gate11x_status, "explicit_completion_marker_blocker_status") == "named"
        and source_status_value(source_gate11x_status, "same_source_carrier_completion_blocker_status") == "named"
    ...
    if incomplete:
        ...
        or source_status_value(source_gate11x_status, "carrier_completion_boundary_status") != "confirmed"
    ):
    ...
    if incomplete:
        minimum_blocker_resolution_rule_status = "deferred"
    elif blocker_resolution_boundary_status == "denied":
        minimum_blocker_resolution_rule_status = "not_defined"
    elif gate11x_blocker_named_state_preservation_status == "deferred":
        minimum_blocker_resolution_rule_status = "deferred"
    ...
    if incomplete:
        ...
        bounded_read_prefix_resolution_requirement_status = "not_defined"
    elif gate11x_blocker_named_state_preservation_status == "deferred":
        bounded_read_prefix_resolution_requirement_status = "deferred"
    elif minimum_blocker_resolution_rule_status != "defined":
        bounded_read_prefix_resolution_requirement_status = "not_defined"
    elif source_blocker in PATH_DEFINABLE_RESIDUAL_BLOCKERS:
    ...
    if incomplete:
        residual_marker_carrier_completion_blocker_resolution_path_status = "deferred"
    elif (
        ...
        or gate11x_blocker_named_state_preservation_status == "not_preserved"
        or blocker_resolution_boundary_status == "denied"
        or broader_trusted_tree_settlement_still_unearned_status != "confirmed"
        or operator_admission_still_denied_status != "confirmed"
        or retroactive_reinterpretation_forbidden_status != "confirmed"
    ):
        residual_marker_carrier_completion_blocker_resolution_path_status = "denied"
    elif (
        gate11x_blocker_named_state_preservation_status == "deferred"
        or named_blocker_preservation_status == "deferred"
        or minimum_blocker_resolution_rule_status == "deferred"
        ...
        named_blocker_preservation_status == "preserved"
        and minimum_blocker_resolution_rule_status == "defined"
        ...
        residual_marker_carrier_completion_blocker_resolution_path_status = "not_yet_defined"
    ...
    if gate10_closeout_preservation_status != "preserved":
    ...
    elif gate11x_blocker_named_state_preservation_status == "not_preserved":
        ...
        next_named_blocker = "minimum_blocker_resolution_rule_not_fixed"
...
def build_report(
    ...
    lines = [
        "# Gate11Y Residual Marker-Carrier Completion Blocker-Resolution Path Read",
        "",
        f"run_id: {run_id}",
        f"source_gate11x_run_id: {source_gate11x_manifest.get('run_id', '')}",
        f"source_gate11x_code_git_commit: {source_gate11x_manifest.get('code_git_commit', '')}",
        "",
        "## Discipline",
        "",
        "- Gate11Y asks only what is the minimum honest path by which the named residual marker-carrier completion blocker could later be resolved under the fixed Gate11X line",
        "- Gate11Y defines path only",
        "- Gate11Y does not resolve the blocker",
        "- Gate11Y does not instantiate a marker",
        "- Gate11Y does not complete the residual",
        "- Gate11Y does not admit a later source",
        "- Gate11Y does not decide one-admissible-later-source explicit-presence judgment",
        "- Gate11Y does not declare a bounded-line insufficiency candidate",
        "- Gate11Y does not declare that explicit declaration already exists",
        "- Gate11Y does not decide reopening eligibility",
        "- Gate11Y does not reopen operator admission",
    ...
    for row in policy_compare_rows:
        lines.append("| " + " | ".join([
            ...
            str(row["minimum_blocker_resolution_rule_status"]),
            str(row["bounded_read_prefix_resolution_requirement_status"]),
            str(row["blocker_resolution_boundary_status"]),
            str(row["residual_marker_carrier_completion_blocker_resolution_path_status"]),
    ...
    lines.extend([
        ...
        f"- minimum_blocker_resolution_rule_status: `{status_payload['minimum_blocker_resolution_rule_status']}`",
        f"- bounded_read_prefix_resolution_requirement_status: `{status_payload['bounded_read_prefix_resolution_requirement_status']}`",
        f"- blocker_resolution_boundary_status: `{status_payload['blocker_resolution_boundary_status']}`",
        f"- residual_marker_carrier_completion_blocker_resolution_path_status: `{status_payload['residual_marker_carrier_completion_blocker_resolution_path_status']}`",
        f"- next_named_blocker: `{status_payload['next_named_blocker']}`",
        "",
        "## Path Slice",
        "",
        "- Gate11Y defines only the minimum same-source path by which the named residual marker-carrier completion blocker could later be resolved",
    ...
    outcome = status_payload["residual_marker_carrier_completion_blocker_resolution_path_status"]
    if outcome == "path_defined":
        ...
        lines.append("- the frozen source is incomplete or contradictory for a residual marker-carrier completion blocker-resolution path judgment")
...
def main() -> None:
    args = parse_args()
    source_gate11x_dir = Path(args.gate11x_dir)
    out_dir = Path(args.out_dir)
    out_dir.mkdir(parents=True, exist_ok=True)
    run_id = out_dir.name

    source_gate11x_manifest = gate9a.read_json(source_gate11x_dir / gate11x.DEFAULT_MANIFEST)
    source_gate11x_status = gate9a.read_json(source_gate11x_dir / gate11x.DEFAULT_STATUS)
    source_gate11x_report = (source_gate11x_dir / gate11x.DEFAULT_REPORT).read_text(encoding="utf-8")

    status_payload = build_status_payload(source_gate11x_manifest, source_gate11x_status, source_gate11x_report)
    registry_rows = build_registry(source_gate11x_manifest, status_payload)
    ...
    gate9a.write_text(report_path, build_report(run_id, source_gate11x_manifest, policy_compare_rows, status_payload))
    ...
    manifest = {
        ...
        "source_gate11x_run_id": str(source_gate11x_manifest.get("run_id") or ""),
        "source_gate11x_code_git_commit": str(source_gate11x_manifest.get("code_git_commit") or ""),
        "inputs": {"gate11x_dir": str(source_gate11x_dir)},
        "outputs": {
