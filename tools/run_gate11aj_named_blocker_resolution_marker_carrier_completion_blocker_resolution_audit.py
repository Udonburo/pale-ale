#!/usr/bin/env python3
"""Run a Gate11AJ named blocker-resolution marker carrier-completion blocker-resolution audit on Gate11AI outputs."""
...
SCHEMA_VERSION = "gate11aj_named_blocker_resolution_marker_carrier_completion_blocker_resolution_audit_v1"
METHOD_ID = "gate11aj_named_blocker_resolution_marker_carrier_completion_blocker_resolution_audit_v1"
DEFAULT_REGISTRY = "named_blocker_resolution_marker_carrier_completion_blocker_resolution_registry.jsonl"
DEFAULT_POLICY_COMPARE = "named_blocker_resolution_marker_carrier_completion_blocker_resolution_policy_compare.csv"
DEFAULT_STATUS = "named_blocker_resolution_marker_carrier_completion_blocker_resolution_status.json"
DEFAULT_REPORT = "gate11aj_named_blocker_resolution_marker_carrier_completion_blocker_resolution_read.md"
DEFAULT_MANIFEST = "manifest.json"
DEFAULT_CHECKSUMS = "checksums.json"

REQUIRED_GATE11AI_STATUS_KEYS = gate11ai.STATUS_FIELDS
...
STATUS_FIELDS = (
    ...
    *gate11ah.CONFIRMED_KEYS,
    ...
    "named_blocker_resolution_marker_carrier_completion_blocker_resolution_status",
    "next_named_blocker",
)
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description=(
            "Build a Gate11AJ named blocker-resolution marker carrier-completion blocker-resolution audit "
            "from the frozen Gate11AI path run without converting path definition into resolution by prose."
        )
    )
    parser.add_argument("--gate11ai-dir", required=True)
...
def source_is_incomplete(source_manifest: Dict[str, Any], source_status: Dict[str, Any], report_text: str) -> bool:
    ...
    if not report_text.strip():
        return True
    return any(key not in source_status for key in REQUIRED_GATE11AI_STATUS_KEYS)
...
def extract_explicit_blocker_resolution_markers(report_text: str) -> List[str]:
    pattern = re.compile(
        r"(?im)^\s*(?:explicit_)?blocker_resolution_marker_status\s*[:=]\s*(present)\s*$"
    )
...
def same_source_blocker_resolution_completed(report_text: str) -> bool:
    ...
    same_source_status_completed = explicit_status_equals(report_text, "same_source_carrier_completion_status", "completed")
...
def build_registry(source_manifest: Dict[str, Any], status_payload: Dict[str, Any]) -> List[Dict[str, Any]]:
    row = {
        "source_gate11ai_run_id": str(source_manifest.get("run_id") or ""),
        "source_gate11ai_code_git_commit": str(source_manifest.get("code_git_commit") or ""),
    }
...
def build_policy_compare(registry_rows: Sequence[Dict[str, Any]]) -> List[Dict[str, Any]]:
    return [{"source_gate11ai_run_id": str(row["source_gate11ai_run_id"]), **{key: str(row[key]) for key in STATUS_FIELDS}} for row in registry_rows]
...
def build_status_payload(source_manifest: Dict[str, Any], source_status: Dict[str, Any], report_text: str) -> Dict[str, Any]:
    ...
    source_gate11ai_status = source_status_value(
        source_status,
        "blocker_resolution_marker_carrier_completion_blocker_resolution_path_status",
    )
    if source_gate11ai_status == "path_defined":
        gate11ai_path_defined_state_preservation_status = "preserved"
    elif source_gate11ai_status == "deferred":
        gate11ai_path_defined_state_preservation_status = "deferred"
    else:
        gate11ai_path_defined_state_preservation_status = "not_preserved"
    status_payload["gate11ai_path_defined_state_preservation_status"] = gate11ai_path_defined_state_preservation_status
    ...
    if incomplete:
        ...
        or gate11ai_path_defined_state_preservation_status == "not_preserved"
    ...
    status_payload["named_blocker_resolution_marker_carrier_completion_blocker_resolution_status"] = overall
    ...
    for key in SOURCE_PRESERVATION_KEYS:
        if status_payload[key] == "not_preserved":
            ...
            break
    if not next_named_blocker and gate11ai_path_defined_state_preservation_status == "not_preserved":
        next_named_blocker = "gate11ai_path_defined_state_not_preserved"
    if not next_named_blocker:
        ...
        elif gate11ai_path_defined_state_preservation_status == "deferred":
...
def build_report(
    ...
    lines = [
        "# Gate11AJ Named Blocker-Resolution Marker Carrier-Completion Blocker-Resolution Read",
        "",
        f"run_id: {run_id}",
        f"source_gate11ai_run_id: {source_manifest.get('run_id', '')}",
        f"source_gate11ai_code_git_commit: {source_manifest.get('code_git_commit', '')}",
        "",
        "## Discipline",
        "",
        "- Gate11AJ asks only whether the named blocker-resolution marker carrier-completion blocker now actually counts as resolved under the fixed Gate11AI path",
        "- Gate11AJ resolves only from bounded same-source evidence",
        "- Gate11AJ does not convert path definition into blocker resolution",
        "- Gate11AJ does not treat prose, hypotheticals, or worker-side synthesis as blocker-resolution evidence",
        "- Gate11AJ does not widen into blocker-resolution marker carrier completion, residual completion, later-source admission, or reopening",
        "",
        "## Source Summary",
        "",
        "| source_gate11ai_run_id | " + " | ".join(STATUS_FIELDS) + " |",
        "|---|" + "---|" * len(STATUS_FIELDS),
    ]
    for row in policy_compare_rows:
        lines.append("| " + " | ".join([str(row["source_gate11ai_run_id"]), *[str(row[key]) for key in STATUS_FIELDS]]) + " |")
    lines.extend(["", "## Status", ""])
    lines.extend([f"- {key}: `{status_payload[key]}`" for key in STATUS_FIELDS])
    lines.extend(["", "## Judgment", ""])

    outcome = status_payload["named_blocker_resolution_marker_carrier_completion_blocker_resolution_status"]
    if outcome == "resolved":
        lines.append("- the fixed Gate11AI path is now backed by bounded same-source blocker-resolution evidence, so the named blocker counts as resolved")
    elif outcome == "not_yet_resolved":
        lines.append("- the fixed Gate11AI path remains only a path definition; the named blocker is not yet resolved on bounded same-source evidence")
    elif outcome == "denied":
    ...
    lines.extend(
        [
            ...
            "## Memory Hook",
            "",
            "- Gate11AJ does not let Gate11AI path-definition prose earn resolved; only bounded same-source blocker-resolution evidence can do that.",
        ]
    )
    return "\n".join(lines) + "\n"
...
def main() -> None:
    args = parse_args()
    source_dir = Path(args.gate11ai_dir)
    out_dir = Path(args.out_dir)
    out_dir.mkdir(parents=True, exist_ok=True)
    run_id = out_dir.name

    source_manifest = gate9a.read_json(source_dir / gate11ai.DEFAULT_MANIFEST)
    source_status = gate9a.read_json(source_dir / gate11ai.DEFAULT_STATUS)
    source_report = (source_dir / gate11ai.DEFAULT_REPORT).read_text(encoding="utf-8")
    ...
    gate9a.write_csv(policy_compare_path, ("source_gate11ai_run_id", *STATUS_FIELDS), policy_compare_rows)
    ...
    gate9a.write_json(
        manifest_path,
        {
            ...
            "source_gate11ai_run_id": str(source_manifest.get("run_id") or ""),
            "source_gate11ai_code_git_commit": str(source_manifest.get("code_git_commit") or ""),
            "inputs": {"gate11ai_dir": str(source_dir)},
            "outputs": {
