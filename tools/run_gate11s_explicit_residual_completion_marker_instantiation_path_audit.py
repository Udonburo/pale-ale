#!/usr/bin/env python3
"""Run a Gate11S explicit residual completion-marker instantiation-path audit on Gate11R outputs."""
...
SCHEMA_VERSION = "gate11s_explicit_residual_completion_marker_instantiation_path_audit_v1"
METHOD_ID = "gate11s_explicit_residual_completion_marker_instantiation_path_audit_v1"

DEFAULT_REGISTRY = "explicit_residual_completion_marker_instantiation_path_registry.jsonl"
DEFAULT_POLICY_COMPARE = "explicit_residual_completion_marker_instantiation_path_policy_compare.csv"
DEFAULT_STATUS = "explicit_residual_completion_marker_instantiation_path_status.json"
DEFAULT_REPORT = "gate11s_explicit_residual_completion_marker_instantiation_path_read.md"
DEFAULT_MANIFEST = "manifest.json"
DEFAULT_CHECKSUMS = "checksums.json"

REQUIRED_GATE11R_STATUS_KEYS = (
...
PATH_DEFINABLE_MARKER_BLOCKERS = {
    "no_explicit_residual_completion_marker",
    "same_source_residual_completion_marker_binding_not_explicit",
    "bounded_read_prefix_not_attached",
}
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description=(
            "Build a Gate11S explicit residual completion-marker instantiation-path audit "
            "from the frozen Gate11R marker-existence run without instantiating a marker."
        )
    )
    parser.add_argument("--gate11r-dir", required=True)
...
def source_is_incomplete(
    source_gate11r_manifest: Dict[str, Any], source_gate11r_status: Dict[str, Any], report_text: str
) -> bool:
    if not str(source_gate11r_manifest.get("run_id") or ""):
        return True
    if not report_text.strip():
        return True
    return any(key not in source_gate11r_status for key in REQUIRED_GATE11R_STATUS_KEYS)
...
def blocker_sentence(blocker: str) -> str:
    mapping = {
        ...
        "minimum_same_source_marker_instantiation_rule_not_fixed": "the marker-not-yet-present line remains preserved, but the minimum same-source marker-instantiation additions are not yet fixed narrowly enough",
    }
    return mapping.get(blocker, "the minimum explicit residual completion-marker instantiation path is not yet fixed narrowly enough")


def build_registry(
    source_gate11r_manifest: Dict[str, Any],
    status_payload: Dict[str, Any],
) -> List[Dict[str, Any]]:
    return [
        {
            "source_gate11r_run_id": str(source_gate11r_manifest.get("run_id") or ""),
            "source_gate11r_code_git_commit": str(source_gate11r_manifest.get("code_git_commit") or ""),
            ...
            "missing_marker_instantiation_components_status": str(status_payload["missing_marker_instantiation_components_status"]),
            "minimum_same_source_marker_instantiation_rule_status": str(status_payload["minimum_same_source_marker_instantiation_rule_status"]),
            "bounded_read_prefix_instantiation_requirement_status": str(status_payload["bounded_read_prefix_instantiation_requirement_status"]),
            "residual_completion_marker_boundary_status": str(status_payload["residual_completion_marker_boundary_status"]),
            "explicit_residual_completion_marker_instantiation_path_status": str(status_payload["explicit_residual_completion_marker_instantiation_path_status"]),
...
def build_policy_compare(registry_rows: Sequence[Dict[str, Any]]) -> List[Dict[str, Any]]:
    return [
        {
            "source_gate11r_run_id": str(row["source_gate11r_run_id"]),
            ...
            "missing_marker_instantiation_components_status": str(row["missing_marker_instantiation_components_status"]),
            "minimum_same_source_marker_instantiation_rule_status": str(row["minimum_same_source_marker_instantiation_rule_status"]),
            "bounded_read_prefix_instantiation_requirement_status": str(row["bounded_read_prefix_instantiation_requirement_status"]),
            "residual_completion_marker_boundary_status": str(row["residual_completion_marker_boundary_status"]),
            "explicit_residual_completion_marker_instantiation_path_status": str(row["explicit_residual_completion_marker_instantiation_path_status"]),
...
def build_status_payload(
    source_gate11r_manifest: Dict[str, Any],
    source_gate11r_status: Dict[str, Any],
    report_text: str,
) -> Dict[str, Any]:
    incomplete = source_is_incomplete(source_gate11r_manifest, source_gate11r_status, report_text)

    gate10_closeout_preservation_status = "preserved" if source_status_value(source_gate11r_status, "gate10_closeout_preservation_status") == "preserved" else "not_preserved"
    gate11a_absence_result_preservation_status = "preserved" if source_status_value(source_gate11r_status, "gate11a_absence_result_preservation_status") == "preserved" else "not_preserved"
    gate11c_declaration_surface_preservation_status = "preserved" if source_status_value(source_gate11r_status, "gate11c_declaration_surface_preservation_status") == "preserved" else "not_preserved"
    gate11d_not_yet_declared_state_preservation_status = "preserved" if source_status_value(source_gate11r_status, "gate11d_not_yet_declared_state_preservation_status") == "preserved" else "not_preserved"
    gate11e_path_defined_state_preservation_status = "preserved" if source_status_value(source_gate11r_status, "gate11e_path_defined_state_preservation_status") == "preserved" else "not_preserved"
    gate11f_not_yet_admissible_state_preservation_status = "preserved" if source_status_value(source_gate11r_status, "gate11f_not_yet_admissible_state_preservation_status") == "preserved" else "not_preserved"
    gate11g_naming_surface_preservation_status = "preserved" if source_status_value(source_gate11r_status, "gate11g_naming_surface_preservation_status") == "preserved" else "not_preserved"
    gate11h_not_yet_named_state_preservation_status = "preserved" if source_status_value(source_gate11r_status, "gate11h_not_yet_named_state_preservation_status") == "preserved" else "not_preserved"
    gate11i_path_defined_state_preservation_status = "preserved" if source_status_value(source_gate11r_status, "gate11i_path_defined_state_preservation_status") == "preserved" else "not_preserved"
    gate11j_not_yet_admissible_state_preservation_status = "preserved" if source_status_value(source_gate11r_status, "gate11j_not_yet_admissible_state_preservation_status") == "preserved" else "not_preserved"
    gate11k_not_yet_present_state_preservation_status = "preserved" if source_status_value(source_gate11r_status, "gate11k_not_yet_present_state_preservation_status") == "preserved" else "not_preserved"
    gate11l_path_defined_state_preservation_status = "preserved" if source_status_value(source_gate11r_status, "gate11l_path_defined_state_preservation_status") == "preserved" else "not_preserved"
    gate11m_not_yet_present_state_preservation_status = "preserved" if source_status_value(source_gate11r_status, "gate11m_not_yet_present_state_preservation_status") == "preserved" else "not_preserved"
    gate11n_residual_named_state_preservation_status = "preserved" if source_status_value(source_gate11r_status, "gate11n_residual_named_state_preservation_status") == "preserved" else "not_preserved"
    gate11o_path_defined_state_preservation_status = "preserved" if source_status_value(source_gate11r_status, "gate11o_path_defined_state_preservation_status") == "preserved" else "not_preserved"
    gate11p_not_yet_completed_state_preservation_status = "preserved" if source_status_value(source_gate11r_status, "gate11p_not_yet_completed_state_preservation_status") == "preserved" else "not_preserved"
    gate11q_surface_defined_state_preservation_status = "preserved" if source_status_value(source_gate11r_status, "gate11q_surface_defined_state_preservation_status") == "preserved" else "not_preserved"
    source_gate11r_marker_status = source_status_value(source_gate11r_status, "one_explicit_residual_completion_marker_status")
    if source_gate11r_marker_status == "not_yet_present":
        gate11r_not_yet_present_state_preservation_status = "preserved"
    elif source_gate11r_marker_status == "deferred":
        gate11r_not_yet_present_state_preservation_status = "deferred"
    else:
        gate11r_not_yet_present_state_preservation_status = "not_preserved"
    broader_trusted_tree_settlement_still_unearned_status = "confirmed" if source_status_value(source_gate11r_status, "broader_trusted_tree_settlement_still_unearned_status") == "confirmed" else "not_confirmed"
    operator_admission_still_denied_status = "confirmed" if source_status_value(source_gate11r_status, "operator_admission_still_denied_status") == "confirmed" else "not_confirmed"
    retroactive_reinterpretation_forbidden_status = "confirmed" if source_status_value(source_gate11r_status, "retroactive_reinterpretation_forbidden_status") == "confirmed" else "not_confirmed"

    source_blocker = source_status_value(source_gate11r_status, "next_named_blocker")
    ...
    if incomplete:
        ...
        or source_status_value(source_gate11r_status, "residual_completion_marker_boundary_status") != "confirmed"
    ):
    ...
    if incomplete:
        missing_marker_instantiation_components_status = "deferred"
    elif source_gate11r_marker_status == "deferred":
        missing_marker_instantiation_components_status = "deferred"
    elif (
        source_status_value(source_gate11r_status, "explicit_residual_completion_marker_status") == "absent"
        and source_status_value(source_gate11r_status, "residual_completion_marker_singularity_status") == "none"
        and source_status_value(source_gate11r_status, "same_source_residual_completion_marker_binding_status") == "not_explicit"
        and source_status_value(source_gate11r_status, "bounded_read_prefix_attachment_status") == "not_attached"
        and source_blocker in PATH_DEFINABLE_MARKER_BLOCKERS
    ):
        missing_marker_instantiation_components_status = "named"
    elif source_blocker in PATH_DEFINABLE_MARKER_BLOCKERS:
        missing_marker_instantiation_components_status = "partially_named"
    else:
        missing_marker_instantiation_components_status = "not_named"

    if incomplete:
        minimum_same_source_marker_instantiation_rule_status = "deferred"
    elif source_gate11r_marker_status == "deferred":
        minimum_same_source_marker_instantiation_rule_status = "deferred"
    elif residual_completion_marker_boundary_status == "denied":
        minimum_same_source_marker_instantiation_rule_status = "not_defined"
    ...
    if incomplete:
        bounded_read_prefix_instantiation_requirement_status = "deferred"
    elif source_gate11r_marker_status == "deferred":
        bounded_read_prefix_instantiation_requirement_status = "deferred"
    elif source_blocker in PATH_DEFINABLE_MARKER_BLOCKERS:
        bounded_read_prefix_instantiation_requirement_status = "defined"
    else:
        bounded_read_prefix_instantiation_requirement_status = "not_defined"

    if incomplete:
        explicit_residual_completion_marker_instantiation_path_status = "deferred"
    elif (
        ...
        or gate11r_not_yet_present_state_preservation_status == "not_preserved"
        or residual_completion_marker_boundary_status == "denied"
        or broader_trusted_tree_settlement_still_unearned_status != "confirmed"
        or operator_admission_still_denied_status != "confirmed"
        or retroactive_reinterpretation_forbidden_status != "confirmed"
    ):
        explicit_residual_completion_marker_instantiation_path_status = "denied"
    elif (
        missing_marker_instantiation_components_status in {"named", "partially_named"}
        and minimum_same_source_marker_instantiation_rule_status == "defined"
        and bounded_read_prefix_instantiation_requirement_status == "defined"
        and residual_completion_marker_boundary_status == "confirmed"
        and gate11r_not_yet_present_state_preservation_status == "preserved"
    ):
        explicit_residual_completion_marker_instantiation_path_status = "path_defined"
    elif (
        missing_marker_instantiation_components_status == "deferred"
        or minimum_same_source_marker_instantiation_rule_status == "deferred"
        or bounded_read_prefix_instantiation_requirement_status == "deferred"
        or residual_completion_marker_boundary_status == "deferred"
        or gate11r_not_yet_present_state_preservation_status == "deferred"
    ):
        explicit_residual_completion_marker_instantiation_path_status = "deferred"
    else:
        explicit_residual_completion_marker_instantiation_path_status = "not_yet_defined"
    ...
    elif gate11r_not_yet_present_state_preservation_status == "not_preserved":
    ...
    elif gate11r_not_yet_present_state_preservation_status == "deferred":
        next_named_blocker = "multiple_future_markers"
    elif missing_marker_instantiation_components_status == "deferred":
        next_named_blocker = "multiple_future_markers"
    elif missing_marker_instantiation_components_status == "not_named":
        next_named_blocker = "missing_marker_instantiation_components_not_named"
    elif minimum_same_source_marker_instantiation_rule_status != "defined":
        next_named_blocker = "minimum_same_source_marker_instantiation_rule_not_fixed"
    elif bounded_read_prefix_instantiation_requirement_status != "defined":
    ...
    return {
        ...
        "missing_marker_instantiation_components_status": missing_marker_instantiation_components_status,
        "minimum_same_source_marker_instantiation_rule_status": minimum_same_source_marker_instantiation_rule_status,
        ...
        "explicit_residual_completion_marker_instantiation_path_status": explicit_residual_completion_marker_instantiation_path_status,
...
def build_report(
    ...
    lines = [
        "# Gate11S Explicit Residual Completion-Marker Instantiation Path Read",
        "",
        f"run_id: {run_id}",
        f"source_gate11r_run_id: {source_gate11r_manifest.get('run_id', '')}",
        f"source_gate11r_code_git_commit: {source_gate11r_manifest.get('code_git_commit', '')}",
        "",
        "## Discipline",
        "",
        "- Gate11S asks only what is the minimum honest path by which one explicit residual completion marker could later be instantiated under the fixed Gate11R line",
        "- Gate11S defines path only",
        "- Gate11S does not instantiate a marker",
        "- Gate11S does not complete the residual",
        "- Gate11S does not admit a later source",
        "- Gate11S does not decide one-admissible-later-source explicit-presence judgment",
        "- Gate11S does not declare a bounded-line insufficiency candidate",
        "- Gate11S does not declare that explicit declaration already exists",
        "- Gate11S does not decide reopening eligibility",
        "- Gate11S does not reopen operator admission",
    ...
    for row in policy_compare_rows:
        lines.append("| " + " | ".join([
            ...
            str(row["missing_marker_instantiation_components_status"]),
            str(row["minimum_same_source_marker_instantiation_rule_status"]),
            str(row["bounded_read_prefix_instantiation_requirement_status"]),
            str(row["residual_completion_marker_boundary_status"]),
            str(row["explicit_residual_completion_marker_instantiation_path_status"]),
    ...
    lines.extend([
        ...
        f"- missing_marker_instantiation_components_status: `{status_payload['missing_marker_instantiation_components_status']}`",
        f"- minimum_same_source_marker_instantiation_rule_status: `{status_payload['minimum_same_source_marker_instantiation_rule_status']}`",
        f"- bounded_read_prefix_instantiation_requirement_status: `{status_payload['bounded_read_prefix_instantiation_requirement_status']}`",
        f"- residual_completion_marker_boundary_status: `{status_payload['residual_completion_marker_boundary_status']}`",
        f"- explicit_residual_completion_marker_instantiation_path_status: `{status_payload['explicit_residual_completion_marker_instantiation_path_status']}`",
        f"- next_named_blocker: `{status_payload['next_named_blocker']}`",
        "",
        "## Marker-Instantiation Path Slice",
        "",
        "- Gate11S defines only the minimum same-source additions by which one explicit residual completion marker could later become honestly instantiated",
        "- path-definition prose, hypothetical examples, and generic read narrative are not a marker",
    ...
    if outcome == "path_defined":
        ...
        lines.append("- the attempted marker-instantiation path is denied because it would depend on shortcut, inflation, rewrite, graph-wide leap, or worker-side synthesis")
    else:
        lines.append("- the frozen source is incomplete or contradictory for a marker-instantiation-path judgment")
    ...
    lines.extend([
        ...
        "## Memory Hook",
        "",
        "- Gate11S does not say a marker exists; it asks for the minimum honest path by which one explicit residual completion marker could later be instantiated under the fixed Gate11R line.",
    ])
    return "\n".join(lines) + "\n"
...
def main() -> None:
    args = parse_args()
    source_gate11r_dir = Path(args.gate11r_dir)
    out_dir = Path(args.out_dir)
    out_dir.mkdir(parents=True, exist_ok=True)
    run_id = out_dir.name

    source_gate11r_manifest = gate9a.read_json(source_gate11r_dir / gate11r.DEFAULT_MANIFEST)
    source_gate11r_status = gate9a.read_json(source_gate11r_dir / gate11r.DEFAULT_STATUS)
    source_gate11r_report = (source_gate11r_dir / gate11r.DEFAULT_REPORT).read_text(encoding="utf-8")

    status_payload = build_status_payload(source_gate11r_manifest, source_gate11r_status, source_gate11r_report)
    registry_rows = build_registry(source_gate11r_manifest, status_payload)
    ...
    gate9a.write_text(report_path, build_report(run_id, source_gate11r_manifest, policy_compare_rows, status_payload))
    ...
    manifest = {
        ...
        "source_gate11r_run_id": str(source_gate11r_manifest.get("run_id") or ""),
        "source_gate11r_code_git_commit": str(source_gate11r_manifest.get("code_git_commit") or ""),
        "inputs": {"gate11r_dir": str(source_gate11r_dir)},
        "outputs": {
