#!/usr/bin/env python3
"""Regression tests for Gate11AO blocker-resolution marker carrier-completion blocker-resolution blocker-resolution blocker-resolution path helpers."""
...
def make_gate11an_manifest(run_id: str = "gate11an_run") -> dict:
    return {"run_id": run_id, "code_git_commit": "b5d4665e2c074f231a96acd43de7845328281158"}


def make_gate11an_status(
    ...
    named_blocker_resolution_marker_carrier_completion_blocker_resolution_blocker_resolution_condition_preservation_status: str = "preserved",
    ...
    blocker_resolution_marker_carrier_completion_blocker_resolution_blocker_resolution_blocker_status: str = "blocker_named",
    ...
    return {
        ...
        "named_blocker_resolution_marker_carrier_completion_blocker_resolution_blocker_resolution_condition_preservation_status": named_blocker_resolution_marker_carrier_completion_blocker_resolution_blocker_resolution_condition_preservation_status,
        ...
        "blocker_resolution_marker_carrier_completion_blocker_resolution_blocker_resolution_blocker_status": blocker_resolution_marker_carrier_completion_blocker_resolution_blocker_resolution_blocker_status,
        "next_named_blocker": next_named_blocker,
    }


class RunGate11AOBlockerResolutionMarkerCarrierCompletionBlockerResolutionBlockerResolutionBlockerResolutionPathAuditTest(unittest.TestCase):
    def test_path_defined_for_current_frozen_gate11an_source(self) -> None:
        status = gate11ao.build_status_payload(
            make_gate11an_manifest(),
            make_gate11an_status(),
            "Gate11AN preserves the blocker-named line under the bounded source.",
        )

        self.assertEqual(status["gate11an_blocker_named_state_preservation_status"], "preserved")
        ...
        self.assertEqual(status["blocker_resolution_marker_carrier_completion_blocker_resolution_blocker_resolution_blocker_resolution_path_status"], "path_defined")
        self.assertEqual(status["next_named_blocker"], "")

    def test_not_yet_defined_when_named_blocker_is_not_preserved(self) -> None:
        status = gate11ao.build_status_payload(
            make_gate11an_manifest(),
            make_gate11an_status(named_blocker_resolution_marker_carrier_completion_blocker_resolution_blocker_resolution_condition_preservation_status="not_preserved"),
            "Gate11AN no longer preserves the named blocker-resolution marker carrier-completion blocker-resolution blocker-resolution condition.",
        )

        self.assertEqual(status["named_blocker_preservation_status"], "not_preserved")
        self.assertEqual(status["blocker_resolution_marker_carrier_completion_blocker_resolution_blocker_resolution_blocker_resolution_path_status"], "not_yet_defined")
        self.assertEqual(status["next_named_blocker"], "named_blocker_not_preserved")

    def test_denied_when_gate11an_blocker_named_state_is_not_preserved(self) -> None:
        status = gate11ao.build_status_payload(
            make_gate11an_manifest(),
            make_gate11an_status(blocker_resolution_marker_carrier_completion_blocker_resolution_blocker_resolution_blocker_status="not_yet_named"),
            "Gate11AN no longer preserves the blocker-named state.",
        )

        self.assertEqual(status["gate11an_blocker_named_state_preservation_status"], "not_preserved")
        self.assertEqual(status["blocker_resolution_marker_carrier_completion_blocker_resolution_blocker_resolution_blocker_resolution_path_status"], "denied")
        self.assertEqual(status["next_named_blocker"], "gate11an_blocker_named_state_not_preserved")

    def test_denied_when_boundary_breaks(self) -> None:
        status = gate11ao.build_status_payload(
            make_gate11an_manifest(),
            make_gate11an_status(blocker_resolution_blocker_boundary_status="denied"),
            "Gate11AN breaks the blocker-resolution boundary.",
        )
    ...
    def test_deferred_when_source_is_incomplete(self) -> None:
        status = gate11ao.build_status_payload(make_gate11an_manifest(run_id=""), make_gate11an_status(), "")
        ...
        self.assertEqual(status["blocker_resolution_marker_carrier_completion_blocker_resolution_blocker_resolution_blocker_resolution_path_status"], "deferred")
    ...
    def test_not_yet_defined_when_source_does_not_narrow_path_enough(self) -> None:
        status = gate11ao.build_status_payload(
            make_gate11an_manifest(),
            make_gate11an_status(next_named_blocker="same_source_blocker_resolution_not_completed"),
            "Gate11AN preserves a blocker, but this slice does not fix a broader path than the frozen line allows.",
        )
    ...
    def test_policy_compare_echoes_single_source_row(self) -> None:
        ...
        registry = gate11ao.build_registry(make_gate11an_manifest(), status)
        compare = gate11ao.build_policy_compare(registry)

        self.assertEqual(len(compare), 1)
        self.assertEqual(compare[0]["source_gate11an_run_id"], "gate11an_run")
        self.assertEqual(compare[0]["blocker_resolution_marker_carrier_completion_blocker_resolution_blocker_resolution_blocker_resolution_path_status"], "path_defined")
