#!/usr/bin/env python3
"""Run a Gate11K one admissible later-source explicit-presence audit on Gate11J outputs."""
...
SCHEMA_VERSION = "gate11k_one_admissible_later_source_explicit_presence_audit_v1"
METHOD_ID = "gate11k_one_admissible_later_source_explicit_presence_audit_v1"

DEFAULT_REGISTRY = "one_admissible_later_source_explicit_presence_registry.jsonl"
DEFAULT_POLICY_COMPARE = "one_admissible_later_source_explicit_presence_policy_compare.csv"
DEFAULT_STATUS = "one_admissible_later_source_explicit_presence_status.json"
DEFAULT_REPORT = "gate11k_one_admissible_later_source_explicit_presence_read.md"
DEFAULT_MANIFEST = "manifest.json"
DEFAULT_CHECKSUMS = "checksums.json"

REQUIRED_GATE11J_STATUS_KEYS = (
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description=(
            "Build a Gate11K one admissible later-source explicit-presence audit from the "
            "frozen Gate11J admissibility run without redesigning surface, path, or admissibility rules."
        )
    )
    parser.add_argument("--gate11j-dir", required=True)
...
def source_is_incomplete(
    source_gate11j_manifest: Dict[str, Any], source_gate11j_status: Dict[str, Any], report_text: str
) -> bool:
    if not str(source_gate11j_manifest.get("run_id") or ""):
        return True
    if not report_text.strip():
        return True
    return any(not source_status_value(source_gate11j_status, key) for key in REQUIRED_GATE11J_STATUS_KEYS)
...
def build_registry(
    source_gate11j_manifest: Dict[str, Any],
    source_gate11j_status: Dict[str, Any],
    status_payload: Dict[str, Any],
) -> List[Dict[str, Any]]:
    return [
        {
            "source_gate11j_run_id": str(source_gate11j_manifest.get("run_id") or ""),
            "source_gate11j_code_git_commit": str(
                source_gate11j_manifest.get("code_git_commit") or ""
            ),
            ...
            "one_admissible_later_source_explicit_presence_status": str(
                status_payload["one_admissible_later_source_explicit_presence_status"]
            ),
        }
    ]
...
def build_policy_compare(registry_rows: Sequence[Dict[str, Any]]) -> List[Dict[str, Any]]:
    return [
        {
            "source_gate11j_run_id": str(row["source_gate11j_run_id"]),
            ...
            "one_admissible_later_source_explicit_presence_status": str(
                row["one_admissible_later_source_explicit_presence_status"]
            ),
        }
...
def build_status_payload(
    ...
    incomplete = source_is_incomplete(source_gate11j_manifest, source_gate11j_status, report_text)

    gate10_closeout_preservation_status = (
        "preserved"
        if source_status_value(source_gate11j_status, "gate10_closeout_preservation_status") == "preserved"
        else "not_preserved"
    )
    gate11a_absence_result_preservation_status = (
        "preserved"
        if source_status_value(source_gate11j_status, "gate11a_absence_result_preservation_status")
        == "preserved"
        else "not_preserved"
    )
    gate11c_declaration_surface_preservation_status = (
        ...
        if source_status_value(
            source_gate11j_status, "gate11c_declaration_surface_preservation_status"
        )
        == "preserved"
        else "not_preserved"
    )
    gate11d_not_yet_declared_state_preservation_status = (
        "preserved"
        if source_status_value(
            source_gate11j_status, "gate11d_not_yet_declared_state_preservation_status"
        )
        == "preserved"
        else "not_preserved"
    )
    gate11e_path_defined_state_preservation_status = (
        "preserved"
        if source_status_value(
            source_gate11j_status, "gate11e_path_defined_state_preservation_status"
        )
        == "preserved"
        else "not_preserved"
    )
    gate11f_not_yet_admissible_state_preservation_status = (
        "preserved"
        if source_status_value(
            source_gate11j_status, "gate11f_not_yet_admissible_state_preservation_status"
        )
        == "preserved"
        else "not_preserved"
    )
    gate11g_naming_surface_preservation_status = (
        "preserved"
        if source_status_value(source_gate11j_status, "gate11g_naming_surface_preservation_status")
        == "preserved"
        else "not_preserved"
    )
    gate11h_not_yet_named_state_preservation_status = (
        "preserved"
        if source_status_value(
            source_gate11j_status, "gate11h_not_yet_named_state_preservation_status"
        )
        == "preserved"
        else "not_preserved"
    )
    gate11i_path_defined_state_preservation_status = (
        "preserved"
        if source_status_value(
            source_gate11j_status, "gate11i_path_defined_state_preservation_status"
        )
        == "preserved"
        else "not_preserved"
    )
    gate11j_not_yet_admissible_state_preservation_status = (
        "preserved"
        if source_status_value(
            source_gate11j_status, "later_source_naming_instantiation_admissibility_status"
        )
        == "not_yet_admissible"
        else "not_preserved"
    )
    broader_trusted_tree_settlement_still_unearned_status = (
        "confirmed"
        if source_status_value(
            source_gate11j_status, "broader_trusted_tree_settlement_still_unearned_status"
        )
        == "confirmed"
        else "not_confirmed"
    )
    operator_admission_still_denied_status = (
        "confirmed"
        if source_status_value(source_gate11j_status, "operator_admission_still_denied_status")
        == "confirmed"
        else "not_confirmed"
    )
    retroactive_reinterpretation_forbidden_status = (
        "confirmed"
        if source_status_value(
            source_gate11j_status, "retroactive_reinterpretation_forbidden_status"
        )
        == "confirmed"
        else "not_confirmed"
    )
    ...
    if incomplete:
        ...
        or source_status_value(source_gate11j_status, "anti_shortcut_boundary_status") != "confirmed"
    ):
    ...
    if incomplete:
        one_admissible_later_source_explicit_presence_status = "deferred"
    elif (
        ...
        one_admissible_later_source_explicit_presence_status = "denied"
    elif (
        explicit_later_source_marker_status == "present"
        ...
        one_admissible_later_source_explicit_presence_status = "present"
    elif (
        explicit_later_source_marker_status == "deferred"
        ...
        one_admissible_later_source_explicit_presence_status = "not_yet_present"
...
def build_report(
    ...
    lines = [
        "# Gate11K One Admissible Later-Source Explicit-Presence Read",
        "",
        f"run_id: {run_id}",
        f"source_gate11j_run_id: {source_gate11j_manifest.get('run_id', '')}",
        f"source_gate11j_code_git_commit: {source_gate11j_manifest.get('code_git_commit', '')}",
        "",
        "## Discipline",
        "",
        "- Gate11K asks only whether one admissible later source is now explicitly present under the fixed Gate11J admissibility line",
        "- Gate11K does not redesign surface, path, or admissibility rules",
        "- Gate11K does not declare a candidate",
        "- Gate11K does not declare that explicit declaration already exists",
        "- Gate11K does not decide reopening eligibility",
        ...
        "- no retroactive rewrite of Gate9, Gate10, Gate11A, Gate11B, Gate11C, Gate11D, Gate11E, Gate11F, Gate11G, Gate11H, Gate11I, or Gate11J memory is permitted",
        "",
        "## Source Summary",
        "",
        "| source_gate11j_run_id | gate10_closeout_preservation_status | gate11a_absence_result_preservation_status | gate11c_declaration_surface_preservation_status | gate11d_not_yet_declared_state_preservation_status | gate11e_path_defined_state_preservation_status | gate11f_not_yet_admissible_state_preservation_status | gate11g_naming_surface_preservation_status | gate11h_not_yet_named_state_preservation_status | gate11i_path_defined_state_preservation_status | gate11j_not_yet_admissible_state_preservation_status | broader_trusted_tree_settlement_still_unearned_status | operator_admission_still_denied_status | retroactive_reinterpretation_forbidden_status | explicit_later_source_marker_status | later_source_singularity_status | same_source_path_attachment_status | admissibility_boundary_status | one_admissible_later_source_explicit_presence_status |",
        "|---|---|---|---|---|---|---|---|---|---|---|---|---|---|---|---|---|---|---|",
    ]
        ...
        lines.append(
            ...
            + " | ".join(
                [
                    ...
                    str(row["one_admissible_later_source_explicit_presence_status"]),
                ]
            )
            + " |"
        )
    ...
    lines.extend(
        [
            ...
            f"- one_admissible_later_source_explicit_presence_status: `{status_payload['one_admissible_later_source_explicit_presence_status']}`",
            f"- next_named_blocker: `{status_payload['next_named_blocker']}`",
            "",
            "## Explicit-Presence Slice",
            "",
            "- Gate11K audits only whether one admissible later source is now explicitly present under the fixed Gate11J admissibility line",
            "- the minimum audit remains exactly: explicit_later_source_marker_status, later_source_singularity_status, same_source_path_attachment_status, and admissibility_boundary_status",
            "",
            "## Judgment",
            "",
        ]
    ...
    if status_payload["one_admissible_later_source_explicit_presence_status"] == "present":
        lines.append(
            "- one admissible later source is now explicitly present under the fixed Gate11J admissibility line, while Gate11K still does not declare a candidate or reopen the line"
        )
    elif status_payload["one_admissible_later_source_explicit_presence_status"] == "not_yet_present":
        lines.append(
            "- the fixed Gate11J admissibility line remains preserved, but no one admissible later source is yet explicitly present there"
        )
    elif status_payload["one_admissible_later_source_explicit_presence_status"] == "denied":
        lines.append(
            "- the attempted explicit presence is denied because it would require shortcut, inflation, rewrite, leap, or synthesis pressure"
        )
    else:
        lines.append(
            "- the frozen source is incomplete for a one-admissible-later-source explicit-presence judgment"
        )
...
def main() -> None:
    ...
    source_gate11j_dir = Path(args.gate11j_dir)
    out_dir = Path(args.out_dir)
    out_dir.mkdir(parents=True, exist_ok=True)
    run_id = out_dir.name

    source_gate11j_manifest = gate9a.read_json(source_gate11j_dir / gate11j.DEFAULT_MANIFEST)
    source_gate11j_status = gate9a.read_json(source_gate11j_dir / gate11j.DEFAULT_STATUS)
    source_gate11j_report = (source_gate11j_dir / gate11j.DEFAULT_REPORT).read_text(encoding="utf-8")

    status_payload = build_status_payload(
        source_gate11j_manifest,
        source_gate11j_status,
        source_gate11j_report,
    )
    registry_rows = build_registry(source_gate11j_manifest, source_gate11j_status, status_payload)
    ...
    gate9a.write_text(
        ...
        build_report(
            run_id=run_id,
            source_gate11j_manifest=source_gate11j_manifest,
    ...
    manifest = {
        ...
        "inputs": {
            "gate11j_dir": str(source_gate11j_dir),
        },
        "outputs": {
