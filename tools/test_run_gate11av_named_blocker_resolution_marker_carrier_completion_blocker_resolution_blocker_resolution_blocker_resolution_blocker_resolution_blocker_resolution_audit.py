#!/usr/bin/env python3
"""Regression tests for Gate11AV named blocker-resolution marker carrier-completion blocker-resolution blocker-resolution blocker-resolution blocker-resolution blocker resolution helpers."""
...
def make_gate11au_manifest(run_id: str = "gate11au_run") -> dict:
    return {"run_id": run_id, "code_git_commit": "baa82e8d2cfd43bc6833e2928c20b3258471e112"}


def make_gate11au_status(**overrides: str) -> dict:
    status = {
...
class RunGate11AVNamedBlockerResolutionMarkerCarrierCompletionBlockerResolutionBlockerResolutionBlockerResolutionBlockerResolutionBlockerResolutionAuditTest(
    unittest.TestCase
):
    def test_not_yet_resolved_for_current_frozen_gate11au_source(self) -> None:
        status = gate11av.build_status_payload(
            make_gate11au_manifest(),
            make_gate11au_status(),
            "Gate11AU preserves the fixed blocker-resolution path but does not explicitly resolve the named blocker.",
        )

        self.assertEqual(status["gate11au_path_defined_state_preservation_status"], "preserved")
    ...
    later_source_id: runs/gate11av_future
    ...
    candidate_id: gate11av_candidate_001
    class: host_failure
...
"""
        status = gate11av.build_status_payload(make_gate11au_manifest(), make_gate11au_status(), report_text)
    ...
    def test_deferred_when_multiple_candidate_later_source_identifiers_compete(self) -> None:
        report_text = """
    blocker_resolution_marker_status: present
    later_source_id: runs/gate11av_future_a
    later_frozen_run_id: runs/gate11av_future_b
"""
        status = gate11av.build_status_payload(make_gate11au_manifest(), make_gate11au_status(), report_text)
    ...
    def test_denied_when_boundary_breaks(self) -> None:
        status = gate11av.build_status_payload(
            make_gate11au_manifest(),
            make_gate11au_status(blocker_resolution_boundary_status="not_confirmed"),
            "Gate11AU breaks the blocker-resolution boundary.",
        )
    ...
    def test_deferred_when_source_is_incomplete(self) -> None:
        status = gate11av.build_status_payload(make_gate11au_manifest(run_id=""), make_gate11au_status(), "")
    ...
    def test_policy_compare_echoes_single_source_row(self) -> None:
        ...
        registry = gate11av.build_registry(make_gate11au_manifest(), status)
        compare = gate11av.build_policy_compare(registry)

        self.assertEqual(len(compare), 1)
        self.assertEqual(compare[0]["source_gate11au_run_id"], "gate11au_run")
        self.assertEqual(
            compare[0][
                "named_blocker_resolution_marker_carrier_completion_blocker_resolution_blocker_resolution_blocker_resolution_blocker_resolution_blocker_resolution_status"
            ],
