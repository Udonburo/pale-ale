#!/usr/bin/env python3
"""Regression tests for Gate7 progression closure consumer v2."""
...
import run_gate7_progression_closure_consumer_v2 as consumer
...
class Gate7ProgressionClosureConsumerV2Tests(unittest.TestCase):
    def test_closure_is_zero_when_next_anchor_survives_two_step_closure(self) -> None:
    ...
    def test_closure_is_one_when_current_and_next_projectors_are_orthogonal(self) -> None:
        current_basis = np.zeros((4, 3), dtype=np.float64)
        current_basis[:, 0] = np.asarray([1.0, 0.0, 0.0, 0.0], dtype=np.float64)
        next_basis = np.zeros((4, 3), dtype=np.float64)
    ...
    def test_consumer_rerun_is_deterministic(self) -> None:
        with tempfile.TemporaryDirectory() as tmp_dir_str:
            ...
            out_a = tmp_dir / "out_a"
            out_b = tmp_dir / "out_b"
            script = REPO_ROOT / "tools" / "run_gate7_progression_closure_consumer_v2.py"
            ...
            self.assertEqual(
                (out_a / "gate7b_token_telemetry.csv").read_text(encoding="utf-8"),
                (out_b / "gate7b_token_telemetry.csv").read_text(encoding="utf-8"),
            )
            self.assertEqual(
                (out_a / "gate7b_sample_summary.csv").read_text(encoding="utf-8"),
                (out_b / "gate7b_sample_summary.csv").read_text(encoding="utf-8"),
            )
            self.assertEqual(
                (out_a / "gate7b_aggregate_summary.md").read_text(encoding="utf-8"),
                (out_b / "gate7b_aggregate_summary.md").read_text(encoding="utf-8"),
            )
            with open(out_a / "gate7b_sample_summary.csv", "r", encoding="utf-8", newline="") as handle:
