#!/usr/bin/env python3
"""Run a Gate11AS named blocker-resolution marker carrier-completion blocker-resolution blocker-resolution blocker-resolution blocker resolution audit on Gate11AR outputs."""
...
import run_gate11ar_blocker_resolution_marker_carrier_completion_blocker_resolution_blocker_resolution_blocker_resolution_blocker_resolution_path_audit as gate11ar
import run_gate9a_graph_gauge_consumer as gate9a


SCHEMA_VERSION = "gate11as_named_blocker_resolution_marker_carrier_completion_blocker_resolution_blocker_resolution_blocker_resolution_blocker_resolution_audit_v1"
METHOD_ID = "gate11as_named_blocker_resolution_marker_carrier_completion_blocker_resolution_blocker_resolution_blocker_resolution_blocker_resolution_audit_v1"
DEFAULT_REGISTRY = "named_blocker_resolution_marker_carrier_completion_blocker_resolution_blocker_resolution_blocker_resolution_blocker_resolution_registry.jsonl"
DEFAULT_POLICY_COMPARE = "named_blocker_resolution_marker_carrier_completion_blocker_resolution_blocker_resolution_blocker_resolution_blocker_resolution_policy_compare.csv"
DEFAULT_STATUS = "named_blocker_resolution_marker_carrier_completion_blocker_resolution_blocker_resolution_blocker_resolution_blocker_resolution_status.json"
DEFAULT_REPORT = "gate11as_named_blocker_resolution_marker_carrier_completion_blocker_resolution_blocker_resolution_blocker_resolution_blocker_resolution_read.md"
...
REQUIRED_GATE11AR_STATUS_KEYS = gate11ar.STATUS_FIELDS
STATUS_FIELDS = (
    *SOURCE_PRESERVATION_KEYS,
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description=(
            "Build a Gate11AS named blocker-resolution marker carrier-completion blocker-resolution blocker-resolution blocker-resolution blocker resolution audit "
            "from the frozen Gate11AR path run without converting path definition into resolution by prose."
        )
    )
    parser.add_argument("--gate11ar-dir", required=True)
...
def source_is_incomplete(source_manifest: Dict[str, Any], source_status: Dict[str, Any], report_text: str) -> bool:
    ...
    if not report_text.strip():
        return True
    return any(key not in source_status for key in REQUIRED_GATE11AR_STATUS_KEYS)
...
def build_registry(source_manifest: Dict[str, Any], status_payload: Dict[str, Any]) -> List[Dict[str, Any]]:
    row = {
        "source_gate11ar_run_id": str(source_manifest.get("run_id") or ""),
        "source_gate11ar_code_git_commit": str(source_manifest.get("code_git_commit") or ""),
    }
...
def build_policy_compare(registry_rows: Sequence[Dict[str, Any]]) -> List[Dict[str, Any]]:
    return [{"source_gate11ar_run_id": str(row["source_gate11ar_run_id"]), **{key: str(row[key]) for key in STATUS_FIELDS}} for row in registry_rows]
...
def build_status_payload(source_manifest: Dict[str, Any], source_status: Dict[str, Any], report_text: str) -> Dict[str, Any]:
    ...
    source_gate11ar_status = source_status_value(
        source_status,
    ...
    if source_gate11ar_status == "path_defined":
        gate11ar_path_defined_state_preservation_status = "preserved"
    elif source_gate11ar_status == "deferred":
        gate11ar_path_defined_state_preservation_status = "deferred"
    else:
        gate11ar_path_defined_state_preservation_status = "not_preserved"
    status_payload["gate11ar_path_defined_state_preservation_status"] = gate11ar_path_defined_state_preservation_status
    ...
    if incomplete:
        ...
        or gate11ar_path_defined_state_preservation_status == "not_preserved"
    ...
    for key in SOURCE_PRESERVATION_KEYS:
        if status_payload[key] == "not_preserved":
            ...
            break
    if not next_named_blocker and gate11ar_path_defined_state_preservation_status == "not_preserved":
        next_named_blocker = "gate11ar_path_defined_state_not_preserved"
    if not next_named_blocker:
        ...
        elif gate11ar_path_defined_state_preservation_status == "deferred":
...
def build_report(
    ...
    lines = [
        "# Gate11AS Named Blocker-Resolution Marker Carrier-Completion Blocker-Resolution Blocker-Resolution Blocker-Resolution Blocker Resolution Read",
        "",
        f"run_id: {run_id}",
        f"source_gate11ar_run_id: {source_manifest.get('run_id', '')}",
        f"source_gate11ar_code_git_commit: {source_manifest.get('code_git_commit', '')}",
        "",
        "## Discipline",
        "",
        "- Gate11AS asks only whether the named blocker-resolution marker carrier-completion blocker-resolution blocker-resolution blocker-resolution blocker now actually counts as resolved under the fixed Gate11AR path",
        "- Gate11AS resolves only from bounded same-source evidence",
        "- Gate11AS does not let Gate11AR path-definition prose, hypotheticals, or summaries count as blocker-resolution evidence",
        "- Gate11AS does not widen into blocker-resolution marker carrier completion, blocker resolution, residual completion, later-source admission, explicit-presence judgment, candidate declaration, reopening eligibility, or operator reopening",
        "",
        "## Source Summary",
        "",
        "| source_gate11ar_run_id | " + " | ".join(STATUS_FIELDS) + " |",
        "|---|" + "---|" * len(STATUS_FIELDS),
    ]
    for row in policy_compare_rows:
        lines.append("| " + " | ".join([str(row["source_gate11ar_run_id"]), *[str(row[key]) for key in STATUS_FIELDS]]) + " |")
    lines.extend(["", "## Status", ""])
    ...
    if outcome == "resolved":
        lines.append("- the fixed Gate11AR path is now backed by bounded same-source blocker-resolution evidence, so the named blocker counts as resolved")
    elif outcome == "not_yet_resolved":
        lines.append("- the fixed Gate11AR path remains only a path definition; the named blocker is not yet resolved on bounded same-source evidence")
    elif outcome == "denied":
    ...
    lines.extend(
        [
            ...
            "## Memory Hook",
            "",
            "- Gate11AS does not let Gate11AR path-definition prose earn resolved; only bounded same-source blocker-resolution evidence can do that.",
        ]
    )
    return "\n".join(lines) + "\n"
...
def main() -> None:
    args = parse_args()
    source_dir = Path(args.gate11ar_dir)
    out_dir = Path(args.out_dir)
    out_dir.mkdir(parents=True, exist_ok=True)
    run_id = out_dir.name

    source_manifest = gate9a.read_json(source_dir / gate11ar.DEFAULT_MANIFEST)
    source_status = gate9a.read_json(source_dir / gate11ar.DEFAULT_STATUS)
    source_report = (source_dir / gate11ar.DEFAULT_REPORT).read_text(encoding="utf-8")
    ...
    gate9a.write_csv(policy_compare_path, ("source_gate11ar_run_id", *STATUS_FIELDS), policy_compare_rows)
    ...
    gate9a.write_json(
        manifest_path,
        {
            ...
            "source_gate11ar_run_id": str(source_manifest.get("run_id") or ""),
            "source_gate11ar_code_git_commit": str(source_manifest.get("code_git_commit") or ""),
            "inputs": {"gate11ar_dir": str(source_dir)},
            "outputs": {
