#!/usr/bin/env python3
"""Regression tests for Gate11AZ blocker-resolution marker carrier-completion blocker-resolution blocker-resolution blocker-resolution blocker-resolution blocker-resolution blocker-resolution blocker helpers."""
...
def make_gate11ay_manifest(run_id: str = "gate11ay_run") -> dict:
    return {"run_id": run_id, "code_git_commit": "dbf3bee6ff65bb5e54ed84b474a8f4f11ec65547"}


def make_gate11ay_status(**overrides: str) -> dict:
    status = {
...
class RunGate11AZBlockerResolutionMarkerCarrierCompletionBlockerResolutionBlockerResolutionBlockerResolutionBlockerResolutionBlockerResolutionBlockerResolutionBlockerAuditTest(unittest.TestCase):
    def test_current_frozen_gate11ay_source_is_blocker_named(self) -> None:
        status = gate11az.build_status_payload(
            make_gate11ay_manifest(),
            make_gate11ay_status(),
            "Gate11AY preserves the fixed not-yet-resolved line with blocker no_explicit_blocker_resolution_marker.",
        )

        self.assertEqual(status["gate11ay_not_yet_resolved_state_preservation_status"], "preserved")
    ...
    def test_named_condition_not_preserved_is_denied(self) -> None:
        status = gate11az.build_status_payload(
            make_gate11ay_manifest(),
            make_gate11ay_status(
                named_blocker_resolution_marker_carrier_completion_blocker_resolution_blocker_resolution_blocker_resolution_blocker_resolution_blocker_resolution_blocker_condition_preservation_status="not_preserved"
            ),
            "Gate11AY no longer preserves the named blocker-resolution marker carrier-completion blocker-resolution blocker-resolution blocker-resolution blocker-resolution blocker-resolution blocker condition.",
        )
        ...
        self.assertEqual(
            ...
            "named_blocker_resolution_marker_carrier_completion_blocker_resolution_blocker_resolution_blocker_resolution_blocker_resolution_blocker_resolution_blocker_resolution_condition_not_preserved",
        )

    def test_boundary_break_causes_denied(self) -> None:
        status = gate11az.build_status_payload(
            make_gate11ay_manifest(),
            make_gate11ay_status(blocker_resolution_boundary_status="not_confirmed"),
            "Gate11AY breaks the blocker-resolution boundary.",
        )
    ...
    def test_incomplete_controlling_source_causes_deferred(self) -> None:
        status = gate11az.build_status_payload(make_gate11ay_manifest(run_id=""), make_gate11ay_status(), "")
    ...
    def test_source_that_no_longer_supports_narrow_blocker_naming_is_not_yet_named(self) -> None:
        status = gate11az.build_status_payload(
            ...
            "Gate11AY no longer supports the fixed no-explicit-marker blocker naming line.",
        )
        ...
        self.assertEqual(
            status["next_named_blocker"],
            "no_blocker_resolution_marker_carrier_completion_blocker_resolution_blocker_resolution_blocker_resolution_blocker_resolution_blocker_resolution_blocker_resolution_blocker_explicitly_named",
        )
    ...
    def test_policy_compare_echoes_single_source_row(self) -> None:
        ...
        registry = gate11az.build_registry(make_gate11ay_manifest(), status)
        compare = gate11az.build_policy_compare(registry)

        self.assertEqual(len(compare), 1)
        self.assertEqual(compare[0]["source_gate11ay_run_id"], "gate11ay_run")
        self.assertEqual(
            compare[0][
                "blocker_resolution_marker_carrier_completion_blocker_resolution_blocker_resolution_blocker_resolution_blocker_resolution_blocker_resolution_blocker_resolution_blocker_status"
            ],
