#!/usr/bin/env python3
"""Regression tests for Gate11T one explicit residual completion-marker path-instantiation helpers."""
...
def make_gate11s_manifest(run_id: str = "gate11s_run") -> dict:
    return {"run_id": run_id, "code_git_commit": "abc123"}


def make_gate11s_status(
    ...
    missing_marker_instantiation_components_status: str = "named",
    minimum_same_source_marker_instantiation_rule_status: str = "defined",
    bounded_read_prefix_instantiation_requirement_status: str = "defined",
    residual_completion_marker_boundary_status: str = "confirmed",
    explicit_residual_completion_marker_instantiation_path_status: str = "path_defined",
...
class RunGate11TOneExplicitResidualCompletionMarkerPathInstantiationAuditTest(unittest.TestCase):
    def test_default_frozen_source_is_not_yet_present(self) -> None:
        status = gate11t.build_status_payload(
            make_gate11s_manifest(),
            make_gate11s_status(),
            "Gate11S preserves the path-defined line without instantiating a marker.",
        )

        self.assertEqual(status["gate11s_path_defined_state_preservation_status"], "preserved")
        ...
        self.assertEqual(status["one_explicit_residual_completion_marker_path_instantiation_status"], "not_yet_present")
    ...
    def test_present_when_one_marker_instantiates_fixed_path(self) -> None:
        ...
        self.assertEqual(status["one_explicit_residual_completion_marker_path_instantiation_status"], "present")
    ...
    def test_path_definition_prose_does_not_count_as_instantiation(self) -> None:
        status = gate11t.build_status_payload(
            make_gate11s_manifest(),
            make_gate11s_status(),
            "A future path could mention a residual completion marker and path attachment in prose, but no bounded marker rows are instantiated.",
        )
    ...
    def test_deferred_when_multiple_candidate_markers_exist(self) -> None:
        ...
        status = gate11t.build_status_payload(make_gate11s_manifest(), make_gate11s_status(), report)
        ...
        self.assertEqual(status["one_explicit_residual_completion_marker_path_instantiation_status"], "deferred")
        self.assertEqual(status["next_named_blocker"], "multiple_candidate_markers")

    def test_denied_when_boundary_breaks(self) -> None:
        status = gate11t.build_status_payload(
            make_gate11s_manifest(),
            make_gate11s_status(residual_completion_marker_boundary_status="not_confirmed"),
            present_marker_report(),
        )

        self.assertEqual(status["residual_completion_marker_boundary_status"], "denied")
        self.assertEqual(status["one_explicit_residual_completion_marker_path_instantiation_status"], "denied")
    ...
    def test_deferred_when_source_is_incomplete(self) -> None:
        status = gate11t.build_status_payload(make_gate11s_manifest(run_id=""), make_gate11s_status(), "")
    ...
    def test_policy_compare_echoes_single_source_row(self) -> None:
        ...
        registry = gate11t.build_registry(make_gate11s_manifest(), status)
        compare = gate11t.build_policy_compare(registry)

        self.assertEqual(len(compare), 1)
        self.assertEqual(compare[0]["source_gate11s_run_id"], "gate11s_run")
        self.assertEqual(compare[0]["one_explicit_residual_completion_marker_path_instantiation_status"], "present")
