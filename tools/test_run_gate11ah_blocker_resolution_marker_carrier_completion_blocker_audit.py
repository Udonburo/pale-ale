#!/usr/bin/env python3
"""Regression tests for Gate11AH blocker-resolution marker carrier-completion blocker helpers."""
...
def make_gate11ag_manifest(run_id: str = "gate11ag_run") -> dict:
    return {"run_id": run_id, "code_git_commit": "abc123"}


def make_gate11ag_status(
    ...
    named_blocker_resolution_marker_carrier_completion_status: str = "not_yet_completed",
    ...
    return {
        ...
        "named_blocker_resolution_marker_carrier_completion_status": named_blocker_resolution_marker_carrier_completion_status,
        "next_named_blocker": next_named_blocker,
    }


class RunGate11AHBlockerResolutionMarkerCarrierCompletionBlockerAuditTest(unittest.TestCase):
    def test_current_frozen_gate11ag_source_is_blocker_named(self) -> None:
        status = gate11ah.build_status_payload(
            make_gate11ag_manifest(),
            make_gate11ag_status(),
            "Gate11AG preserves the fixed not-yet-completed line with blocker no_explicit_blocker_resolution_marker.",
        )

        self.assertEqual(status["gate11ag_not_yet_completed_state_preservation_status"], "preserved")
        ...
        self.assertEqual(status["blocker_resolution_marker_carrier_completion_blocker_status"], "blocker_named")
        self.assertEqual(status["next_named_blocker"], "no_explicit_blocker_resolution_marker")

    def test_named_condition_not_preserved_is_not_blocker_named(self) -> None:
        status = gate11ah.build_status_payload(
            make_gate11ag_manifest(),
            make_gate11ag_status(named_blocker_resolution_marker_carrier_condition_preservation_status="not_preserved"),
            "Gate11AG no longer preserves the named blocker-resolution marker carrier condition.",
        )

        self.assertEqual(status["blocker_resolution_marker_carrier_completion_blocker_status"], "denied")
        self.assertEqual(status["next_named_blocker"], "named_blocker_resolution_marker_carrier_condition_not_preserved")

    def test_boundary_break_causes_denied(self) -> None:
        status = gate11ah.build_status_payload(
            make_gate11ag_manifest(),
            make_gate11ag_status(carrier_completion_boundary_status="denied"),
            "Gate11AG breaks the carrier-completion boundary.",
        )
    ...
    def test_incomplete_controlling_source_causes_deferred(self) -> None:
        status = gate11ah.build_status_payload(make_gate11ag_manifest(run_id=""), make_gate11ag_status(), "")
        ...
        self.assertEqual(status["blocker_resolution_marker_carrier_completion_blocker_status"], "deferred")
    ...
    def test_source_that_no_longer_supports_narrow_blocker_naming_is_not_yet_named(self) -> None:
        status = gate11ah.build_status_payload(
            ...
            "Gate11AG no longer supports the fixed no-explicit-marker blocker naming line.",
        )

        self.assertEqual(status["explicit_blocker_resolution_marker_blocker_status"], "not_named")
        self.assertEqual(status["same_source_carrier_completion_blocker_status"], "named")
        self.assertEqual(status["blocker_resolution_marker_carrier_completion_blocker_status"], "not_yet_named")
        self.assertEqual(status["next_named_blocker"], "no_blocker_resolution_marker_carrier_completion_blocker_explicitly_named")
    ...
    def test_policy_compare_echoes_single_source_row(self) -> None:
        ...
        registry = gate11ah.build_registry(make_gate11ag_manifest(), status)
        compare = gate11ah.build_policy_compare(registry)

        self.assertEqual(len(compare), 1)
        self.assertEqual(compare[0]["source_gate11ag_run_id"], "gate11ag_run")
        self.assertEqual(compare[0]["blocker_resolution_marker_carrier_completion_blocker_status"], "blocker_named")
