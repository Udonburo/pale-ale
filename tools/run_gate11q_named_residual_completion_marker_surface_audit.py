#!/usr/bin/env python3
"""Run a Gate11Q named residual completion-marker surface audit on Gate11P outputs."""
...
SCHEMA_VERSION = "gate11q_named_residual_completion_marker_surface_audit_v1"
METHOD_ID = "gate11q_named_residual_completion_marker_surface_audit_v1"

DEFAULT_REGISTRY = "named_residual_completion_marker_surface_registry.jsonl"
DEFAULT_POLICY_COMPARE = "named_residual_completion_marker_surface_policy_compare.csv"
DEFAULT_STATUS = "named_residual_completion_marker_surface_status.json"
DEFAULT_REPORT = "gate11q_named_residual_completion_marker_surface_read.md"
DEFAULT_MANIFEST = "manifest.json"
DEFAULT_CHECKSUMS = "checksums.json"

REQUIRED_GATE11P_STATUS_KEYS = (
...
SURFACE_FIXABLE_BLOCKERS = {
    ...
    "same_source_residual_completion_not_instantiated",
}
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description=(
            "Build a Gate11Q named residual completion-marker surface audit "
            "from the frozen Gate11P completion run without deciding completion itself."
        )
    )
    parser.add_argument("--gate11p-dir", required=True)
...
def source_is_incomplete(
    source_gate11p_manifest: Dict[str, Any], source_gate11p_status: Dict[str, Any], report_text: str
) -> bool:
    if not str(source_gate11p_manifest.get("run_id") or ""):
        return True
    if not report_text.strip():
        return True
    return any(not source_status_value(source_gate11p_status, key) for key in REQUIRED_GATE11P_STATUS_KEYS)


def build_registry(
    source_gate11p_manifest: Dict[str, Any],
    status_payload: Dict[str, Any],
) -> List[Dict[str, Any]]:
    return [
        {
            "source_gate11p_run_id": str(source_gate11p_manifest.get("run_id") or ""),
            "source_gate11p_code_git_commit": str(source_gate11p_manifest.get("code_git_commit") or ""),
            ...
            "bounded_marker_surface_rows_status": str(status_payload["bounded_marker_surface_rows_status"]),
            ...
            "named_residual_completion_marker_surface_status": str(status_payload["named_residual_completion_marker_surface_status"]),
...
def build_policy_compare(registry_rows: Sequence[Dict[str, Any]]) -> List[Dict[str, Any]]:
    return [
        {
            "source_gate11p_run_id": str(row["source_gate11p_run_id"]),
            ...
            "bounded_marker_surface_rows_status": str(row["bounded_marker_surface_rows_status"]),
            ...
            "named_residual_completion_marker_surface_status": str(row["named_residual_completion_marker_surface_status"]),
...
def build_status_payload(
    source_gate11p_manifest: Dict[str, Any],
    source_gate11p_status: Dict[str, Any],
    report_text: str,
) -> Dict[str, Any]:
    incomplete = source_is_incomplete(source_gate11p_manifest, source_gate11p_status, report_text)

    gate10_closeout_preservation_status = "preserved" if source_status_value(source_gate11p_status, "gate10_closeout_preservation_status") == "preserved" else "not_preserved"
    gate11a_absence_result_preservation_status = "preserved" if source_status_value(source_gate11p_status, "gate11a_absence_result_preservation_status") == "preserved" else "not_preserved"
    gate11c_declaration_surface_preservation_status = "preserved" if source_status_value(source_gate11p_status, "gate11c_declaration_surface_preservation_status") == "preserved" else "not_preserved"
    gate11d_not_yet_declared_state_preservation_status = "preserved" if source_status_value(source_gate11p_status, "gate11d_not_yet_declared_state_preservation_status") == "preserved" else "not_preserved"
    gate11e_path_defined_state_preservation_status = "preserved" if source_status_value(source_gate11p_status, "gate11e_path_defined_state_preservation_status") == "preserved" else "not_preserved"
    gate11f_not_yet_admissible_state_preservation_status = "preserved" if source_status_value(source_gate11p_status, "gate11f_not_yet_admissible_state_preservation_status") == "preserved" else "not_preserved"
    gate11g_naming_surface_preservation_status = "preserved" if source_status_value(source_gate11p_status, "gate11g_naming_surface_preservation_status") == "preserved" else "not_preserved"
    gate11h_not_yet_named_state_preservation_status = "preserved" if source_status_value(source_gate11p_status, "gate11h_not_yet_named_state_preservation_status") == "preserved" else "not_preserved"
    gate11i_path_defined_state_preservation_status = "preserved" if source_status_value(source_gate11p_status, "gate11i_path_defined_state_preservation_status") == "preserved" else "not_preserved"
    gate11j_not_yet_admissible_state_preservation_status = "preserved" if source_status_value(source_gate11p_status, "gate11j_not_yet_admissible_state_preservation_status") == "preserved" else "not_preserved"
    gate11k_not_yet_present_state_preservation_status = "preserved" if source_status_value(source_gate11p_status, "gate11k_not_yet_present_state_preservation_status") == "preserved" else "not_preserved"
    gate11l_path_defined_state_preservation_status = "preserved" if source_status_value(source_gate11p_status, "gate11l_path_defined_state_preservation_status") == "preserved" else "not_preserved"
    gate11m_not_yet_present_state_preservation_status = "preserved" if source_status_value(source_gate11p_status, "gate11m_not_yet_present_state_preservation_status") == "preserved" else "not_preserved"
    gate11n_residual_named_state_preservation_status = "preserved" if source_status_value(source_gate11p_status, "gate11n_residual_named_state_preservation_status") == "preserved" else "not_preserved"
    gate11o_path_defined_state_preservation_status = "preserved" if source_status_value(source_gate11p_status, "gate11o_path_defined_state_preservation_status") == "preserved" else "not_preserved"
    source_gate11p_completion_status = source_status_value(source_gate11p_status, "named_residual_carrier_completion_status")
    if source_gate11p_completion_status == "not_yet_completed":
        gate11p_not_yet_completed_state_preservation_status = "preserved"
    elif source_gate11p_completion_status == "deferred":
        gate11p_not_yet_completed_state_preservation_status = "deferred"
    else:
        gate11p_not_yet_completed_state_preservation_status = "not_preserved"
    broader_trusted_tree_settlement_still_unearned_status = "confirmed" if source_status_value(source_gate11p_status, "broader_trusted_tree_settlement_still_unearned_status") == "confirmed" else "not_confirmed"
    operator_admission_still_denied_status = "confirmed" if source_status_value(source_gate11p_status, "operator_admission_still_denied_status") == "confirmed" else "not_confirmed"
    retroactive_reinterpretation_forbidden_status = "confirmed" if source_status_value(source_gate11p_status, "retroactive_reinterpretation_forbidden_status") == "confirmed" else "not_confirmed"

    source_blocker = source_status_value(source_gate11p_status, "next_named_blocker")
    ...
    if incomplete:
        ...
        or source_status_value(source_gate11p_status, "residual_completion_boundary_status") != "confirmed"
    ):
    ...
    if incomplete:
        bounded_marker_surface_rows_status = "deferred"
    elif source_status_value(source_gate11p_status, "explicit_residual_completion_marker_status") == "deferred":
        bounded_marker_surface_rows_status = "deferred"
    elif source_blocker in SURFACE_FIXABLE_BLOCKERS:
        bounded_marker_surface_rows_status = "defined"
    else:
        bounded_marker_surface_rows_status = "not_defined"

    if incomplete:
        same_source_binding_requirement_status = "deferred"
    elif source_status_value(source_gate11p_status, "same_source_residual_completion_status") == "deferred":
        same_source_binding_requirement_status = "deferred"
    elif source_blocker == "multiple_later_sources":
        same_source_binding_requirement_status = "deferred"
    elif source_blocker in SURFACE_FIXABLE_BLOCKERS:
    ...
    if incomplete:
        named_residual_completion_marker_surface_status = "deferred"
    elif (
        ...
        or operator_admission_still_denied_status != "confirmed"
        or retroactive_reinterpretation_forbidden_status != "confirmed"
        or gate11p_not_yet_completed_state_preservation_status == "not_preserved"
    ):
        named_residual_completion_marker_surface_status = "denied"
    elif (
        bounded_marker_surface_rows_status == "defined"
        ...
        or residual_completion_boundary_status == "deferred"
        or gate11p_not_yet_completed_state_preservation_status == "deferred"
    ):
        named_residual_completion_marker_surface_status = "deferred"
    else:
        named_residual_completion_marker_surface_status = "not_yet_defined"
    ...
    elif gate11p_not_yet_completed_state_preservation_status == "not_preserved":
    ...
    elif gate11p_not_yet_completed_state_preservation_status == "deferred":
        next_named_blocker = "multiple_candidate_marker_surfaces"
    elif bounded_marker_surface_rows_status == "deferred" or same_source_binding_requirement_status == "deferred" or bounded_read_prefix_requirement_status == "deferred":
        next_named_blocker = "multiple_candidate_marker_surfaces"
    elif bounded_marker_surface_rows_status != "defined":
        next_named_blocker = "bounded_marker_surface_rows_not_fixed"
...
def build_report(
    ...
    lines = [
        "# Gate11Q Named Residual Completion-Marker Surface Read",
        "",
        f"run_id: {run_id}",
        f"source_gate11p_run_id: {source_gate11p_manifest.get('run_id', '')}",
        f"source_gate11p_code_git_commit: {source_gate11p_manifest.get('code_git_commit', '')}",
        "",
        "## Discipline",
        "",
        "- Gate11Q asks only what would count as a valid explicit completion-marker surface for the named residual carrier condition under the fixed Gate11P line",
        "- Gate11Q defines marker surface only",
        "- Gate11Q does not complete the residual",
        "- Gate11Q does not admit a later source",
        "- Gate11Q does not decide one-admissible-later-source explicit-presence judgment",
        "- Gate11Q does not declare a candidate",
        "- Gate11Q does not declare that explicit declaration already exists",
        "- Gate11Q does not decide reopening eligibility",
        "- Gate11Q does not reopen operator admission",
        ...
        "- no retroactive rewrite of Gate9, Gate10, Gate11A, Gate11B, Gate11C, Gate11D, Gate11E, Gate11F, Gate11G, Gate11H, Gate11I, Gate11J, Gate11K, Gate11L, Gate11M, Gate11N, Gate11O, or Gate11P memory is permitted",
        "",
        "## Source Summary",
        "",
        "| source_gate11p_run_id | gate10_closeout_preservation_status | gate11a_absence_result_preservation_status | gate11c_declaration_surface_preservation_status | gate11d_not_yet_declared_state_preservation_status | gate11e_path_defined_state_preservation_status | gate11f_not_yet_admissible_state_preservation_status | gate11g_naming_surface_preservation_status | gate11h_not_yet_named_state_preservation_status | gate11i_path_defined_state_preservation_status | gate11j_not_yet_admissible_state_preservation_status | gate11k_not_yet_present_state_preservation_status | gate11l_path_defined_state_preservation_status | gate11m_not_yet_present_state_preservation_status | gate11n_residual_named_state_preservation_status | gate11o_path_defined_state_preservation_status | gate11p_not_yet_completed_state_preservation_status | broader_trusted_tree_settlement_still_unearned_status | operator_admission_still_denied_status | retroactive_reinterpretation_forbidden_status | bounded_marker_surface_rows_status | same_source_binding_requirement_status | bounded_read_prefix_requirement_status | residual_completion_boundary_status | named_residual_completion_marker_surface_status | next_named_blocker |",
        "|---|---|---|---|---|---|---|---|---|---|---|---|---|---|---|---|---|---|---|---|---|---|---|---|---|---|",
    ]
    for row in policy_compare_rows:
        lines.append("| " + " | ".join([
            str(row["source_gate11p_run_id"]),
            ...
            str(row["bounded_marker_surface_rows_status"]),
            ...
            str(row["named_residual_completion_marker_surface_status"]),
    ...
    lines.extend([
        ...
        f"- bounded_marker_surface_rows_status: `{status_payload['bounded_marker_surface_rows_status']}`",
        ...
        f"- named_residual_completion_marker_surface_status: `{status_payload['named_residual_completion_marker_surface_status']}`",
        f"- next_named_blocker: `{status_payload['next_named_blocker']}`",
        "",
        "## Marker-Surface Slice",
        "",
        "- Gate11Q defines only what would count as a valid explicit completion-marker surface for the named residual carrier condition under the fixed Gate11P line",
        "- the minimum surface remains exactly: bounded marker surface rows, same-source binding requirement, bounded read-prefix requirement, and residual completion boundary",
        "",
        "## Judgment",
        "",
    ])

    outcome = status_payload["named_residual_completion_marker_surface_status"]
    if outcome == "surface_defined":
        lines.append("- the explicit residual completion-marker surface is now fixed narrowly enough for a later audit, without completing the residual itself")
    elif outcome == "not_yet_defined":
        lines.append("- the fixed Gate11P line remains preserved, but the explicit residual completion-marker surface is not yet fixed narrowly enough")
    elif outcome == "denied":
        lines.append("- the attempted completion-marker surface definition is denied because it would depend on shortcut, inflation, rewrite, leap, or worker-side synthesis")
    else:
        lines.append("- the frozen source is incomplete for a residual completion-marker surface judgment")
...
def main() -> None:
    args = parse_args()
    source_gate11p_dir = Path(args.gate11p_dir)
    out_dir = Path(args.out_dir)
    out_dir.mkdir(parents=True, exist_ok=True)
    run_id = out_dir.name

    source_gate11p_manifest = gate9a.read_json(source_gate11p_dir / gate11p.DEFAULT_MANIFEST)
    source_gate11p_status = gate9a.read_json(source_gate11p_dir / gate11p.DEFAULT_STATUS)
    source_gate11p_report = (source_gate11p_dir / gate11p.DEFAULT_REPORT).read_text(encoding="utf-8")

    status_payload = build_status_payload(source_gate11p_manifest, source_gate11p_status, source_gate11p_report)
    registry_rows = build_registry(source_gate11p_manifest, status_payload)
    ...
    gate9a.write_text(report_path, build_report(run_id, source_gate11p_manifest, policy_compare_rows, status_payload))
    ...
    manifest = {
        ...
        "source_gate11p_run_id": str(source_gate11p_manifest.get("run_id") or ""),
        "source_gate11p_code_git_commit": str(source_gate11p_manifest.get("code_git_commit") or ""),
        "inputs": {"gate11p_dir": str(source_gate11p_dir)},
        "outputs": {
