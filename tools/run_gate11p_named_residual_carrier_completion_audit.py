#!/usr/bin/env python3
"""Run a Gate11P named residual carrier completion audit on Gate11O outputs."""
...
SCHEMA_VERSION = "gate11p_named_residual_carrier_completion_audit_v1"
METHOD_ID = "gate11p_named_residual_carrier_completion_audit_v1"

DEFAULT_REGISTRY = "named_residual_carrier_completion_registry.jsonl"
DEFAULT_POLICY_COMPARE = "named_residual_carrier_completion_policy_compare.csv"
DEFAULT_STATUS = "named_residual_carrier_completion_status.json"
DEFAULT_REPORT = "gate11p_named_residual_carrier_completion_read.md"
DEFAULT_MANIFEST = "manifest.json"
DEFAULT_CHECKSUMS = "checksums.json"

REQUIRED_GATE11O_STATUS_KEYS = (
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description=(
            "Build a Gate11P named residual carrier completion audit "
            "from the frozen Gate11O path-definition run without leaping into later-source admission."
        )
    )
    parser.add_argument("--gate11o-dir", required=True)
...
def source_is_incomplete(
    source_gate11o_manifest: Dict[str, Any], source_gate11o_status: Dict[str, Any], report_text: str
) -> bool:
    if not str(source_gate11o_manifest.get("run_id") or ""):
        return True
    if not report_text.strip():
        return True
    return any(not source_status_value(source_gate11o_status, key) for key in REQUIRED_GATE11O_STATUS_KEYS)
...
def same_source_residual_completion_instantiated(report_text: str) -> bool:
    required_phrases = (
...
def build_registry(
    source_gate11o_manifest: Dict[str, Any],
    status_payload: Dict[str, Any],
) -> List[Dict[str, Any]]:
    return [
        {
            "source_gate11o_run_id": str(source_gate11o_manifest.get("run_id") or ""),
            "source_gate11o_code_git_commit": str(source_gate11o_manifest.get("code_git_commit") or ""),
            ...
            "same_source_residual_completion_status": str(status_payload["same_source_residual_completion_status"]),
            "residual_completion_boundary_status": str(status_payload["residual_completion_boundary_status"]),
            "named_residual_carrier_completion_status": str(status_payload["named_residual_carrier_completion_status"]),
...
def build_policy_compare(registry_rows: Sequence[Dict[str, Any]]) -> List[Dict[str, Any]]:
    return [
        {
            "source_gate11o_run_id": str(row["source_gate11o_run_id"]),
            ...
            "same_source_residual_completion_status": str(row["same_source_residual_completion_status"]),
            "residual_completion_boundary_status": str(row["residual_completion_boundary_status"]),
            "named_residual_carrier_completion_status": str(row["named_residual_carrier_completion_status"]),
...
def build_status_payload(
    source_gate11o_manifest: Dict[str, Any],
    source_gate11o_status: Dict[str, Any],
    report_text: str,
) -> Dict[str, Any]:
    incomplete = source_is_incomplete(source_gate11o_manifest, source_gate11o_status, report_text)

    gate10_closeout_preservation_status = "preserved" if source_status_value(source_gate11o_status, "gate10_closeout_preservation_status") == "preserved" else "not_preserved"
    gate11a_absence_result_preservation_status = "preserved" if source_status_value(source_gate11o_status, "gate11a_absence_result_preservation_status") == "preserved" else "not_preserved"
    gate11c_declaration_surface_preservation_status = "preserved" if source_status_value(source_gate11o_status, "gate11c_declaration_surface_preservation_status") == "preserved" else "not_preserved"
    gate11d_not_yet_declared_state_preservation_status = "preserved" if source_status_value(source_gate11o_status, "gate11d_not_yet_declared_state_preservation_status") == "preserved" else "not_preserved"
    gate11e_path_defined_state_preservation_status = "preserved" if source_status_value(source_gate11o_status, "gate11e_path_defined_state_preservation_status") == "preserved" else "not_preserved"
    gate11f_not_yet_admissible_state_preservation_status = "preserved" if source_status_value(source_gate11o_status, "gate11f_not_yet_admissible_state_preservation_status") == "preserved" else "not_preserved"
    gate11g_naming_surface_preservation_status = "preserved" if source_status_value(source_gate11o_status, "gate11g_naming_surface_preservation_status") == "preserved" else "not_preserved"
    gate11h_not_yet_named_state_preservation_status = "preserved" if source_status_value(source_gate11o_status, "gate11h_not_yet_named_state_preservation_status") == "preserved" else "not_preserved"
    gate11i_path_defined_state_preservation_status = "preserved" if source_status_value(source_gate11o_status, "gate11i_path_defined_state_preservation_status") == "preserved" else "not_preserved"
    gate11j_not_yet_admissible_state_preservation_status = "preserved" if source_status_value(source_gate11o_status, "gate11j_not_yet_admissible_state_preservation_status") == "preserved" else "not_preserved"
    gate11k_not_yet_present_state_preservation_status = "preserved" if source_status_value(source_gate11o_status, "gate11k_not_yet_present_state_preservation_status") == "preserved" else "not_preserved"
    gate11l_path_defined_state_preservation_status = "preserved" if source_status_value(source_gate11o_status, "gate11l_path_defined_state_preservation_status") == "preserved" else "not_preserved"
    gate11m_not_yet_present_state_preservation_status = "preserved" if source_status_value(source_gate11o_status, "gate11m_not_yet_present_state_preservation_status") == "preserved" else "not_preserved"
    gate11n_residual_named_state_preservation_status = "preserved" if source_status_value(source_gate11o_status, "gate11n_residual_named_state_preservation_status") == "preserved" else "not_preserved"
    gate11o_path_defined_state_preservation_status = "preserved" if source_status_value(source_gate11o_status, "admissible_later_source_carrier_completion_instantiation_path_status") == "path_defined" else "not_preserved"
    broader_trusted_tree_settlement_still_unearned_status = "confirmed" if source_status_value(source_gate11o_status, "broader_trusted_tree_settlement_still_unearned_status") == "confirmed" else "not_confirmed"
    operator_admission_still_denied_status = "confirmed" if source_status_value(source_gate11o_status, "operator_admission_still_denied_status") == "confirmed" else "not_confirmed"
    retroactive_reinterpretation_forbidden_status = "confirmed" if source_status_value(source_gate11o_status, "retroactive_reinterpretation_forbidden_status") == "confirmed" else "not_confirmed"

    if incomplete:
        named_residual_carrier_condition_preservation_status = "deferred"
    elif source_status_value(source_gate11o_status, "named_residual_carrier_condition_preservation_status") == "preserved":
    ...
    if incomplete:
        ...
        or source_status_value(source_gate11o_status, "residual_completion_boundary_status") != "confirmed"
    ):
    ...
    elif explicit_residual_completion_marker_present(report_text):
    ...
    if incomplete:
        ...
        and same_source_residual_completion_instantiated(report_text)
    ):
        same_source_residual_completion_status = "completed"
    else:
    ...
    if incomplete:
        ...
        named_residual_carrier_completion_status = "denied"
    elif (
        named_residual_carrier_condition_preservation_status == "preserved"
        and explicit_residual_completion_marker_status == "present"
        and same_source_residual_completion_status == "completed"
        and residual_completion_boundary_status == "confirmed"
    ):
        named_residual_carrier_completion_status = "completed"
    elif (
        explicit_residual_completion_marker_status == "deferred"
        or same_source_residual_completion_status == "deferred"
    ...
    elif same_source_residual_completion_status == "deferred":
        next_named_blocker = "multiple_later_sources"
    elif same_source_residual_completion_status != "completed":
        next_named_blocker = "same_source_residual_completion_not_instantiated"
    else:
        next_named_blocker = ""
...
def build_report(
    ...
    lines = [
        "# Gate11P Named Residual Carrier Completion Read",
        "",
        f"run_id: {run_id}",
        f"source_gate11o_run_id: {source_gate11o_manifest.get('run_id', '')}",
        f"source_gate11o_code_git_commit: {source_gate11o_manifest.get('code_git_commit', '')}",
        "",
        "## Discipline",
        "",
        "- Gate11P asks only whether the named residual carrier condition now counts as completed under the fixed Gate11O path",
        "- Gate11P audits completion only",
        "- Gate11P does not leap into later-source admission",
        "- Gate11P does not leap into explicit-presence judgment",
        "- Gate11P does not declare a candidate",
        "- Gate11P does not declare that explicit declaration already exists",
        "- Gate11P does not decide reopening eligibility",
        "- Gate11P does not reopen operator admission",
        ...
        "- no retroactive rewrite of Gate9, Gate10, Gate11A, Gate11B, Gate11C, Gate11D, Gate11E, Gate11F, Gate11G, Gate11H, Gate11I, Gate11J, Gate11K, Gate11L, Gate11M, Gate11N, or Gate11O memory is permitted",
        "",
        "## Source Summary",
        "",
        "| source_gate11o_run_id | gate10_closeout_preservation_status | gate11a_absence_result_preservation_status | gate11c_declaration_surface_preservation_status | gate11d_not_yet_declared_state_preservation_status | gate11e_path_defined_state_preservation_status | gate11f_not_yet_admissible_state_preservation_status | gate11g_naming_surface_preservation_status | gate11h_not_yet_named_state_preservation_status | gate11i_path_defined_state_preservation_status | gate11j_not_yet_admissible_state_preservation_status | gate11k_not_yet_present_state_preservation_status | gate11l_path_defined_state_preservation_status | gate11m_not_yet_present_state_preservation_status | gate11n_residual_named_state_preservation_status | gate11o_path_defined_state_preservation_status | broader_trusted_tree_settlement_still_unearned_status | operator_admission_still_denied_status | retroactive_reinterpretation_forbidden_status | named_residual_carrier_condition_preservation_status | explicit_residual_completion_marker_status | same_source_residual_completion_status | residual_completion_boundary_status | named_residual_carrier_completion_status | next_named_blocker |",
        "|---|---|---|---|---|---|---|---|---|---|---|---|---|---|---|---|---|---|---|---|---|---|---|---|---|",
    ]
    for row in policy_compare_rows:
        lines.append("| " + " | ".join([
            str(row["source_gate11o_run_id"]),
            ...
            str(row["same_source_residual_completion_status"]),
            str(row["residual_completion_boundary_status"]),
            str(row["named_residual_carrier_completion_status"]),
    ...
    lines.extend([
        ...
        f"- same_source_residual_completion_status: `{status_payload['same_source_residual_completion_status']}`",
        f"- residual_completion_boundary_status: `{status_payload['residual_completion_boundary_status']}`",
        f"- named_residual_carrier_completion_status: `{status_payload['named_residual_carrier_completion_status']}`",
        f"- next_named_blocker: `{status_payload['next_named_blocker']}`",
        "",
        "## Completion Slice",
        "",
        "- Gate11P audits only whether the named residual carrier condition is now actually completed under the fixed Gate11O path",
        "- the minimum audit remains exactly: named residual carrier condition preservation, explicit residual completion marker status, same-source residual completion status, and residual completion boundary status",
        "",
        "## Judgment",
        "",
    ])

    outcome = status_payload["named_residual_carrier_completion_status"]
    if outcome == "completed":
        lines.append("- the named residual carrier condition is now explicitly completed under the fixed Gate11O path, without leaping into later-source admission or reopening judgment")
    elif outcome == "not_yet_completed":
        lines.append("- the fixed Gate11O path remains preserved, but the named residual carrier condition is not yet actually completed under that path")
    elif outcome == "denied":
        lines.append("- the attempted named residual completion is denied because it would depend on shortcut, inflation, rewrite, leap, or worker-side synthesis")
    else:
        lines.append("- the frozen source is incomplete for a named residual carrier completion judgment")
...
def main() -> None:
    args = parse_args()
    source_gate11o_dir = Path(args.gate11o_dir)
    out_dir = Path(args.out_dir)
    out_dir.mkdir(parents=True, exist_ok=True)
    run_id = out_dir.name

    source_gate11o_manifest = gate9a.read_json(source_gate11o_dir / gate11o.DEFAULT_MANIFEST)
    source_gate11o_status = gate9a.read_json(source_gate11o_dir / gate11o.DEFAULT_STATUS)
    source_gate11o_report = (source_gate11o_dir / gate11o.DEFAULT_REPORT).read_text(encoding="utf-8")

    status_payload = build_status_payload(source_gate11o_manifest, source_gate11o_status, source_gate11o_report)
    registry_rows = build_registry(source_gate11o_manifest, status_payload)
    ...
    gate9a.write_text(report_path, build_report(run_id, source_gate11o_manifest, policy_compare_rows, status_payload))
    ...
    manifest = {
        ...
        "source_gate11o_run_id": str(source_gate11o_manifest.get("run_id") or ""),
        "source_gate11o_code_git_commit": str(source_gate11o_manifest.get("code_git_commit") or ""),
        "inputs": {"gate11o_dir": str(source_gate11o_dir)},
        "outputs": {
