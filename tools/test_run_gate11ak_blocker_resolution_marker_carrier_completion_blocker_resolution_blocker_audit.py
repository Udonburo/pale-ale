#!/usr/bin/env python3
"""Regression tests for Gate11AK blocker-resolution marker carrier-completion blocker-resolution blocker helpers."""
...
def make_gate11aj_manifest(run_id: str = "gate11aj_run") -> dict:
    return {"run_id": run_id, "code_git_commit": "abc123"}


def make_gate11aj_status(
    ...
    named_blocker_resolution_marker_carrier_completion_blocker_resolution_status: str = "not_yet_resolved",
    ...
    return {
        ...
        "named_blocker_resolution_marker_carrier_completion_blocker_resolution_status": named_blocker_resolution_marker_carrier_completion_blocker_resolution_status,
        "next_named_blocker": next_named_blocker,
    }


class RunGate11AKBlockerResolutionMarkerCarrierCompletionBlockerResolutionBlockerAuditTest(unittest.TestCase):
    def test_current_frozen_gate11aj_source_is_blocker_named(self) -> None:
        status = gate11ak.build_status_payload(
            make_gate11aj_manifest(),
            make_gate11aj_status(),
            "Gate11AJ preserves the fixed not-yet-resolved line with blocker no_explicit_blocker_resolution_marker.",
        )

        self.assertEqual(status["gate11aj_not_yet_resolved_state_preservation_status"], "preserved")
        ...
        self.assertEqual(status["blocker_resolution_marker_carrier_completion_blocker_resolution_blocker_status"], "blocker_named")
    ...
    def test_named_condition_not_preserved_is_denied(self) -> None:
        status = gate11ak.build_status_payload(
            make_gate11aj_manifest(),
            make_gate11aj_status(named_blocker_preservation_status="not_preserved"),
            "Gate11AJ no longer preserves the named blocker-resolution marker carrier-completion blocker-resolution condition.",
        )

        self.assertEqual(status["blocker_resolution_marker_carrier_completion_blocker_resolution_blocker_status"], "denied")
        self.assertEqual(
            status["next_named_blocker"],
            "named_blocker_resolution_marker_carrier_completion_blocker_resolution_condition_not_preserved",
        )

    def test_boundary_break_causes_denied(self) -> None:
        status = gate11ak.build_status_payload(
            make_gate11aj_manifest(),
            make_gate11aj_status(blocker_resolution_boundary_status="not_confirmed"),
            "Gate11AJ breaks the blocker-resolution boundary.",
        )
    ...
    def test_incomplete_controlling_source_causes_deferred(self) -> None:
        status = gate11ak.build_status_payload(make_gate11aj_manifest(run_id=""), make_gate11aj_status(), "")
        ...
        self.assertEqual(status["blocker_resolution_marker_carrier_completion_blocker_resolution_blocker_status"], "deferred")
    ...
    def test_source_that_no_longer_supports_narrow_blocker_naming_is_not_yet_named(self) -> None:
        status = gate11ak.build_status_payload(
            ...
            "Gate11AJ no longer supports the fixed no-explicit-marker blocker naming line.",
        )

        self.assertEqual(status["explicit_blocker_resolution_marker_blocker_status"], "not_named")
        self.assertEqual(status["same_source_blocker_resolution_blocker_status"], "named")
        self.assertEqual(status["blocker_resolution_marker_carrier_completion_blocker_resolution_blocker_status"], "not_yet_named")
        self.assertEqual(status["next_named_blocker"], "no_blocker_resolution_marker_carrier_completion_blocker_resolution_blocker_explicitly_named")
    ...
    def test_policy_compare_echoes_single_source_row(self) -> None:
        ...
        registry = gate11ak.build_registry(make_gate11aj_manifest(), status)
        compare = gate11ak.build_policy_compare(registry)

        self.assertEqual(len(compare), 1)
        self.assertEqual(compare[0]["source_gate11aj_run_id"], "gate11aj_run")
        self.assertEqual(compare[0]["blocker_resolution_marker_carrier_completion_blocker_resolution_blocker_status"], "blocker_named")
