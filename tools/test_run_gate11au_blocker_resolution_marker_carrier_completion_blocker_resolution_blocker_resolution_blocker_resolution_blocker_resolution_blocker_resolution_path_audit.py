#!/usr/bin/env python3
"""Regression tests for Gate11AU blocker-resolution marker carrier-completion blocker-resolution blocker-resolution blocker-resolution blocker-resolution blocker resolution path helpers."""
...
def make_gate11at_manifest(run_id: str = "gate11at_run") -> dict:
    return {"run_id": run_id, "code_git_commit": "93ac493ee3df2629984b6170ed16e221433d0c75"}


def make_gate11at_status(**overrides: str) -> dict:
    status = {
        ...
        "named_blocker_resolution_marker_carrier_completion_blocker_resolution_blocker_resolution_blocker_resolution_blocker_resolution_condition_preservation_status": "preserved",
        ...
        "blocker_resolution_marker_carrier_completion_blocker_resolution_blocker_resolution_blocker_resolution_blocker_resolution_blocker_status": "blocker_named",
...
class RunGate11AUBlockerResolutionMarkerCarrierCompletionBlockerResolutionBlockerResolutionBlockerResolutionBlockerResolutionBlockerResolutionPathAuditTest(
    unittest.TestCase
):
    def test_path_defined_for_current_frozen_gate11at_source(self) -> None:
        status = gate11au.build_status_payload(
            make_gate11at_manifest(),
            make_gate11at_status(),
            "Gate11AT preserves the blocker-named line under the bounded source.",
        )

        self.assertEqual(status["gate11at_blocker_named_state_preservation_status"], "preserved")
    ...
    def test_not_yet_defined_when_named_blocker_is_not_preserved(self) -> None:
        status = gate11au.build_status_payload(
            make_gate11at_manifest(),
            make_gate11at_status(
                named_blocker_resolution_marker_carrier_completion_blocker_resolution_blocker_resolution_blocker_resolution_blocker_resolution_condition_preservation_status="not_preserved"
            ),
            "Gate11AT no longer preserves the named blocker-resolution marker carrier-completion blocker-resolution blocker-resolution blocker-resolution blocker-resolution condition.",
        )
    ...
    def test_denied_when_gate11at_blocker_named_state_is_not_preserved(self) -> None:
        status = gate11au.build_status_payload(
            make_gate11at_manifest(),
            make_gate11at_status(
                blocker_resolution_marker_carrier_completion_blocker_resolution_blocker_resolution_blocker_resolution_blocker_resolution_blocker_status="not_yet_named"
            ),
            "Gate11AT no longer preserves the blocker-named state.",
        )

        self.assertEqual(status["gate11at_blocker_named_state_preservation_status"], "not_preserved")
        self.assertEqual(
            status[
                "blocker_resolution_marker_carrier_completion_blocker_resolution_blocker_resolution_blocker_resolution_blocker_resolution_blocker_resolution_path_status"
            ],
        ...
        self.assertEqual(status["next_named_blocker"], "gate11at_blocker_named_state_not_preserved")

    def test_denied_when_boundary_breaks(self) -> None:
        status = gate11au.build_status_payload(
            make_gate11at_manifest(),
            make_gate11at_status(blocker_resolution_blocker_boundary_status="denied"),
            "Gate11AT breaks the blocker-resolution boundary.",
        )
    ...
    def test_deferred_when_source_is_incomplete(self) -> None:
        status = gate11au.build_status_payload(make_gate11at_manifest(run_id=""), make_gate11at_status(), "")
    ...
    def test_not_yet_defined_when_source_does_not_narrow_path_enough(self) -> None:
        status = gate11au.build_status_payload(
            make_gate11at_manifest(),
            make_gate11at_status(next_named_blocker="same_source_blocker_resolution_not_completed"),
            "Gate11AT preserves a blocker, but this slice does not fix a broader path than the frozen line allows.",
        )
    ...
    def test_policy_compare_echoes_single_source_row(self) -> None:
        ...
        registry = gate11au.build_registry(make_gate11at_manifest(), status)
        compare = gate11au.build_policy_compare(registry)

        self.assertEqual(len(compare), 1)
        self.assertEqual(compare[0]["source_gate11at_run_id"], "gate11at_run")
        self.assertEqual(
            compare[0][
                "blocker_resolution_marker_carrier_completion_blocker_resolution_blocker_resolution_blocker_resolution_blocker_resolution_blocker_resolution_path_status"
            ],
