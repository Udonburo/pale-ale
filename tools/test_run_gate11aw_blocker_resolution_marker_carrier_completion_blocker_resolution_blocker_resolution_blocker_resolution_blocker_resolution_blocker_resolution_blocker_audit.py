#!/usr/bin/env python3
"""Regression tests for Gate11AW blocker-resolution marker carrier-completion blocker-resolution blocker-resolution blocker-resolution blocker-resolution blocker-resolution blocker helpers."""
...
def make_gate11av_manifest(run_id: str = "gate11av_run") -> dict:
    return {"run_id": run_id, "code_git_commit": "44acdded1ea9b4c8d49d3f054c90a8301cd48b3e"}


def make_gate11av_status(**overrides: str) -> dict:
    status = {
...
class RunGate11AWBlockerResolutionMarkerCarrierCompletionBlockerResolutionBlockerResolutionBlockerResolutionBlockerResolutionBlockerResolutionBlockerAuditTest(unittest.TestCase):
    def test_current_frozen_gate11av_source_is_blocker_named(self) -> None:
        status = gate11aw.build_status_payload(
            make_gate11av_manifest(),
            make_gate11av_status(),
            "Gate11AV preserves the fixed not-yet-resolved line with blocker no_explicit_blocker_resolution_marker.",
        )

        self.assertEqual(status["gate11av_not_yet_resolved_state_preservation_status"], "preserved")
    ...
    def test_named_condition_not_preserved_is_denied(self) -> None:
        status = gate11aw.build_status_payload(
            make_gate11av_manifest(),
            make_gate11av_status(named_blocker_preservation_status="not_preserved"),
            "Gate11AV no longer preserves the named blocker-resolution marker carrier-completion blocker-resolution blocker-resolution blocker-resolution blocker-resolution blocker-resolution condition.",
        )
        ...
        self.assertEqual(
            ...
            "named_blocker_resolution_marker_carrier_completion_blocker_resolution_blocker_resolution_blocker_resolution_blocker_resolution_blocker_resolution_condition_not_preserved",
        )

    def test_boundary_break_causes_denied(self) -> None:
        status = gate11aw.build_status_payload(
            make_gate11av_manifest(),
            make_gate11av_status(blocker_resolution_boundary_status="not_confirmed"),
            "Gate11AV breaks the blocker-resolution boundary.",
        )
    ...
    def test_incomplete_controlling_source_causes_deferred(self) -> None:
        status = gate11aw.build_status_payload(make_gate11av_manifest(run_id=""), make_gate11av_status(), "")
    ...
    def test_source_that_no_longer_supports_narrow_blocker_naming_is_not_yet_named(self) -> None:
        status = gate11aw.build_status_payload(
            ...
            "Gate11AV no longer supports the fixed no-explicit-marker blocker naming line.",
        )
        ...
        self.assertEqual(
            status["next_named_blocker"],
            "no_blocker_resolution_marker_carrier_completion_blocker_resolution_blocker_resolution_blocker_resolution_blocker_resolution_blocker_resolution_blocker_explicitly_named",
        )
    ...
    def test_policy_compare_echoes_single_source_row(self) -> None:
        ...
        registry = gate11aw.build_registry(make_gate11av_manifest(), status)
        compare = gate11aw.build_policy_compare(registry)

        self.assertEqual(len(compare), 1)
        self.assertEqual(compare[0]["source_gate11av_run_id"], "gate11av_run")
        self.assertEqual(
            compare[0][
                "blocker_resolution_marker_carrier_completion_blocker_resolution_blocker_resolution_blocker_resolution_blocker_resolution_blocker_resolution_blocker_status"
            ],
