#!/usr/bin/env python3
"""Regression tests for Gate11AL blocker-resolution marker carrier-completion blocker-resolution blocker-resolution path helpers."""
...
def make_gate11ak_manifest(run_id: str = "gate11ak_run") -> dict:
    return {"run_id": run_id, "code_git_commit": "abc123"}


def make_gate11ak_status(
    ...
    named_blocker_resolution_marker_carrier_completion_blocker_resolution_condition_preservation_status: str = "preserved",
    ...
    blocker_resolution_marker_carrier_completion_blocker_resolution_blocker_status: str = "blocker_named",
    ...
    return {
        ...
        "named_blocker_resolution_marker_carrier_completion_blocker_resolution_condition_preservation_status": named_blocker_resolution_marker_carrier_completion_blocker_resolution_condition_preservation_status,
        ...
        "blocker_resolution_marker_carrier_completion_blocker_resolution_blocker_status": blocker_resolution_marker_carrier_completion_blocker_resolution_blocker_status,
        "next_named_blocker": next_named_blocker,
    }


class RunGate11ALBlockerResolutionMarkerCarrierCompletionBlockerResolutionBlockerResolutionPathAuditTest(unittest.TestCase):
    def test_path_defined_for_current_frozen_gate11ak_source(self) -> None:
        status = gate11al.build_status_payload(
            make_gate11ak_manifest(),
            make_gate11ak_status(),
            "Gate11AK preserves the blocker-named line under the bounded source.",
        )

        self.assertEqual(status["gate11ak_blocker_named_state_preservation_status"], "preserved")
        ...
        self.assertEqual(status["blocker_resolution_marker_carrier_completion_blocker_resolution_blocker_resolution_path_status"], "path_defined")
        self.assertEqual(status["next_named_blocker"], "")

    def test_not_yet_defined_when_named_blocker_is_not_preserved(self) -> None:
        status = gate11al.build_status_payload(
            make_gate11ak_manifest(),
            make_gate11ak_status(named_blocker_resolution_marker_carrier_completion_blocker_resolution_condition_preservation_status="not_preserved"),
            "Gate11AK no longer preserves the named blocker-resolution marker carrier-completion blocker-resolution condition.",
        )

        self.assertEqual(status["named_blocker_preservation_status"], "not_preserved")
        self.assertEqual(status["blocker_resolution_marker_carrier_completion_blocker_resolution_blocker_resolution_path_status"], "not_yet_defined")
        self.assertEqual(status["next_named_blocker"], "named_blocker_not_preserved")

    def test_denied_when_gate11ak_blocker_named_state_is_not_preserved(self) -> None:
        status = gate11al.build_status_payload(
            make_gate11ak_manifest(),
            make_gate11ak_status(blocker_resolution_marker_carrier_completion_blocker_resolution_blocker_status="not_yet_named"),
            "Gate11AK no longer preserves the blocker-named state.",
        )

        self.assertEqual(status["gate11ak_blocker_named_state_preservation_status"], "not_preserved")
        self.assertEqual(status["blocker_resolution_marker_carrier_completion_blocker_resolution_blocker_resolution_path_status"], "denied")
        self.assertEqual(status["next_named_blocker"], "gate11ak_blocker_named_state_not_preserved")

    def test_denied_when_boundary_breaks(self) -> None:
        status = gate11al.build_status_payload(
            make_gate11ak_manifest(),
            make_gate11ak_status(blocker_resolution_blocker_boundary_status="denied"),
            "Gate11AK breaks the blocker-resolution boundary.",
        )
    ...
    def test_deferred_when_source_is_incomplete(self) -> None:
        status = gate11al.build_status_payload(make_gate11ak_manifest(run_id=""), make_gate11ak_status(), "")
        ...
        self.assertEqual(status["blocker_resolution_marker_carrier_completion_blocker_resolution_blocker_resolution_path_status"], "deferred")
    ...
    def test_not_yet_defined_when_source_does_not_narrow_path_enough(self) -> None:
        status = gate11al.build_status_payload(
            make_gate11ak_manifest(),
            make_gate11ak_status(next_named_blocker="same_source_blocker_resolution_not_completed"),
            "Gate11AK preserves a blocker, but this slice does not fix a broader path than the frozen line allows.",
        )
    ...
    def test_policy_compare_echoes_single_source_row(self) -> None:
        ...
        registry = gate11al.build_registry(make_gate11ak_manifest(), status)
        compare = gate11al.build_policy_compare(registry)

        self.assertEqual(len(compare), 1)
        self.assertEqual(compare[0]["source_gate11ak_run_id"], "gate11ak_run")
        self.assertEqual(compare[0]["blocker_resolution_marker_carrier_completion_blocker_resolution_blocker_resolution_path_status"], "path_defined")
