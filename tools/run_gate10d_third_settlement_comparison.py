#!/usr/bin/env python3
"""Run a Gate10D third trusted-tree settlement comparison on Gate10C outputs."""
...
SCHEMA_VERSION = "gate10d_third_settlement_comparison_v1"
METHOD_ID = "gate10d_third_settlement_comparison_v1"

DEFAULT_REGISTRY = "trusted_tree_third_settlement_comparison_registry.jsonl"
DEFAULT_POLICY_COMPARE = "trusted_tree_third_settlement_comparison_policy_compare.csv"
DEFAULT_STATUS = "trusted_tree_third_settlement_comparison_status.json"
DEFAULT_REPORT = "gate10d_trusted_tree_third_settlement_comparison_read.md"
DEFAULT_MANIFEST = "manifest.json"
DEFAULT_CHECKSUMS = "checksums.json"

BASELINE_LANE = gate10b.BASELINE_LANE
SOURCE_CANDIDATE_LANE = gate10b.CANDIDATE_LANE
THIRD_CANDIDATE_LANE = "direct_contradiction"
THIRD_CANDIDATE_CELL_ID = "direct_contradiction"


def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description=(
            "Build a Gate10D third trusted-tree settlement comparison for the "
            "declaratively extracted direct_contradiction sublane from the frozen "
            "Gate10B source referenced by the Gate10C manifest."
        )
    )
...
def resolve_source_gate10b_dir(source_gate10b_dir: str) -> Path:
    candidate = Path(source_gate10b_dir)
...
def is_third_candidate_row(row: Dict[str, Any]) -> bool:
    return (
        str(row["broader_candidate_class"]) == SOURCE_CANDIDATE_LANE
        and str(row["cell_class"]) == "conflict"
        and str(row["cell_id"]) == THIRD_CANDIDATE_CELL_ID
    )
...
def build_comparison_registry(
    ...
    for row in source_registry_rows:
        if str(row["broader_candidate_class"]) == BASELINE_LANE:
            ...
            third_candidate_declaration = "not_applicable"
        elif is_third_candidate_row(row):
            comparison_lane = THIRD_CANDIDATE_LANE
            third_candidate_declaration = "declaratively_extracted"
        else:
            continue
        ...
        registry_rows.append(
            {
                ...
                "third_candidate_declaration": third_candidate_declaration,
...
def build_policy_compare(
    ...
    for lane_name in (BASELINE_LANE, THIRD_CANDIDATE_LANE):
...
def build_status_payload(
    ...
    source_gate10c_status: Dict[str, Any],
) -> Dict[str, Any]:
    baseline_rows = lane_rows(registry_rows, BASELINE_LANE)
    candidate_rows = lane_rows(registry_rows, THIRD_CANDIDATE_LANE)
    ...
    source_gate10c_preserved = (
        str(source_gate10c_status.get("forward_basis_baseline_preservation_status", ""))
        == "clear"
        and str(
            source_gate10c_status.get(
                "gate10b_slice_non_retroactive_preservation_status", ""
            )
        )
        == "clear"
        and str(source_gate10c_status.get("non_retroactive_memory_preservation_status", ""))
        == "clear"
        and str(source_gate10c_status.get("comparison_outcome_status", ""))
        == "settled"
        and str(source_gate10c_status.get("operator_admission_still_denied_status", ""))
        == "confirmed"
        and str(source_gate10c_status.get("broader_tree_settlement_non_promotion_status", ""))
        == "clear"
    )
    ...
    gate10b_slice_non_retroactive_preservation_status = (
        "clear" if source_gate10b_preserved else "denied"
    )

    gate10c_slice_non_retroactive_preservation_status = (
        "clear"
        if source_gate10c_preserved
        and all(
            str(row["comparison_lane"]) != THIRD_CANDIDATE_LANE
    ...
    third_candidate_declaration_status = (
        ...
        if candidate_rows
        and all(
            str(row["third_candidate_declaration"]) == "declaratively_extracted"
            and str(row["cell_id"]) == THIRD_CANDIDATE_CELL_ID
            and str(row["cell_class"]) == "conflict"
    ...
    operator_adjacent_rescue_pressure_status = (
        ...
        == "clear"
        and str(source_gate10c_status.get("operator_adjacent_rescue_pressure_status", ""))
        == "clear"
    ...
    decision_relevant_gain_beyond_baseline_status = (
        "present"
        if comparison_slice_complete
        and third_candidate_declaration_status == "clear"
    ...
    elif not source_gate10c_preserved:
        comparison_outcome_status = "deferred"
        next_named_blocker = str(source_gate10c_status.get("next_named_blocker", ""))
    ...
    elif gate10c_slice_non_retroactive_preservation_status != "clear":
        comparison_outcome_status = "denied"
        next_named_blocker = "gate10c_slice_reinterpretation_pressure"
    elif third_candidate_declaration_status != "clear":
        comparison_outcome_status = "denied"
        next_named_blocker = "third_candidate_declaration_integrity_fails"
    ...
    return {
        ...
        "gate10c_slice_non_retroactive_preservation_status": gate10c_slice_non_retroactive_preservation_status,
        "third_candidate_declaration_status": third_candidate_declaration_status,
...
def build_report(
    run_id: str,
    source_gate10c_manifest: Dict[str, Any],
    source_gate10b_manifest: Dict[str, Any],
    policy_compare_rows: Sequence[Dict[str, Any]],
    status_payload: Dict[str, Any],
) -> str:
    lines = [
        "# Gate10D Third Trusted-Tree Settlement Comparison Read",
        "",
        f"run_id: {run_id}",
        f"source_gate10c_run_id: {source_gate10c_manifest.get('run_id', '')}",
        f"source_gate10c_code_git_commit: {source_gate10c_manifest.get('code_git_commit', '')}",
        ...
        "## Discipline",
        "",
        "- one narrow third comparison only",
        "- Gate9Q forward-basis split remains the preserved baseline",
        "- third candidate is the declaratively extracted direct_contradiction sublane only",
        "- Gate10B and Gate10C settled slices remain preserved, operator admission remains denied, and no scalar masking is used",
        "",
        "## Comparison Summary",
        "",
    ...
    lines.extend(
        [
            ...
            f"- gate10c_slice_non_retroactive_preservation_status: `{status_payload['gate10c_slice_non_retroactive_preservation_status']}`",
            f"- third_candidate_declaration_status: `{status_payload['third_candidate_declaration_status']}`",
    ...
    if outcome == "settled":
        lines.extend(
            [
                ...
                "## Judgment",
                "",
                "- the declared direct_contradiction third candidate survives the narrow comparison against the preserved baseline",
                "- this remains a third slice-local settlement sentence only and does not declare broader trusted-tree settlement",
            ]
        )
        ...
        lines.extend(
            [
                ...
                "## Judgment",
                "",
                "- the third candidate remains informative relative to baseline but does not earn doctrine-safe settlement",
                f"- next_named_blocker: `{status_payload['next_named_blocker']}`",
            ]
        )
    elif outcome == "denied":
        lines.extend(
            [
                ...
                "## Judgment",
                "",
                "- the third candidate fails the declared third settlement comparison",
                f"- next_named_blocker: `{status_payload['next_named_blocker']}`",
            ]
        )
    else:
        lines.extend(
            [
                ...
                "## Judgment",
                "",
                "- the third comparison remains incomplete for settlement",
                f"- next_named_blocker: `{status_payload['next_named_blocker']}`",
            ]
        )
...
def main() -> int:
    ...
    source_gate10c_manifest = gate9a.read_json(
        source_gate10c_dir / gate10c.DEFAULT_MANIFEST
    )
    source_gate10c_status = gate9a.read_json(
        source_gate10c_dir / gate10c.DEFAULT_STATUS
    )

    source_gate10b_dir = resolve_source_gate10b_dir(
        str(source_gate10c_manifest.get("source_gate10b_dir") or "")
    )
    ...
    source_gate10b_registry_rows = gate9a.read_jsonl(
        source_gate10b_dir / gate10b.DEFAULT_REGISTRY
    )

    registry_rows = build_comparison_registry(source_gate10b_registry_rows)
    policy_compare_rows = build_policy_compare(registry_rows)
    status_payload = build_status_payload(
        registry_rows, source_gate10b_status, source_gate10c_status
    )
    ...
    gate9a.write_text(
        ...
        build_report(
            run_id=run_id,
            source_gate10c_manifest=source_gate10c_manifest,
    ...
    manifest = {
        ...
        "source_gate10c_run_id": str(source_gate10c_manifest.get("run_id") or ""),
        "source_gate10c_code_git_commit": str(
            source_gate10c_manifest.get("code_git_commit") or ""
        ),
