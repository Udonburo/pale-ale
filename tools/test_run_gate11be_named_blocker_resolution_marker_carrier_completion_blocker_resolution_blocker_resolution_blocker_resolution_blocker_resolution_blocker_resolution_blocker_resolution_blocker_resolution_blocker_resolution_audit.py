#!/usr/bin/env python3
"""Regression tests for Gate11BE named blocker-resolution marker carrier-completion blocker-resolution blocker-resolution blocker-resolution blocker-resolution blocker-resolution blocker-resolution blocker-resolution blocker-resolution helpers."""
...
def make_gate11bd_manifest(run_id: str = "gate11bd_run") -> dict:
    return {"run_id": run_id, "code_git_commit": "8a3565b3bcd070bf4bbee5e4ddcebbaa7a45524d"}


def make_gate11bd_status(**overrides: str) -> dict:
    status = {
...
class RunGate11BENamedBlockerResolutionMarkerCarrierCompletionBlockerResolutionBlockerResolutionBlockerResolutionBlockerResolutionBlockerResolutionBlockerResolutionBlockerResolutionBlockerResolutionAuditTest(
    unittest.TestCase
):
    def test_not_yet_resolved_for_current_frozen_gate11bd_source(self) -> None:
        status = gate11be.build_status_payload(
            make_gate11bd_manifest(),
            make_gate11bd_status(),
            "Gate11BD defines only the path and does not instantiate any explicit blocker-resolution evidence.",
        )

        self.assertEqual(status["gate11bd_path_defined_state_preservation_status"], "preserved")
        self.assertEqual(
            status[
    ...
    def test_resolved_when_bounded_same_source_resolution_is_explicit(self) -> None:
        report_text = """
    blocker_resolution_marker_status: present
    later_source_id: runs/gate11be_future
    ...
    candidate_id: gate11be_candidate_001
    class: host_failure
...
"""
        status = gate11be.build_status_payload(make_gate11bd_manifest(), make_gate11bd_status(), report_text)
    ...
    def test_path_prose_does_not_count_as_resolution(self) -> None:
        report_text = """
    ...
    def test_deferred_when_multiple_candidate_later_source_identifiers_compete(self) -> None:
        report_text = """
    blocker_resolution_marker_status: present
    later_source_id: runs/gate11be_future_a
    later_frozen_run_id: runs/gate11be_future_b
"""
        status = gate11be.build_status_payload(make_gate11bd_manifest(), make_gate11bd_status(), report_text)
    ...
    def test_denied_when_boundary_breaks(self) -> None:
        status = gate11be.build_status_payload(
            make_gate11bd_manifest(),
            make_gate11bd_status(blocker_resolution_boundary_status="not_confirmed"),
            "Gate11BD breaks the blocker-resolution boundary.",
        )
    ...
    def test_deferred_when_source_is_incomplete(self) -> None:
        status = gate11be.build_status_payload(make_gate11bd_manifest(run_id=""), make_gate11bd_status(), "")
    ...
    def test_policy_compare_echoes_single_source_row(self) -> None:
        status = gate11be.build_status_payload(
            make_gate11bd_manifest(),
            make_gate11bd_status(),
            "Gate11BD preserves the path but does not explicitly resolve the named blocker.",
        )
        registry = gate11be.build_registry(make_gate11bd_manifest(), status)
        compare = gate11be.build_policy_compare(registry)

        self.assertEqual(len(compare), 1)
        self.assertEqual(compare[0]["source_gate11bd_run_id"], "gate11bd_run")
        self.assertEqual(
            compare[0][
                "named_blocker_resolution_marker_carrier_completion_blocker_resolution_blocker_resolution_blocker_resolution_blocker_resolution_blocker_resolution_blocker_resolution_blocker_resolution_blocker_resolution_status"
            ],
