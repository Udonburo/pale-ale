#!/usr/bin/env python3
"""Regression tests for Gate11AM named blocker-resolution marker carrier-completion blocker-resolution helpers."""
...
def make_gate11al_manifest(run_id: str = "gate11al_run") -> dict:
    return {"run_id": run_id, "code_git_commit": "838661f9a0a523a36c1dd7a8aacfa0ed2d1ca82a"}


def make_gate11al_status(
    ...
    blocker_resolution_marker_carrier_completion_blocker_resolution_blocker_resolution_path_status: str = "path_defined",
    ...
    return {
        ...
        "blocker_resolution_marker_carrier_completion_blocker_resolution_blocker_resolution_path_status": blocker_resolution_marker_carrier_completion_blocker_resolution_blocker_resolution_path_status,
        "next_named_blocker": next_named_blocker,
    }


class RunGate11AMNamedBlockerResolutionMarkerCarrierCompletionBlockerResolutionBlockerResolutionAuditTest(unittest.TestCase):
    def test_not_yet_resolved_for_current_frozen_gate11al_source(self) -> None:
        status = gate11am.build_status_payload(
            make_gate11al_manifest(),
            make_gate11al_status(),
            "Gate11AL preserves the fixed blocker-resolution path but does not explicitly resolve the named blocker.",
        )

        self.assertEqual(status["gate11al_path_defined_state_preservation_status"], "preserved")
        ...
        self.assertEqual(status["named_blocker_resolution_marker_carrier_completion_blocker_resolution_blocker_resolution_status"], "not_yet_resolved")
    ...
    later_source_id: runs/gate11am_future
    ...
    candidate_id: gate11am_candidate_001
    class: host_failure
...
"""
        status = gate11am.build_status_payload(make_gate11al_manifest(), make_gate11al_status(), report_text)
        ...
        self.assertEqual(status["named_blocker_resolution_marker_carrier_completion_blocker_resolution_blocker_resolution_status"], "resolved")
    ...
    def test_not_yet_resolved_when_resolution_phrases_appear_only_in_path_prose(self) -> None:
        report_text = """
    The fixed Gate11AL path requires one explicit blocker-resolution marker, one explicit later-source identifier,
    ...
    def test_deferred_when_multiple_candidate_later_source_identifiers_compete(self) -> None:
        report_text = """
    blocker_resolution_marker_status: present
    later_source_id: runs/gate11am_future_a
    later_frozen_run_id: runs/gate11am_future_b
"""
        status = gate11am.build_status_payload(make_gate11al_manifest(), make_gate11al_status(), report_text)

        self.assertEqual(status["same_source_blocker_resolution_status"], "deferred")
        self.assertEqual(status["named_blocker_resolution_marker_carrier_completion_blocker_resolution_blocker_resolution_status"], "deferred")
        self.assertEqual(status["next_named_blocker"], "multiple_candidate_resolutions")

    def test_denied_when_boundary_breaks(self) -> None:
        status = gate11am.build_status_payload(
            make_gate11al_manifest(),
            make_gate11al_status(blocker_resolution_boundary_status="not_confirmed"),
            "Gate11AL breaks the blocker-resolution boundary.",
        )

        self.assertEqual(status["blocker_resolution_boundary_status"], "denied")
        self.assertEqual(status["named_blocker_resolution_marker_carrier_completion_blocker_resolution_blocker_resolution_status"], "denied")
    ...
    def test_deferred_when_source_is_incomplete(self) -> None:
        status = gate11am.build_status_payload(make_gate11al_manifest(run_id=""), make_gate11al_status(), "")
    ...
    def test_denied_when_gate11al_path_defined_state_is_not_preserved(self) -> None:
        status = gate11am.build_status_payload(
            make_gate11al_manifest(),
            make_gate11al_status(blocker_resolution_marker_carrier_completion_blocker_resolution_blocker_resolution_path_status="not_yet_defined"),
            "Gate11AL no longer preserves the path-defined state.",
        )

        self.assertEqual(status["gate11al_path_defined_state_preservation_status"], "not_preserved")
        self.assertEqual(status["named_blocker_resolution_marker_carrier_completion_blocker_resolution_blocker_resolution_status"], "denied")
        self.assertEqual(status["next_named_blocker"], "gate11al_path_defined_state_not_preserved")
    ...
    def test_policy_compare_echoes_single_source_row(self) -> None:
        ...
        registry = gate11am.build_registry(make_gate11al_manifest(), status)
        compare = gate11am.build_policy_compare(registry)

        self.assertEqual(len(compare), 1)
        self.assertEqual(compare[0]["source_gate11al_run_id"], "gate11al_run")
        self.assertEqual(compare[0]["named_blocker_resolution_marker_carrier_completion_blocker_resolution_blocker_resolution_status"], "not_yet_resolved")
