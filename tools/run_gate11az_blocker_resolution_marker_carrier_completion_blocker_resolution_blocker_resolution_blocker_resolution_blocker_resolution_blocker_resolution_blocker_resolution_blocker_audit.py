#!/usr/bin/env python3
"""Run a Gate11AZ blocker-resolution marker carrier-completion blocker-resolution blocker-resolution blocker-resolution blocker-resolution blocker-resolution blocker-resolution blocker audit on Gate11AY outputs."""
...
SCHEMA_VERSION = "gate11az_blocker_resolution_marker_carrier_completion_blocker_resolution_blocker_resolution_blocker_resolution_blocker_resolution_blocker_resolution_blocker_resolution_blocker_audit_v1"
METHOD_ID = "gate11az_blocker_resolution_marker_carrier_completion_blocker_resolution_blocker_resolution_blocker_resolution_blocker_resolution_blocker_resolution_blocker_resolution_blocker_audit_v1"
DEFAULT_REGISTRY = "blocker_resolution_marker_carrier_completion_blocker_resolution_blocker_resolution_blocker_resolution_blocker_resolution_blocker_resolution_blocker_resolution_blocker_registry.jsonl"
DEFAULT_POLICY_COMPARE = "blocker_resolution_marker_carrier_completion_blocker_resolution_blocker_resolution_blocker_resolution_blocker_resolution_blocker_resolution_blocker_resolution_blocker_policy_compare.csv"
DEFAULT_STATUS = "blocker_resolution_marker_carrier_completion_blocker_resolution_blocker_resolution_blocker_resolution_blocker_resolution_blocker_resolution_blocker_resolution_blocker_status.json"
DEFAULT_REPORT = "gate11az_blocker_resolution_marker_carrier_completion_blocker_resolution_blocker_resolution_blocker_resolution_blocker_resolution_blocker_resolution_blocker_resolution_blocker_read.md"
DEFAULT_MANIFEST = "manifest.json"
DEFAULT_CHECKSUMS = "checksums.json"

SOURCE_PRESERVATION_KEYS = (*gate11ay.SOURCE_PRESERVATION_KEYS, "gate11ax_path_defined_state_preservation_status")
CONFIRMED_KEYS = gate11ah.CONFIRMED_KEYS
REQUIRED_GATE11AY_STATUS_KEYS = gate11ay.STATUS_FIELDS
STATUS_FIELDS = (
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description=(
            "Build a Gate11AZ blocker-resolution marker carrier-completion blocker-resolution blocker-resolution blocker-resolution blocker-resolution blocker-resolution blocker-resolution blocker audit "
            "from the frozen Gate11AY resolution run without widening into resolution, admission, or reopening."
        )
    )
    parser.add_argument("--gate11ay-dir", required=True)
...
def source_is_incomplete(source_manifest: Dict[str, Any], source_status: Dict[str, Any], report_text: str) -> bool:
    ...
    if not report_text.strip():
        return True
    return any(key not in source_status for key in REQUIRED_GATE11AY_STATUS_KEYS)


def blocker_sentence(blocker: str) -> str:
    mapping = {
        "no_explicit_blocker_resolution_marker": (
            "the blocker-resolution marker carrier-completion blocker-resolution blocker-resolution blocker-resolution blocker-resolution blocker-resolution blocker-resolution blocker is named narrowly: no explicit blocker-resolution marker still blocks resolution under the fixed Gate11AY line"
        ),
    }
    return mapping.get(
        blocker,
        "the blocker-resolution marker carrier-completion blocker-resolution blocker-resolution blocker-resolution blocker-resolution blocker-resolution blocker-resolution blocker is not yet named narrowly enough",
    )


def build_registry(source_manifest: Dict[str, Any], status_payload: Dict[str, Any]) -> List[Dict[str, Any]]:
    row = {
        "source_gate11ay_run_id": str(source_manifest.get("run_id") or ""),
        "source_gate11ay_code_git_commit": str(source_manifest.get("code_git_commit") or ""),
    }
...
def build_policy_compare(registry_rows: Sequence[Dict[str, Any]]) -> List[Dict[str, Any]]:
    return [{"source_gate11ay_run_id": str(row["source_gate11ay_run_id"]), **{key: str(row[key]) for key in STATUS_FIELDS}} for row in registry_rows]
...
def build_status_payload(source_manifest: Dict[str, Any], source_status: Dict[str, Any], report_text: str) -> Dict[str, Any]:
    ...
    source_gate11ay_status = source_status_value(
        source_status,
    ...
    if source_gate11ay_status == "not_yet_resolved":
        gate11ay_not_yet_resolved_state_preservation_status = "preserved"
    elif source_gate11ay_status == "deferred":
        gate11ay_not_yet_resolved_state_preservation_status = "deferred"
    else:
        gate11ay_not_yet_resolved_state_preservation_status = "not_preserved"
    status_payload["gate11ay_not_yet_resolved_state_preservation_status"] = gate11ay_not_yet_resolved_state_preservation_status
    ...
    if incomplete:
        ...
        or gate11ay_not_yet_resolved_state_preservation_status == "not_preserved"
    ...
    for key in SOURCE_PRESERVATION_KEYS:
        if status_payload[key] == "not_preserved":
            ...
            break
    if not next_named_blocker and gate11ay_not_yet_resolved_state_preservation_status == "not_preserved":
        next_named_blocker = "gate11ay_not_yet_resolved_state_not_preserved"
    if not next_named_blocker:
        ...
        elif gate11ay_not_yet_resolved_state_preservation_status == "deferred":
            next_named_blocker = "upstream_resolution_judgment_deferred"
        elif overall == "not_yet_named":
            next_named_blocker = "no_blocker_resolution_marker_carrier_completion_blocker_resolution_blocker_resolution_blocker_resolution_blocker_resolution_blocker_resolution_blocker_resolution_blocker_explicitly_named"
        else:
...
def build_report(run_id: str, source_manifest: Dict[str, Any], status_payload: Dict[str, Any]) -> str:
    lines = [
        "# Gate11AZ Blocker-Resolution Marker Carrier-Completion Blocker-Resolution Blocker-Resolution Blocker-Resolution Blocker-Resolution Blocker-Resolution Blocker-Resolution Blocker Read",
        "",
        f"run_id: {run_id}",
        f"source_gate11ay_run_id: {source_manifest.get('run_id', '')}",
        f"source_gate11ay_code_git_commit: {source_manifest.get('code_git_commit', '')}",
        "",
        "## Discipline",
        "",
        "- Gate11AZ asks only which blocker-resolution marker carrier-completion blocker-resolution blocker-resolution blocker-resolution blocker-resolution blocker-resolution blocker-resolution blocker still blocks resolution under the fixed Gate11AY line",
        "- Gate11AZ names blockers only",
        "- Gate11AZ does not convert not-yet-resolved into resolution",
        "- Gate11AZ does not widen into blocker-resolution marker carrier-completion blocker-resolution path definition, blocker-resolution marker carrier-completion blocker-resolution judgment, blocker-resolution judgment, residual completion, later-source admission, reopening eligibility, or operator reopening",
        "- Gate11AZ does not inherit worker-side interpretations from path prose or generic narrative",
        "",
        "## Status",
        "",
    ]
    ...
    if outcome == "blocker_named":
        ...
        lines.append("- the fixed Gate11AY line still does not name the blocker-resolution marker carrier-completion blocker-resolution blocker-resolution blocker-resolution blocker-resolution blocker-resolution blocker-resolution blocker narrowly enough")
    elif outcome == "denied":
        ...
        lines.append("- the frozen source is incomplete or contradictory for a blocker-resolution marker carrier-completion blocker-resolution blocker-resolution blocker-resolution blocker-resolution blocker-resolution blocker-resolution blocker judgment")
    ...
    lines.extend(
        [
            ...
            "## Memory Hook",
            "",
            "- Gate11AZ does not say the named blocker is resolved; it asks which blocker-resolution marker carrier-completion blocker-resolution blocker-resolution blocker-resolution blocker-resolution blocker-resolution blocker-resolution blocker still blocks resolution under the fixed Gate11AY line.",
        ]
    )
    return "\n".join(lines) + "\n"
...
def main() -> None:
    args = parse_args()
    source_dir = Path(args.gate11ay_dir)
    out_dir = Path(args.out_dir)
    out_dir.mkdir(parents=True, exist_ok=True)
    run_id = out_dir.name

    source_manifest = gate9a.read_json(source_dir / gate11ay.DEFAULT_MANIFEST)
    source_status = gate9a.read_json(source_dir / gate11ay.DEFAULT_STATUS)
    source_report = (source_dir / gate11ay.DEFAULT_REPORT).read_text(encoding="utf-8")
    ...
    gate9a.write_csv(policy_compare_path, ("source_gate11ay_run_id", *STATUS_FIELDS), policy_compare_rows)
    ...
    gate9a.write_json(
        manifest_path,
        {
            ...
            "source_gate11ay_run_id": str(source_manifest.get("run_id") or ""),
            "source_gate11ay_code_git_commit": str(source_manifest.get("code_git_commit") or ""),
            "inputs": {"gate11ay_dir": str(source_dir)},
            "outputs": {
