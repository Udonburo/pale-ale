#!/usr/bin/env python3
"""Run a Gate11AA named blocker-resolution marker surface audit on Gate11Z outputs."""
...
SCHEMA_VERSION = "gate11aa_named_blocker_resolution_marker_surface_audit_v1"
METHOD_ID = "gate11aa_named_blocker_resolution_marker_surface_audit_v1"

DEFAULT_REGISTRY = "named_blocker_resolution_marker_surface_registry.jsonl"
DEFAULT_POLICY_COMPARE = "named_blocker_resolution_marker_surface_policy_compare.csv"
DEFAULT_STATUS = "named_blocker_resolution_marker_surface_status.json"
DEFAULT_REPORT = "gate11aa_named_blocker_resolution_marker_surface_read.md"
DEFAULT_MANIFEST = "manifest.json"
DEFAULT_CHECKSUMS = "checksums.json"

REQUIRED_GATE11Z_STATUS_KEYS = (
...
SURFACE_FIXABLE_BLOCKERS = {
    "",
    "no_explicit_blocker_resolution_marker",
    "same_source_blocker_resolution_not_completed",
}
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description=(
            "Build a Gate11AA named blocker-resolution marker surface audit "
            "from the frozen Gate11Z resolution run without deciding blocker resolution itself."
        )
    )
    parser.add_argument("--gate11z-dir", required=True)
...
def source_is_incomplete(
    source_gate11z_manifest: Dict[str, Any], source_gate11z_status: Dict[str, Any], report_text: str
) -> bool:
    if not str(source_gate11z_manifest.get("run_id") or ""):
        return True
    if not report_text.strip():
        return True
    return any(key not in source_gate11z_status for key in REQUIRED_GATE11Z_STATUS_KEYS)


def build_registry(
    source_gate11z_manifest: Dict[str, Any],
    status_payload: Dict[str, Any],
) -> List[Dict[str, Any]]:
    return [
        {
            "source_gate11z_run_id": str(source_gate11z_manifest.get("run_id") or ""),
            "source_gate11z_code_git_commit": str(source_gate11z_manifest.get("code_git_commit") or ""),
            ...
            "bounded_blocker_resolution_marker_rows_status": str(status_payload["bounded_blocker_resolution_marker_rows_status"]),
            "same_source_binding_requirement_status": str(status_payload["same_source_binding_requirement_status"]),
            "bounded_read_prefix_requirement_status": str(status_payload["bounded_read_prefix_requirement_status"]),
            "blocker_resolution_boundary_status": str(status_payload["blocker_resolution_boundary_status"]),
            "named_blocker_resolution_marker_surface_status": str(status_payload["named_blocker_resolution_marker_surface_status"]),
...
def build_policy_compare(registry_rows: Sequence[Dict[str, Any]]) -> List[Dict[str, Any]]:
    return [
        {
            "source_gate11z_run_id": str(row["source_gate11z_run_id"]),
            ...
            "bounded_blocker_resolution_marker_rows_status": str(row["bounded_blocker_resolution_marker_rows_status"]),
            "same_source_binding_requirement_status": str(row["same_source_binding_requirement_status"]),
            "bounded_read_prefix_requirement_status": str(row["bounded_read_prefix_requirement_status"]),
            "blocker_resolution_boundary_status": str(row["blocker_resolution_boundary_status"]),
            "named_blocker_resolution_marker_surface_status": str(row["named_blocker_resolution_marker_surface_status"]),
...
def build_status_payload(
    source_gate11z_manifest: Dict[str, Any],
    source_gate11z_status: Dict[str, Any],
    report_text: str,
) -> Dict[str, Any]:
    incomplete = source_is_incomplete(source_gate11z_manifest, source_gate11z_status, report_text)

    gate10_closeout_preservation_status = "preserved" if source_status_value(source_gate11z_status, "gate10_closeout_preservation_status") == "preserved" else "not_preserved"
    gate11a_absence_result_preservation_status = "preserved" if source_status_value(source_gate11z_status, "gate11a_absence_result_preservation_status") == "preserved" else "not_preserved"
    gate11c_declaration_surface_preservation_status = "preserved" if source_status_value(source_gate11z_status, "gate11c_declaration_surface_preservation_status") == "preserved" else "not_preserved"
    gate11d_not_yet_declared_state_preservation_status = "preserved" if source_status_value(source_gate11z_status, "gate11d_not_yet_declared_state_preservation_status") == "preserved" else "not_preserved"
    gate11e_path_defined_state_preservation_status = "preserved" if source_status_value(source_gate11z_status, "gate11e_path_defined_state_preservation_status") == "preserved" else "not_preserved"
    gate11f_not_yet_admissible_state_preservation_status = "preserved" if source_status_value(source_gate11z_status, "gate11f_not_yet_admissible_state_preservation_status") == "preserved" else "not_preserved"
    gate11g_naming_surface_preservation_status = "preserved" if source_status_value(source_gate11z_status, "gate11g_naming_surface_preservation_status") == "preserved" else "not_preserved"
    gate11h_not_yet_named_state_preservation_status = "preserved" if source_status_value(source_gate11z_status, "gate11h_not_yet_named_state_preservation_status") == "preserved" else "not_preserved"
    gate11i_path_defined_state_preservation_status = "preserved" if source_status_value(source_gate11z_status, "gate11i_path_defined_state_preservation_status") == "preserved" else "not_preserved"
    gate11j_not_yet_admissible_state_preservation_status = "preserved" if source_status_value(source_gate11z_status, "gate11j_not_yet_admissible_state_preservation_status") == "preserved" else "not_preserved"
    gate11k_not_yet_present_state_preservation_status = "preserved" if source_status_value(source_gate11z_status, "gate11k_not_yet_present_state_preservation_status") == "preserved" else "not_preserved"
    gate11l_path_defined_state_preservation_status = "preserved" if source_status_value(source_gate11z_status, "gate11l_path_defined_state_preservation_status") == "preserved" else "not_preserved"
    gate11m_not_yet_present_state_preservation_status = "preserved" if source_status_value(source_gate11z_status, "gate11m_not_yet_present_state_preservation_status") == "preserved" else "not_preserved"
    gate11n_residual_named_state_preservation_status = "preserved" if source_status_value(source_gate11z_status, "gate11n_residual_named_state_preservation_status") == "preserved" else "not_preserved"
    gate11o_path_defined_state_preservation_status = "preserved" if source_status_value(source_gate11z_status, "gate11o_path_defined_state_preservation_status") == "preserved" else "not_preserved"
    gate11p_not_yet_completed_state_preservation_status = "preserved" if source_status_value(source_gate11z_status, "gate11p_not_yet_completed_state_preservation_status") == "preserved" else "not_preserved"
    gate11q_surface_defined_state_preservation_status = "preserved" if source_status_value(source_gate11z_status, "gate11q_surface_defined_state_preservation_status") == "preserved" else "not_preserved"
    gate11r_not_yet_present_state_preservation_status = "preserved" if source_status_value(source_gate11z_status, "gate11r_not_yet_present_state_preservation_status") == "preserved" else "not_preserved"
    gate11s_path_defined_state_preservation_status = "preserved" if source_status_value(source_gate11z_status, "gate11s_path_defined_state_preservation_status") == "preserved" else "not_preserved"
    gate11t_not_yet_present_state_preservation_status = "preserved" if source_status_value(source_gate11z_status, "gate11t_not_yet_present_state_preservation_status") == "preserved" else "not_preserved"
    gate11u_residual_named_state_preservation_status = "preserved" if source_status_value(source_gate11z_status, "gate11u_residual_named_state_preservation_status") == "preserved" else "not_preserved"
    gate11v_path_defined_state_preservation_status = "preserved" if source_status_value(source_gate11z_status, "gate11v_path_defined_state_preservation_status") == "preserved" else "not_preserved"
    gate11w_not_yet_completed_state_preservation_status = "preserved" if source_status_value(source_gate11z_status, "gate11w_not_yet_completed_state_preservation_status") == "preserved" else "not_preserved"
    gate11x_blocker_named_state_preservation_status = "preserved" if source_status_value(source_gate11z_status, "gate11x_blocker_named_state_preservation_status") == "preserved" else "not_preserved"
    gate11y_path_defined_state_preservation_status = "preserved" if source_status_value(source_gate11z_status, "gate11y_path_defined_state_preservation_status") == "preserved" else "not_preserved"
    source_gate11z_resolution_status = source_status_value(source_gate11z_status, "named_residual_marker_carrier_completion_blocker_resolution_status")
    if source_gate11z_resolution_status == "not_yet_resolved":
        gate11z_not_yet_resolved_state_preservation_status = "preserved"
    elif source_gate11z_resolution_status == "deferred":
        gate11z_not_yet_resolved_state_preservation_status = "deferred"
    else:
        gate11z_not_yet_resolved_state_preservation_status = "not_preserved"
    broader_trusted_tree_settlement_still_unearned_status = "confirmed" if source_status_value(source_gate11z_status, "broader_trusted_tree_settlement_still_unearned_status") == "confirmed" else "not_confirmed"
    operator_admission_still_denied_status = "confirmed" if source_status_value(source_gate11z_status, "operator_admission_still_denied_status") == "confirmed" else "not_confirmed"
    retroactive_reinterpretation_forbidden_status = "confirmed" if source_status_value(source_gate11z_status, "retroactive_reinterpretation_forbidden_status") == "confirmed" else "not_confirmed"

    source_blocker = source_status_value(source_gate11z_status, "next_named_blocker")
    ...
    if incomplete:
        ...
        or source_status_value(source_gate11z_status, "blocker_resolution_boundary_status") != "confirmed"
    ):
    ...
    if incomplete:
        bounded_blocker_resolution_marker_rows_status = "deferred"
    elif source_status_value(source_gate11z_status, "explicit_blocker_resolution_marker_status") == "deferred":
        bounded_blocker_resolution_marker_rows_status = "deferred"
    elif source_blocker in SURFACE_FIXABLE_BLOCKERS:
        bounded_blocker_resolution_marker_rows_status = "defined"
    else:
        bounded_blocker_resolution_marker_rows_status = "not_defined"

    if incomplete:
        same_source_binding_requirement_status = "deferred"
    elif source_status_value(source_gate11z_status, "same_source_blocker_resolution_status") == "deferred":
        same_source_binding_requirement_status = "deferred"
    elif source_blocker == "multiple_candidate_resolutions":
        same_source_binding_requirement_status = "deferred"
    elif source_blocker in SURFACE_FIXABLE_BLOCKERS:
        same_source_binding_requirement_status = "defined"
    else:
        same_source_binding_requirement_status = "not_defined"

    if incomplete:
        bounded_read_prefix_requirement_status = "deferred"
    elif source_status_value(source_gate11z_status, "explicit_blocker_resolution_marker_status") == "deferred":
        bounded_read_prefix_requirement_status = "deferred"
    elif source_blocker in SURFACE_FIXABLE_BLOCKERS:
        bounded_read_prefix_requirement_status = "defined"
    else:
        bounded_read_prefix_requirement_status = "not_defined"

    if incomplete:
        named_blocker_resolution_marker_surface_status = "deferred"
    elif (
        ...
        or operator_admission_still_denied_status != "confirmed"
        or retroactive_reinterpretation_forbidden_status != "confirmed"
        or gate11z_not_yet_resolved_state_preservation_status == "not_preserved"
    ):
        named_blocker_resolution_marker_surface_status = "denied"
    elif (
        bounded_blocker_resolution_marker_rows_status == "defined"
        and same_source_binding_requirement_status == "defined"
        and bounded_read_prefix_requirement_status == "defined"
        and blocker_resolution_boundary_status == "confirmed"
    ):
        named_blocker_resolution_marker_surface_status = "surface_defined"
    elif (
        bounded_blocker_resolution_marker_rows_status == "deferred"
        or same_source_binding_requirement_status == "deferred"
        or bounded_read_prefix_requirement_status == "deferred"
        or blocker_resolution_boundary_status == "deferred"
        or gate11z_not_yet_resolved_state_preservation_status == "deferred"
    ):
        named_blocker_resolution_marker_surface_status = "deferred"
    else:
        named_blocker_resolution_marker_surface_status = "not_yet_defined"
    ...
    elif gate11z_not_yet_resolved_state_preservation_status == "not_preserved":
    ...
    elif gate11z_not_yet_resolved_state_preservation_status == "deferred":
        next_named_blocker = "upstream_blocker_resolution_state_deferred"
    elif bounded_blocker_resolution_marker_rows_status == "deferred" or same_source_binding_requirement_status == "deferred" or bounded_read_prefix_requirement_status == "deferred":
        next_named_blocker = "multiple_candidate_marker_surfaces"
    elif bounded_blocker_resolution_marker_rows_status != "defined":
        next_named_blocker = "bounded_blocker_resolution_marker_rows_not_fixed"
    elif same_source_binding_requirement_status != "defined":
        next_named_blocker = "same_source_binding_requirement_not_fixed"
    elif bounded_read_prefix_requirement_status != "defined":
        next_named_blocker = "bounded_read_prefix_requirement_not_fixed"
    else:
        next_named_blocker = ""
...
def build_report(
    ...
    lines = [
        "# Gate11AA Named Blocker-Resolution Marker Surface Read",
        "",
        f"run_id: {run_id}",
        f"source_gate11z_run_id: {source_gate11z_manifest.get('run_id', '')}",
        f"source_gate11z_code_git_commit: {source_gate11z_manifest.get('code_git_commit', '')}",
        "",
        "## Discipline",
        "",
        "- Gate11AA asks only what would count as a valid explicit blocker-resolution marker surface under the fixed Gate11Z line",
        "- Gate11AA defines marker surface only",
        "- Gate11AA does not judge blocker resolution",
        "- Gate11AA does not instantiate a blocker-resolution marker",
        "- Gate11AA does not complete the residual",
        "- Gate11AA does not admit a later source",
        "- Gate11AA does not decide one-admissible-later-source explicit-presence judgment",
        "- Gate11AA does not declare a bounded-line insufficiency candidate",
        "- Gate11AA does not declare that explicit declaration already exists",
        "- Gate11AA does not decide reopening eligibility",
        "- Gate11AA does not reopen operator admission",
    ...
    for row in policy_compare_rows:
        lines.append("| " + " | ".join([
            ...
            str(row["bounded_blocker_resolution_marker_rows_status"]),
            str(row["same_source_binding_requirement_status"]),
            str(row["bounded_read_prefix_requirement_status"]),
            str(row["blocker_resolution_boundary_status"]),
            str(row["named_blocker_resolution_marker_surface_status"]),
    ...
    lines.extend([
        ...
        f"- bounded_blocker_resolution_marker_rows_status: `{status_payload['bounded_blocker_resolution_marker_rows_status']}`",
        f"- same_source_binding_requirement_status: `{status_payload['same_source_binding_requirement_status']}`",
        f"- bounded_read_prefix_requirement_status: `{status_payload['bounded_read_prefix_requirement_status']}`",
        f"- blocker_resolution_boundary_status: `{status_payload['blocker_resolution_boundary_status']}`",
        f"- named_blocker_resolution_marker_surface_status: `{status_payload['named_blocker_resolution_marker_surface_status']}`",
        f"- next_named_blocker: `{status_payload['next_named_blocker']}`",
        "",
        "## Surface Slice",
        "",
        "- Gate11AA defines only the marker surface by which one explicit blocker-resolution marker could later count as valid",
    ...
    outcome = status_payload["named_blocker_resolution_marker_surface_status"]
    if outcome == "surface_defined":
        lines.append("- the bounded blocker-resolution marker surface is fixed narrowly enough for a later audit without converting non-resolution into blocker resolution")
    elif outcome == "not_yet_defined":
        lines.append("- the current source does not yet fix the blocker-resolution marker surface narrowly enough")
    elif outcome == "denied":
        lines.append("- the proposed surface is denied because it would depend on shortcut, inflation, rewrite, graph-wide leap, or worker-side synthesis")
    else:
        lines.append("- the frozen source is incomplete or contradictory for a blocker-resolution marker surface judgment")
...
def main() -> None:
    args = parse_args()
    source_gate11z_dir = Path(args.gate11z_dir)
    out_dir = Path(args.out_dir)
    out_dir.mkdir(parents=True, exist_ok=True)
    run_id = out_dir.name

    source_gate11z_manifest = gate9a.read_json(source_gate11z_dir / gate11z.DEFAULT_MANIFEST)
    source_gate11z_status = gate9a.read_json(source_gate11z_dir / gate11z.DEFAULT_STATUS)
    source_gate11z_report = (source_gate11z_dir / gate11z.DEFAULT_REPORT).read_text(encoding="utf-8")

    status_payload = build_status_payload(source_gate11z_manifest, source_gate11z_status, source_gate11z_report)
    registry_rows = build_registry(source_gate11z_manifest, status_payload)
    ...
    gate9a.write_text(report_path, build_report(run_id, source_gate11z_manifest, policy_compare_rows, status_payload))
    ...
    manifest = {
        ...
        "source_gate11z_run_id": str(source_gate11z_manifest.get("run_id") or ""),
        "source_gate11z_code_git_commit": str(source_gate11z_manifest.get("code_git_commit") or ""),
        "inputs": {"gate11z_dir": str(source_gate11z_dir)},
        "outputs": {
