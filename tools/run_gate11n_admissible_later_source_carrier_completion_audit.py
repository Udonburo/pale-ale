#!/usr/bin/env python3
"""Run a Gate11N admissible later-source carrier-completion audit on Gate11M outputs."""
...
SCHEMA_VERSION = "gate11n_admissible_later_source_carrier_completion_audit_v1"
METHOD_ID = "gate11n_admissible_later_source_carrier_completion_audit_v1"

DEFAULT_REGISTRY = "admissible_later_source_carrier_completion_registry.jsonl"
DEFAULT_POLICY_COMPARE = "admissible_later_source_carrier_completion_policy_compare.csv"
DEFAULT_STATUS = "admissible_later_source_carrier_completion_status.json"
DEFAULT_REPORT = "gate11n_admissible_later_source_carrier_completion_read.md"
DEFAULT_MANIFEST = "manifest.json"
DEFAULT_CHECKSUMS = "checksums.json"

REQUIRED_GATE11M_STATUS_KEYS = (
    ...
    "explicit_admissible_later_source_presence_marker_status",
    "later_source_singularity_status",
    "same_source_fixed_gate11l_path_instantiation_status",
    "admissibility_boundary_status",
    "one_admissible_later_source_explicit_presence_path_instantiation_status",
    "next_named_blocker",
)

KNOWN_RESIDUAL_BLOCKERS = {
    "no_explicit_admissible_later_source_presence_marker",
    "multiple_later_sources",
    "fixed_gate11l_path_not_instantiated_on_same_source",
}
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description=(
            "Build a Gate11N admissible later-source carrier-completion audit "
            "from the frozen Gate11M path-instantiation run without deciding reopening eligibility."
        )
    )
    parser.add_argument("--gate11m-dir", required=True)
...
def source_is_incomplete(
    source_gate11m_manifest: Dict[str, Any], source_gate11m_status: Dict[str, Any], report_text: str
) -> bool:
    if not str(source_gate11m_manifest.get("run_id") or ""):
        return True
    if not report_text.strip():
        return True
    return any(not source_status_value(source_gate11m_status, key) for key in REQUIRED_GATE11M_STATUS_KEYS)


def blocker_sentence(blocker: str) -> str:
    mapping = {
        "no_explicit_admissible_later_source_presence_marker": "the residual carrier condition is named narrowly: no explicit admissible later-source presence marker is yet carried under the fixed Gate11M line",
        "multiple_later_sources": "the residual carrier condition is named narrowly: later-source singularity is still incomplete because multiple later sources remain unresolved",
        "fixed_gate11l_path_not_instantiated_on_same_source": "the residual carrier condition is named narrowly: same-source attachment to the fixed Gate11L path is still incomplete",
    }
    return mapping.get(blocker, "the residual carrier condition is not yet named narrowly enough")


def build_registry(
    source_gate11m_manifest: Dict[str, Any],
    status_payload: Dict[str, Any],
) -> List[Dict[str, Any]]:
    return [
        {
            "source_gate11m_run_id": str(source_gate11m_manifest.get("run_id") or ""),
            "source_gate11m_code_git_commit": str(
                source_gate11m_manifest.get("code_git_commit") or ""
            ),
            ...
            "explicit_presence_marker_carrier_completion_status": str(status_payload["explicit_presence_marker_carrier_completion_status"]),
            "later_source_singularity_carrier_completion_status": str(status_payload["later_source_singularity_carrier_completion_status"]),
            ...
            "admissible_later_source_carrier_completion_status": str(status_payload["admissible_later_source_carrier_completion_status"]),
...
def build_policy_compare(registry_rows: Sequence[Dict[str, Any]]) -> List[Dict[str, Any]]:
    return [
        {
            "source_gate11m_run_id": str(row["source_gate11m_run_id"]),
            ...
            "explicit_presence_marker_carrier_completion_status": str(row["explicit_presence_marker_carrier_completion_status"]),
            "later_source_singularity_carrier_completion_status": str(row["later_source_singularity_carrier_completion_status"]),
            ...
            "admissible_later_source_carrier_completion_status": str(row["admissible_later_source_carrier_completion_status"]),
...
def build_status_payload(
    source_gate11m_manifest: Dict[str, Any],
    source_gate11m_status: Dict[str, Any],
    report_text: str,
) -> Dict[str, Any]:
    incomplete = source_is_incomplete(source_gate11m_manifest, source_gate11m_status, report_text)

    gate10_closeout_preservation_status = "preserved" if source_status_value(source_gate11m_status, "gate10_closeout_preservation_status") == "preserved" else "not_preserved"
    gate11a_absence_result_preservation_status = "preserved" if source_status_value(source_gate11m_status, "gate11a_absence_result_preservation_status") == "preserved" else "not_preserved"
    gate11c_declaration_surface_preservation_status = "preserved" if source_status_value(source_gate11m_status, "gate11c_declaration_surface_preservation_status") == "preserved" else "not_preserved"
    gate11d_not_yet_declared_state_preservation_status = "preserved" if source_status_value(source_gate11m_status, "gate11d_not_yet_declared_state_preservation_status") == "preserved" else "not_preserved"
    gate11e_path_defined_state_preservation_status = "preserved" if source_status_value(source_gate11m_status, "gate11e_path_defined_state_preservation_status") == "preserved" else "not_preserved"
    gate11f_not_yet_admissible_state_preservation_status = "preserved" if source_status_value(source_gate11m_status, "gate11f_not_yet_admissible_state_preservation_status") == "preserved" else "not_preserved"
    gate11g_naming_surface_preservation_status = "preserved" if source_status_value(source_gate11m_status, "gate11g_naming_surface_preservation_status") == "preserved" else "not_preserved"
    gate11h_not_yet_named_state_preservation_status = "preserved" if source_status_value(source_gate11m_status, "gate11h_not_yet_named_state_preservation_status") == "preserved" else "not_preserved"
    gate11i_path_defined_state_preservation_status = "preserved" if source_status_value(source_gate11m_status, "gate11i_path_defined_state_preservation_status") == "preserved" else "not_preserved"
    gate11j_not_yet_admissible_state_preservation_status = "preserved" if source_status_value(source_gate11m_status, "gate11j_not_yet_admissible_state_preservation_status") == "preserved" else "not_preserved"
    gate11k_not_yet_present_state_preservation_status = "preserved" if source_status_value(source_gate11m_status, "gate11k_not_yet_present_state_preservation_status") == "preserved" else "not_preserved"
    gate11l_path_defined_state_preservation_status = "preserved" if source_status_value(source_gate11m_status, "gate11l_path_defined_state_preservation_status") == "preserved" else "not_preserved"
    gate11m_not_yet_present_state_preservation_status = "preserved" if source_status_value(source_gate11m_status, "one_admissible_later_source_explicit_presence_path_instantiation_status") == "not_yet_present" else "not_preserved"
    broader_trusted_tree_settlement_still_unearned_status = "confirmed" if source_status_value(source_gate11m_status, "broader_trusted_tree_settlement_still_unearned_status") == "confirmed" else "not_confirmed"
    operator_admission_still_denied_status = "confirmed" if source_status_value(source_gate11m_status, "operator_admission_still_denied_status") == "confirmed" else "not_confirmed"
    retroactive_reinterpretation_forbidden_status = "confirmed" if source_status_value(source_gate11m_status, "retroactive_reinterpretation_forbidden_status") == "confirmed" else "not_confirmed"

    if incomplete:
        explicit_presence_marker_carrier_completion_status = "deferred"
    elif source_status_value(source_gate11m_status, "explicit_admissible_later_source_presence_marker_status") == "present":
        explicit_presence_marker_carrier_completion_status = "complete"
    else:
        explicit_presence_marker_carrier_completion_status = "missing"

    if incomplete:
        later_source_singularity_carrier_completion_status = "deferred"
    elif source_status_value(source_gate11m_status, "later_source_singularity_status") == "single":
        later_source_singularity_carrier_completion_status = "complete"
    else:
        later_source_singularity_carrier_completion_status = "missing"

    if incomplete:
        same_source_path_attachment_carrier_completion_status = "deferred"
    elif source_status_value(source_gate11m_status, "same_source_fixed_gate11l_path_instantiation_status") == "instantiated":
    ...
    if incomplete:
        ...
        or source_status_value(source_gate11m_status, "admissibility_boundary_status") != "confirmed"
    ):
        carrier_completion_boundary_status = "denied"
    else:
        carrier_completion_boundary_status = "confirmed"

    next_named_blocker = source_status_value(source_gate11m_status, "next_named_blocker")
    residual_condition_explicitly_named = next_named_blocker in KNOWN_RESIDUAL_BLOCKERS

    if incomplete:
        admissible_later_source_carrier_completion_status = "deferred"
    elif (
        ...
        admissible_later_source_carrier_completion_status = "denied"
    elif residual_condition_explicitly_named:
        admissible_later_source_carrier_completion_status = "residual_named"
    else:
        admissible_later_source_carrier_completion_status = "not_yet_named"
...
def build_report(
    ...
    lines = [
        "# Gate11N Admissible Later-Source Carrier-Completion Read",
        "",
        f"run_id: {run_id}",
        f"source_gate11m_run_id: {source_gate11m_manifest.get('run_id', '')}",
        f"source_gate11m_code_git_commit: {source_gate11m_manifest.get('code_git_commit', '')}",
        "",
        "## Discipline",
        "",
        "- Gate11N asks only which residual carrier-completion condition still blocks one admissible later source from being actually present under the fixed Gate11M line",
        "- Gate11N does not redesign prior surfaces, paths, or admissibility rules",
        "- Gate11N does not admit a later source",
        "- Gate11N does not create explicit presence",
        "- Gate11N does not decide reopening eligibility",
        "- Gate11N does not reopen operator admission",
        "- Gate11N does not declare a candidate",
        "- Gate11N does not declare that explicit declaration already exists",
        ...
        "- no retroactive rewrite of Gate9, Gate10, Gate11A, Gate11B, Gate11C, Gate11D, Gate11E, Gate11F, Gate11G, Gate11H, Gate11I, Gate11J, Gate11K, Gate11L, or Gate11M memory is permitted",
        "",
        "## Source Summary",
        "",
        "| source_gate11m_run_id | gate10_closeout_preservation_status | gate11a_absence_result_preservation_status | gate11c_declaration_surface_preservation_status | gate11d_not_yet_declared_state_preservation_status | gate11e_path_defined_state_preservation_status | gate11f_not_yet_admissible_state_preservation_status | gate11g_naming_surface_preservation_status | gate11h_not_yet_named_state_preservation_status | gate11i_path_defined_state_preservation_status | gate11j_not_yet_admissible_state_preservation_status | gate11k_not_yet_present_state_preservation_status | gate11l_path_defined_state_preservation_status | gate11m_not_yet_present_state_preservation_status | broader_trusted_tree_settlement_still_unearned_status | operator_admission_still_denied_status | retroactive_reinterpretation_forbidden_status | explicit_presence_marker_carrier_completion_status | later_source_singularity_carrier_completion_status | same_source_path_attachment_carrier_completion_status | carrier_completion_boundary_status | admissible_later_source_carrier_completion_status | next_named_blocker |",
        "|---|---|---|---|---|---|---|---|---|---|---|---|---|---|---|---|---|---|---|---|---|---|---|",
    ]
    for row in policy_compare_rows:
        lines.append("| " + " | ".join([
            str(row["source_gate11m_run_id"]),
            ...
            str(row["explicit_presence_marker_carrier_completion_status"]),
            str(row["later_source_singularity_carrier_completion_status"]),
            str(row["same_source_path_attachment_carrier_completion_status"]),
            str(row["carrier_completion_boundary_status"]),
            str(row["admissible_later_source_carrier_completion_status"]),
    ...
    lines.extend([
        ...
        f"- explicit_presence_marker_carrier_completion_status: `{status_payload['explicit_presence_marker_carrier_completion_status']}`",
        f"- later_source_singularity_carrier_completion_status: `{status_payload['later_source_singularity_carrier_completion_status']}`",
        ...
        f"- admissible_later_source_carrier_completion_status: `{status_payload['admissible_later_source_carrier_completion_status']}`",
        f"- next_named_blocker: `{status_payload['next_named_blocker']}`",
        "",
        "## Carrier-Completion Slice",
        "",
        "- Gate11N names residual carrier conditions only: explicit presence marker completion, later-source singularity completion, same-source fixed-path attachment completion, and carrier-completion boundary status",
        "- Gate11N does not instantiate any missing carrier condition",
        "",
        "## Judgment",
        "",
    ])

    outcome = status_payload["admissible_later_source_carrier_completion_status"]
    if outcome == "residual_named":
        lines.append(f"- {blocker_sentence(status_payload['next_named_blocker'])}")
    elif outcome == "not_yet_named":
        lines.append("- the fixed Gate11M line remains preserved, but the residual carrier condition is not yet named narrowly enough")
    elif outcome == "denied":
        lines.append("- the attempted carrier-completion naming is denied because it would depend on shortcut, inflation, rewrite, leap, or worker-side synthesis")
    else:
        lines.append("- the frozen source is incomplete for a residual carrier-completion naming judgment")
...
def main() -> None:
    args = parse_args()
    source_gate11m_dir = Path(args.gate11m_dir)
    out_dir = Path(args.out_dir)
    out_dir.mkdir(parents=True, exist_ok=True)
    run_id = out_dir.name

    source_gate11m_manifest = gate9a.read_json(source_gate11m_dir / gate11m.DEFAULT_MANIFEST)
    source_gate11m_status = gate9a.read_json(source_gate11m_dir / gate11m.DEFAULT_STATUS)
    source_gate11m_report = (source_gate11m_dir / gate11m.DEFAULT_REPORT).read_text(encoding="utf-8")

    status_payload = build_status_payload(source_gate11m_manifest, source_gate11m_status, source_gate11m_report)
    registry_rows = build_registry(source_gate11m_manifest, status_payload)
    ...
    gate9a.write_text(report_path, build_report(run_id, source_gate11m_manifest, policy_compare_rows, status_payload))
    ...
    manifest = {
        ...
        "source_gate11m_run_id": str(source_gate11m_manifest.get("run_id") or ""),
        "source_gate11m_code_git_commit": str(source_gate11m_manifest.get("code_git_commit") or ""),
        "inputs": {"gate11m_dir": str(source_gate11m_dir)},
        "outputs": {
