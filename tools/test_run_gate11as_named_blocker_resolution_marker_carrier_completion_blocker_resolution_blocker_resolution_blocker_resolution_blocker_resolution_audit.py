#!/usr/bin/env python3
"""Regression tests for Gate11AS named blocker-resolution marker carrier-completion blocker-resolution blocker-resolution blocker-resolution blocker resolution helpers."""
...
def make_gate11ar_manifest(run_id: str = "gate11ar_run") -> dict:
    return {"run_id": run_id, "code_git_commit": "9d16856ff0ac48e1e87480415f5dc0eaaeccadd4"}


def make_gate11ar_status(
    ...
    blocker_resolution_marker_carrier_completion_blocker_resolution_blocker_resolution_blocker_resolution_blocker_resolution_path_status: str = "path_defined",
    ...
    return {
        ...
        "blocker_resolution_marker_carrier_completion_blocker_resolution_blocker_resolution_blocker_resolution_blocker_resolution_path_status": blocker_resolution_marker_carrier_completion_blocker_resolution_blocker_resolution_blocker_resolution_blocker_resolution_path_status,
        "next_named_blocker": next_named_blocker,
    }


class RunGate11ASNamedBlockerResolutionMarkerCarrierCompletionBlockerResolutionBlockerResolutionBlockerResolutionBlockerResolutionAuditTest(unittest.TestCase):
    def test_not_yet_resolved_for_current_frozen_gate11ar_source(self) -> None:
        status = gate11as.build_status_payload(
            make_gate11ar_manifest(),
            make_gate11ar_status(),
            "Gate11AR preserves the fixed blocker-resolution path but does not explicitly resolve the named blocker.",
        )

        self.assertEqual(status["gate11ar_path_defined_state_preservation_status"], "preserved")
    ...
    later_source_id: runs/gate11as_future
    ...
    candidate_id: gate11as_candidate_001
    class: host_failure
...
"""
        status = gate11as.build_status_payload(make_gate11ar_manifest(), make_gate11ar_status(), report_text)
    ...
    def test_deferred_when_multiple_candidate_later_source_identifiers_compete(self) -> None:
        report_text = """
    blocker_resolution_marker_status: present
    later_source_id: runs/gate11as_future_a
    later_frozen_run_id: runs/gate11as_future_b
"""
        status = gate11as.build_status_payload(make_gate11ar_manifest(), make_gate11ar_status(), report_text)
    ...
    def test_denied_when_boundary_breaks(self) -> None:
        status = gate11as.build_status_payload(
            make_gate11ar_manifest(),
            make_gate11ar_status(blocker_resolution_boundary_status="not_confirmed"),
            "Gate11AR breaks the blocker-resolution boundary.",
        )
    ...
    def test_deferred_when_source_is_incomplete(self) -> None:
        status = gate11as.build_status_payload(make_gate11ar_manifest(run_id=""), make_gate11ar_status(), "")
    ...
    def test_policy_compare_echoes_single_source_row(self) -> None:
        ...
        registry = gate11as.build_registry(make_gate11ar_manifest(), status)
        compare = gate11as.build_policy_compare(registry)

        self.assertEqual(len(compare), 1)
        self.assertEqual(compare[0]["source_gate11ar_run_id"], "gate11ar_run")
        self.assertEqual(
            compare[0][
                "named_blocker_resolution_marker_carrier_completion_blocker_resolution_blocker_resolution_blocker_resolution_blocker_resolution_status"
            ],
