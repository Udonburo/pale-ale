#!/usr/bin/env python3
"""Run a minimal Gate7 progression leakage consumer from Gate6 native objects."""
...
DEFAULT_TOKEN_CSV = "gate7a_token_telemetry.csv"
DEFAULT_SAMPLE_CSV = "gate7a_sample_summary.csv"
DEFAULT_AGGREGATE = "gate7a_aggregate_summary.md"

SCHEMA_VERSION = "gate7_progression_leak_consumer_artifacts_v1"
METHOD_ID = "gate7_progression_leak_consumer_v1"
PRIMARY_METRIC_ID = "progression_leak_v1"
PRIMARY_AUX_METRIC_ID = "progression_projection_energy_ratio_v1"
PRIMARY_LEAKAGE_METRIC_ID = "progression_projected_norm_v1"
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description=(
            "Run a minimal Gate7 progression consumer that measures leakage of V_{t+1} "
            "out of the current local projector P_t."
        )
    )
...
def compute_progression_metrics(
    ...
    projected_coeffs = current_basis_slice.T @ next_v
    projected = current_basis_slice @ projected_coeffs
    next_norm_sq = float(np.dot(next_v, next_v))
    projected_norm_sq = float(np.dot(projected, projected))
    if not np.isfinite(next_norm_sq) or next_norm_sq <= 1e-12 or not np.isfinite(projected_norm_sq):
        return {
            "loop_outcome": "invalid_projection_energy",
            BASELINE_METRIC_ID: None,
            PRIMARY_METRIC_ID: None,
            PRIMARY_AUX_METRIC_ID: None,
            PRIMARY_LEAKAGE_METRIC_ID: None,
    ...
    energy_ratio = float(np.clip(projected_norm_sq / next_norm_sq, 0.0, 1.0))
    progression_leak = float(np.clip(1.0 - energy_ratio, 0.0, 1.0))
    projected_norm = float(np.sqrt(max(0.0, projected_norm_sq)))
    return {
        "loop_outcome": "none",
        BASELINE_METRIC_ID: score_f,
        PRIMARY_METRIC_ID: progression_leak,
        PRIMARY_AUX_METRIC_ID: energy_ratio,
        PRIMARY_LEAKAGE_METRIC_ID: projected_norm,
    }
...
def build_aggregate_summary(
    ...
    lines = [
        "# Gate7 Progression Leak Consumer Summary",
        "",
        f"run_id: {run_id}",
