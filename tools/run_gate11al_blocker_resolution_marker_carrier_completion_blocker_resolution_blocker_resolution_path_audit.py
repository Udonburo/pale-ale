#!/usr/bin/env python3
"""Run a Gate11AL blocker-resolution marker carrier-completion blocker-resolution blocker-resolution path audit on Gate11AK outputs."""
...
import run_gate11ak_blocker_resolution_marker_carrier_completion_blocker_resolution_blocker_audit as gate11ak
import run_gate9a_graph_gauge_consumer as gate9a


SCHEMA_VERSION = "gate11al_blocker_resolution_marker_carrier_completion_blocker_resolution_blocker_resolution_path_audit_v1"
METHOD_ID = "gate11al_blocker_resolution_marker_carrier_completion_blocker_resolution_blocker_resolution_path_audit_v1"
DEFAULT_REGISTRY = "blocker_resolution_marker_carrier_completion_blocker_resolution_blocker_resolution_path_registry.jsonl"
DEFAULT_POLICY_COMPARE = "blocker_resolution_marker_carrier_completion_blocker_resolution_blocker_resolution_path_policy_compare.csv"
DEFAULT_STATUS = "blocker_resolution_marker_carrier_completion_blocker_resolution_blocker_resolution_path_status.json"
DEFAULT_REPORT = "gate11al_blocker_resolution_marker_carrier_completion_blocker_resolution_blocker_resolution_path_read.md"
DEFAULT_MANIFEST = "manifest.json"
DEFAULT_CHECKSUMS = "checksums.json"

REQUIRED_GATE11AK_STATUS_KEYS = gate11ak.STATUS_FIELDS
KNOWN_BLOCKERS = gate11ak.KNOWN_BLOCKERS
PATH_DEFINABLE_BLOCKERS = {
    "no_explicit_blocker_resolution_marker",
}
STATUS_FIELDS = (
    *gate11ak.SOURCE_PRESERVATION_KEYS,
    ...
    *gate11ah.CONFIRMED_KEYS,
    ...
    "blocker_resolution_marker_carrier_completion_blocker_resolution_blocker_resolution_path_status",
    "next_named_blocker",
)
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description=(
            "Build a Gate11AL blocker-resolution marker carrier-completion blocker-resolution blocker-resolution path audit "
            "from the frozen Gate11AK blocker audit without resolving the blocker itself."
        )
    )
    parser.add_argument("--gate11ak-dir", required=True)
...
def source_is_incomplete(source_manifest: Dict[str, Any], source_status: Dict[str, Any], report_text: str) -> bool:
    ...
    if not report_text.strip():
        return True
    return any(key not in source_status for key in REQUIRED_GATE11AK_STATUS_KEYS)
...
def blocker_sentence(blocker: str) -> str:
    mapping = {
        ...
        "minimum_same_source_blocker_resolution_rule_not_fixed": (
            "the named blocker-resolution marker carrier-completion blocker-resolution blocker remains preserved, but the minimum same-source blocker-resolution rule is not yet fixed narrowly enough"
        ),
        "bounded_read_prefix_resolution_requirement_not_fixed": (
            "the named blocker-resolution marker carrier-completion blocker-resolution blocker remains preserved, but the bounded read-prefix resolution requirement is not yet fixed narrowly enough"
        ),
        "named_blocker_not_preserved": (
            "the controlling source no longer preserves the named blocker-resolution marker carrier-completion blocker-resolution blocker narrowly enough for path definition"
        ),
    }
    return mapping.get(blocker, "the blocker-resolution marker carrier-completion blocker-resolution blocker-resolution path is not yet fixed narrowly enough")


def build_registry(source_manifest: Dict[str, Any], status_payload: Dict[str, Any]) -> List[Dict[str, Any]]:
    row = {
        "source_gate11ak_run_id": str(source_manifest.get("run_id") or ""),
        "source_gate11ak_code_git_commit": str(source_manifest.get("code_git_commit") or ""),
    }
...
def build_policy_compare(registry_rows: Sequence[Dict[str, Any]]) -> List[Dict[str, Any]]:
    return [{"source_gate11ak_run_id": str(row["source_gate11ak_run_id"]), **{key: str(row[key]) for key in STATUS_FIELDS}} for row in registry_rows]
...
def build_status_payload(source_manifest: Dict[str, Any], source_status: Dict[str, Any], report_text: str) -> Dict[str, Any]:
    ...
    status_payload = {
        ...
        for key in gate11ak.SOURCE_PRESERVATION_KEYS
    }

    source_gate11aj_status = source_status_value(source_status, "gate11aj_not_yet_resolved_state_preservation_status")
    if source_gate11aj_status == "preserved":
    ...
    source_gate11ak_status = source_status_value(
        source_status, "blocker_resolution_marker_carrier_completion_blocker_resolution_blocker_status"
    )
    if source_gate11ak_status == "blocker_named":
        gate11ak_blocker_named_state_preservation_status = "preserved"
    elif source_gate11ak_status == "deferred":
        gate11ak_blocker_named_state_preservation_status = "deferred"
    else:
        gate11ak_blocker_named_state_preservation_status = "not_preserved"
    status_payload["gate11ak_blocker_named_state_preservation_status"] = gate11ak_blocker_named_state_preservation_status
    ...
    elif gate11ak_blocker_named_state_preservation_status == "deferred":
        minimum_same_source_blocker_resolution_rule_status = "deferred"
    elif gate11ak_blocker_named_state_preservation_status != "preserved":
        minimum_same_source_blocker_resolution_rule_status = "not_defined"
    ...
    if incomplete:
        ...
        any(status_payload[key] != "preserved" for key in gate11ak.SOURCE_PRESERVATION_KEYS)
        or gate11aj_not_yet_resolved_state_preservation_status == "not_preserved"
        or gate11ak_blocker_named_state_preservation_status == "not_preserved"
        ...
        gate11aj_not_yet_resolved_state_preservation_status == "deferred"
        or gate11ak_blocker_named_state_preservation_status == "deferred"
    ...
    status_payload["blocker_resolution_marker_carrier_completion_blocker_resolution_blocker_resolution_path_status"] = overall
    ...
    if not next_named_blocker and gate11ak_blocker_named_state_preservation_status == "not_preserved":
        next_named_blocker = "gate11ak_blocker_named_state_not_preserved"
...
def build_report(
    ...
    lines = [
        "# Gate11AL Blocker-Resolution Marker Carrier-Completion Blocker-Resolution Blocker-Resolution Path Read",
        "",
        f"run_id: {run_id}",
        f"source_gate11ak_run_id: {source_manifest.get('run_id', '')}",
        f"source_gate11ak_code_git_commit: {source_manifest.get('code_git_commit', '')}",
        "",
        "## Discipline",
        "",
        "- Gate11AL asks only what is the minimum honest path by which the named blocker-resolution marker carrier-completion blocker-resolution blocker could later be resolved under the fixed Gate11AK line",
        "- Gate11AL defines path only",
        "- Gate11AL does not resolve the blocker",
        "- Gate11AL does not convert path definition into blocker resolution",
        "- Gate11AL does not widen into blocker-resolution marker carrier completion, blocker resolution, residual completion, later-source admission, or reopening",
        "- Gate11AL does not treat prose as blocker-resolution evidence",
        "",
        "## Source Summary",
        "",
        "| source_gate11ak_run_id | " + " | ".join(STATUS_FIELDS) + " |",
        "|---|" + "---|" * len(STATUS_FIELDS),
    ]
    for row in policy_compare_rows:
        lines.append("| " + " | ".join([str(row["source_gate11ak_run_id"]), *[str(row[key]) for key in STATUS_FIELDS]]) + " |")
    lines.extend(["", "## Status", ""])
    lines.extend([f"- {key}: `{status_payload[key]}`" for key in STATUS_FIELDS])
    lines.extend(["", "## Judgment", ""])

    outcome = status_payload["blocker_resolution_marker_carrier_completion_blocker_resolution_blocker_resolution_path_status"]
    if outcome == "path_defined":
        ...
        lines.append("- the fixed Gate11AK line still does not define the blocker-resolution path narrowly enough")
    elif outcome == "denied":
...
def main() -> None:
    args = parse_args()
    source_dir = Path(args.gate11ak_dir)
    out_dir = Path(args.out_dir)
    out_dir.mkdir(parents=True, exist_ok=True)
    run_id = out_dir.name

    source_manifest = gate9a.read_json(source_dir / gate11ak.DEFAULT_MANIFEST)
    source_status = gate9a.read_json(source_dir / gate11ak.DEFAULT_STATUS)
    source_report = (source_dir / gate11ak.DEFAULT_REPORT).read_text(encoding="utf-8")
    ...
    gate9a.write_csv(policy_compare_path, ("source_gate11ak_run_id", *STATUS_FIELDS), policy_compare_rows)
    ...
    gate9a.write_json(
        manifest_path,
        {
            ...
            "source_gate11ak_run_id": str(source_manifest.get("run_id") or ""),
            "source_gate11ak_code_git_commit": str(source_manifest.get("code_git_commit") or ""),
            "inputs": {"gate11ak_dir": str(source_dir)},
            "outputs": {
