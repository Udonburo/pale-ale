...
| The archive signature survives stricter scale support and motif observer-scope expansion | archive motif top-k runs plus archive strict/core boundary runs |
...
| `archive_core_boundary_runs` | Gate12B strict/core archive boundary runs | 24 | support `221` / `222` boundary discussion | 24/24 `checksums.json` present; mismatch count `0` |
...
This set contains the 36 motif strict runs used by `223` and `228`.
...
## 6. Source Queue and Annotation Set
...
## 7. Archive Strict/Core Boundary Set
...
| Table 2: Archive strict support across `top_k=1/3/5` | motif Gate12B archive runs; archive strict/core boundary set if appendix includes default-core boundary |
