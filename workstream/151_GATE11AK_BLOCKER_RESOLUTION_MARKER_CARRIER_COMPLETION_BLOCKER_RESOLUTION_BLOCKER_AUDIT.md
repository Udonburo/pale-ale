...
- ask which blocker-resolution marker carrier-completion blocker-resolution blocker still blocks resolution under the fixed Gate11AJ line
...
- name only the blocker-resolution marker carrier-completion blocker-resolution blocker that still blocks resolution

Gate11AK does not:

- judge blocker-resolution marker carrier completion beyond naming the blocker
- judge blocker resolution beyond naming the blocker
...
The Gate11AK question is:

- `which blocker-resolution marker carrier-completion blocker-resolution blocker still blocks resolution under the fixed Gate11AJ line?`
...
- the residual follow-up gate for naming what still blocks resolution under the fixed Gate11AJ line
...
- name the blocker-resolution marker carrier-completion blocker-resolution blocker that still blocks resolution under the fixed Gate11AJ line

## 4. Blocker Discipline

Gate11AK must name the blocker, not resolve it.

The blocker counts as named only if all of the following remain explicit:

- the named blocker-resolution marker carrier-completion blocker remains preserved
...
If those conditions are not named explicitly, Gate11AK must not promote the line into blocker naming.

## 5. Expected Default

If Gate11AJ lands on the conservative default implied by the current Gate11AI source, the honest carried default into Gate11AK is:

- `named_blocker_resolution_marker_carrier_completion_blocker_resolution_status = not_yet_resolved`
- `next_named_blocker = no_explicit_blocker_resolution_marker`

So the most likely Gate11AK result under that source is:

- `blocker_named`

## 6. Outcome Ladder

Gate11AK outcomes are limited to these four.

### 6.1 Blocker Named

Use `blocker_named` if:

- the blocker-resolution marker carrier-completion blocker-resolution blocker is now named narrowly enough for a later paired path slice
...
Gate11AK is not a reopening chapter.
...
- the narrow Gate11 court for naming which blocker-resolution marker carrier-completion blocker-resolution blocker still blocks resolution under the fixed Gate11AJ line

The memory sentence to preserve is:

- `Gate11AK does not widen the line beyond the fixed Gate11AJ path; it names only which blocker-resolution marker carrier-completion blocker-resolution blocker still blocks resolution there.`
