...
WIDTHS = [(30,70), (34,33,33), (13,21,22,16,28), (10,19,29,22,20),
          (9,15,12,15,21,14,14), (15,23,13,15,20,14),
          (40,30,30), (16,28,25,31)]
    ...
    while i<len(blocks):
        ...
        if b["t"]=="Header":
            ...
            if plain(b["c"][2])=="References":
                out.extend([raw("#pagebreak(weak: true)"),b,
                            raw("#set text(size: 9pt)\n#set par(leading: .45em, spacing: .4em)")])
                references=True
                i+=1
                continue
        if references and b["t"]=="OrderedList":
            # Keep each entry intact without making the full bibliography an
            # indivisible block. Pandoc retains the list's original numbering.
            b["c"][1]=[[raw("#block(breakable: false)["),*entry,raw("]")]
                       for entry in b["c"][1]]
        title=plain(b["c"]) if b["t"]=="Para" else ""
        if (title.startswith("Corollary 1.") and i+1<len(blocks)
                and blocks[i+1]["t"]=="Para"
                and any(x["t"]=="Math" and x["c"][0]["t"]=="DisplayMath"
                        for x in blocks[i+1]["c"])):
            # Keep the statement with its defining covariance display.
            out.extend([raw("#keep["),b,blocks[i+1],raw("]")])
            i+=2
            continue
        if (b["t"]=="Para" and title and len(title)<260 and i+1<len(blocks)
                and blocks[i+1]["t"]=="Para" and len(blocks[i+1]["c"])==1
                and blocks[i+1]["c"][0]["t"]=="Math"
                and blocks[i+1]["c"][0]["c"][0]["t"]=="DisplayMath"):
            # Keep a short equation introduction with the equation it introduces.
            out.extend([raw("#keep["),b,blocks[i+1],raw("]")])
            i+=2
            continue
        if b["t"]=="Para" and i+1<len(blocks) and blocks[i+1]["t"]=="Table":
            group=[b,blocks[i+1]]
            advance=2
            if (title.startswith("The payoff also changes") and i+2<len(blocks)
                    and blocks[i+2]["t"]=="Para"):
                group.append(blocks[i+2])
                advance=3
            out.extend([raw("#keep["),*group,raw("]")])
            i+=advance
            continue
        if b["t"]=="Para" and any(x["t"]=="Image" for x in b["c"]):
            ...
            # Keep figures in the reading order, before the next subsection,
            # with their captions and without an automatic second caption.
