...
The A2 intervention changed the visible current register while preserving the next action and remaining protocol. Each transition call received the current visible register, the next action, and the frozen protocol context, but not the preceding action history. In self-fed continuation, the previous forced-choice state prediction became the next current register. After an overwrite, the edited register was therefore the only task-relevant carrier of prior state available to the next transition; the model could not reconstruct that state from an earlier action sequence included in the prompt. In both Qwen3.5-27B and Qwen3.6-27B, the immediate successor was correct in 8/8 cases, all 32/32 downstream states followed the externally implied continuation, and the final state flipped in 8/8 cases; a marker-only control left continuation unchanged in 40/40 cases. This supports selective use of the written register under the frozen intervention family, not a hidden register, unique mediation, or an internal realization of Equation~\eqref{eq:naturality}. Controlled edits to explicit intermediate structures motivate the same separation between apparent consistency and causal use~\cite{somov2026}.
...
Qwen3.6-27B was the highest-priority A2-passing checkpoint under the selection rule frozen before the panel was opened. The bounded operator study used layers 21, 43, and 62; 24 activation samples per node per independently generated, disjoint half; four nodes from the exact square plus one matched broken-control node; centered rank-4 frames; node-wise orthogonal Procrustes alignment; and frozen paired ridge coordinate maps. Activations were collected at the last prompt position immediately before the forced-choice answer token. The run used 240 unique scientific forwards.
...
Stage E was conditional on successful Stage M qualification. Because the map did not qualify, Stage E executed zero forwards. The functional quantity
\begin{equation}
  Y_b=\sqrt{\operatorname{mean}_e\left[(m_{beP}-m_{beQ})^2\right]}
\end{equation}
was never collected. The planned leave-one-block-out predictive comparison and directional Freedman--Lane permutation test were never run. There is no observed coefficient, predictive increment, $p$-value, null association, or power statement about an observed outcome.
...
Scientific code, frozen protocol files, and the terminal closeout are available from the public repository at \url{https://github.com/Udonburo/pale-ale}, with the execution record fixed at commit \hash{407d20fd4f074b9ef4524c82c8136874efaec476}. The Zenodo release accompanying this report contains a model-forward-free reproducibility capsule, exact model and runtime identities, the 20-block and 60 layer-block qualification records, deterministic Figure~\ref{fig:main} and Table~\ref{tab:gate-decomposition} inputs, licenses, and top-level \code{SHA256SUMS}. Its archival DOI is inserted in the release build after reservation. No new model execution or activation extraction was performed for reporting, and no inferential outcome analysis was opened.
...
\begin{table}[ht]
\centering
\caption{Exact fresh checkpoint revisions. The 8B row is a preserved baseline bound from the preceding campaign rather than a new execution in this panel.}
\label{tab:revisions}
...
Models were loaded unquantized in BF16 with frozen weights. Qwen3-14B used an NVIDIA L40S with Python 3.11.2, PyTorch 2.7.1+cu126, CUDA 12.6, Transformers 5.15.0, and tokenizers 0.22.2. The 27B panel and operator runs used an NVIDIA A100-80GB with the same Python, PyTorch, CUDA, and tokenizers versions and Transformers 5.15.1 at commit \hash{550d7b3834670483a4df436541272c055dc364bf}. The forced-choice score used the first semantic assistant-answer token at the validated score slot. Generation, sampling, generated chain-of-thought, and quantization were absent.
...
A0 used 80 calls per fresh checkpoint. Its primary components were 32 one-step transitions and eight exact self-fed rollouts, with frozen strata and instrument checks filling the remaining ledger. A1 used 64 calls: 16 correct demonstrations and three 16-case controls (corrupted transition, label-shuffled, and format-matched). A2 used 88 calls: eight immediate successors, 32 downstream steps, eight final-state flips, and 40 marker-only no-change controls. Thus a checkpoint reaching A2 used at most 232 new behavioral calls. No failed upstream checkpoint was opened downstream.
...
The direct semantic codebook and answer tokens were verified as single tokens at the exact score position for every checkpoint. The Qwen3.5 and Qwen3.6 A1 controls were identical in count and structure, while token identifiers were recorded per tokenizer revision. The panel did not adapt prompts after observing an earlier checkpoint.
...
The 20 block identifiers were fixed as \code{tc-b00-d2} through \code{tc-b19-d8} in depth-major order, five per depth. Public labels B01--B20 in Figure~\ref{fig:main} map one-to-one to those exact identifiers in order (B01 $=$ \code{tc-b00-d2}, \ldots, B20 $=$ \code{tc-b19-d8}); the figure does not sort blocks by outcome. Each block used 240 map forwards, for 4,800 total. Accepted case identifiers were unique. The result artifacts contained all 4,800 activation hashes and complete case-ID coverage.
...
\begin{equation}
 ...
 \label{eq:Rb}
\end{equation}
Multiplication across unrelated half gauges was forbidden. The single planned functional outcome was the root-mean-square path-margin discrepancy $Y_b$ shown in Section~\ref{sec:fresh} over 24 behavior episodes per block.

The nuisance model contained an intercept, categorical rollout-depth indicators with depth 2 as reference, and a map-derived competence margin. The full model added $R_b$. The planned primary statistic was
\begin{equation}
 T_{\mathrm{LOBO}}=1-\frac{\mathrm{SSE}^{\mathrm{LOBO}}_{\mathrm{full}}}
 {\mathrm{SSE}^{\mathrm{LOBO}}_{\mathrm{nuisance}}}.
\end{equation}
A positive result would have required $T_{\mathrm{LOBO}}>0$, a positive full-cohort coefficient for $R_b$, and a one-sided depth-stratified Freedman--Lane permutation $p\leq0.05$, with 99,999 permutations and fold-local scaling. The permutation implementation passed a model-free null audit before the campaign. None of these quantities was computed on scientific outcomes because Stage E was unopened.
