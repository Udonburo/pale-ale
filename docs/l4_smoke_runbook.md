...
Read stdout first. The l4-smoke runner prints these sections:
...
Then inspect the status artifact:

```bash
cat "$OUT_DIR/eval_factory_l4_smoke_status.json"
```

The status artifact records the tier, mode, model id, family set, entrypoint,
executed command, output directory, subprocess return code, per-family results,
and notes. It is an operator artifact for the run.
...
| `--device cuda requested but CUDA is unavailable`. | The command is running on a non-GPU machine, the wrong interpreter, or a VM environment where CUDA/PyTorch is not visible. | Stop. Check `nvidia-smi`, `python3 -c "import sys; print(sys.executable)"`, and `python3 -c "import torch; print(torch.cuda.is_available())"` on the VM. Do not reinterpret this as a Gate12A result. |
...
| `eval_factory_l4_smoke_status.json` is missing or malformed. | The runner failed before writing the artifact, the process was interrupted, or the output directory is not the one being inspected. | Check stdout, stderr, `final pass/fail summary`, and the exact `out-dir` line. Rerun only after fixing the underlying posture or path issue. |
